...
{
  sme_UpdateRoamPrefer5GHz ((tHalHandle) (pHddCtx->hHal),
			    pHddCtx->cfg_ini->nRoamPrefer5GHz);
}

static void
cbNotifySetImmediateRoamRssiDiff (hdd_context_t * pHddCtx,
...
static void
cbNotifySetFastTransitionEnabled (hdd_context_t * pHddCtx,
				  unsigned long NotifyId)
{
  sme_UpdateFastTransitionEnabled ((tHalHandle) (pHddCtx->hHal),
...
{
  sme_setRoamIntraBand ((tHalHandle) (pHddCtx->hHal),
			pHddCtx->cfg_ini->nRoamIntraBand);
}
...
cbNotifySetWESMode (hdd_context_t * pHddCtx, unsigned long NotifyId)
{
  ...
  sme_UpdateWESMode ((tHalHandle) (pHddCtx->hHal),
...
{
  sme_UpdateRoamScanNProbes ((tHalHandle) (pHddCtx->hHal),
			     pHddCtx->cfg_ini->nProbes);
}

static void
cbNotifySetRoamScanHomeAwayTime (hdd_context_t * pHddCtx,
				 unsigned long NotifyId)
{
  sme_UpdateRoamScanHomeAwayTime ((tHalHandle) (pHddCtx->hHal),
...
{
  // at the point this routine is called, the value in the cfg_ini table has already been updated
  sme_UpdateIsFastRoamIniFeatureEnabled ((tHalHandle) (pHddCtx->hHal),
					 pHddCtx->cfg_ini->
					 isFastRoamIniFeatureEnabled);
}

static void
NotifyIsMAWCIniFeatureEnabled (hdd_context_t * pHddCtx,
			       unsigned long NotifyId)
{
  /* at the point this routine is called, the value in the cfg_ini table has already been updated */
  sme_UpdateIsMAWCIniFeatureEnabled ((tHalHandle) (pHddCtx->hHal),
...
{
  // at the point this routine is called, the value in the cfg_ini table has already been updated
  sme_UpdateIsEseFeatureEnabled ((tHalHandle) (pHddCtx->hHal),
...
{
  // at the point this routine is called, the value in the cfg_ini table has already been updated
  sme_UpdateConfigFwRssiMonitoring ((tHalHandle) (pHddCtx->hHal),
				    pHddCtx->cfg_ini->
...
{
  // at the point this routine is called, the value in the cfg_ini table has already been updated
  sme_setNeighborLookupRssiThreshold ((tHalHandle) (pHddCtx->hHal),
				      pHddCtx->cfg_ini->
				      nNeighborLookupRssiThreshold);
}

static void
cbNotifySetNeighborScanPeriod (hdd_context_t * pHddCtx,
			       unsigned long NotifyId)
{
  // at the point this routine is called, the value in the cfg_ini table has already been updated
  sme_setNeighborScanPeriod ((tHalHandle) (pHddCtx->hHal),
...
{
  // at the point this routine is called, the value in the cfg_ini table has already been updated
  sme_setNeighborScanRefreshPeriod ((tHalHandle) (pHddCtx->hHal),
				    pHddCtx->cfg_ini->
...
{
  // at the point this routine is called, the value in the cfg_ini table has already been updated
  sme_UpdateEmptyScanRefreshPeriod ((tHalHandle) (pHddCtx->hHal),
				    pHddCtx->cfg_ini->
...
{
  // at the point this routine is called, the value in the cfg_ini table has already been updated
  sme_setNeighborScanMinChanTime ((tHalHandle) (pHddCtx->hHal),
...
cbNotifySetNeighborScanMaxChanTime (hdd_context_t * pHddCtx,
				    unsigned long NotifyId)
{
  sme_setNeighborScanMaxChanTime ((tHalHandle) (pHddCtx->hHal),
				  pHddCtx->cfg_ini->nNeighborScanMaxChanTime);
}
#endif

static void
cbNotifySetEnableSSR (hdd_context_t * pHddCtx, unsigned long NotifyId)
{
  sme_UpdateEnableSSR ((tHalHandle) (pHddCtx->hHal),
		       pHddCtx->cfg_ini->enableSSR);
}
  ...
  if (0 == pHddCtx->cfg_ini->isRoamOffloadScanEnabled)
    {
      ...
      sme_UpdateEnableFastRoamInConcurrency ((tHalHandle) (pHddCtx->hHal),
					     pHddCtx->cfg_ini->
...
REG_TABLE_ENTRY g_registry_table[] = {
  ...
  REG_VARIABLE (CFG_NEIGHBOR_INITIAL_FORCED_ROAM_TO_5GH_ENABLE_NAME,
		WLAN_PARAM_Integer,
...
/* CFG_VOS_TRACE_ENABLE Parameters */
  ...
  REG_VARIABLE (CFG_PER_ROAM_FULL_SCAN_RSSI_THRESHOLD,
  ...
  REG_VARIABLE (CFG_SPLIT_SCAN_TRAFFIC_MONITOR_THRSHLD_NAME,
		WLAN_PARAM_Integer,
  ...
  REG_VARIABLE (CFG_BTC_CRITICAL_HID_NSNIFF_BLK_GUIDANCE_NAME,
		WLAN_PARAM_Integer,
 ...
 */
static char *
get_next_line (char *str, char *str_end)
{
  char c;
...
void
dump_cfg_ini (tCfgIniEntry * iniTable, unsigned long entries)
{
  unsigned long i;
  ...
  for (i = 0; i < entries; i++)
    {
      VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
		 "%s entry Name=[%s] value=[%s]", WLAN_INI_FILE,
 ...
 */
VOS_STATUS
hdd_parse_config_ini (hdd_context_t * pHddCtx)
{
  int status, i = 0;
  ...
  hddLog (VOS_TRACE_LEVEL_INFO, "%s: qcom_cfg.ini Size %zu", __func__,
	  fw->size);
...
static void
print_hdd_cfg (hdd_context_t * pHddCtx)
{
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "*********Config values in HDD Adapter*******");
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [RTSThreshold] Value = %u",
	     pHddCtx->cfg_ini->RTSThreshold);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [OperatingChannel] Value = [%u]",
	     pHddCtx->cfg_ini->OperatingChannel);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [PowerUsageControl] Value = [%s]",
	     pHddCtx->cfg_ini->PowerUsageControl);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [fIsImpsEnabled] Value = [%u]",
	     pHddCtx->cfg_ini->fIsImpsEnabled);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [AutoBmpsTimerEnabled] Value = [%u]",
	     pHddCtx->cfg_ini->fIsAutoBmpsTimerEnabled);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [nAutoBmpsTimerValue] Value = [%u]",
	     pHddCtx->cfg_ini->nAutoBmpsTimerValue);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [nVccRssiTrigger] Value = [%u]",
	     pHddCtx->cfg_ini->nVccRssiTrigger);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
  ...
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gAPCntryCode] Value =[%c%c%c]",
	     pHddCtx->cfg_ini->apCntryCode[0],
	     pHddCtx->cfg_ini->apCntryCode[1],
  ...
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gApAutoChannelSelection] value = [%u]",
  ...
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [%s] Value = [%u] ", CFG_OVERRIDE_HT40_20_24GHZ_NAME,
	     pHddCtx->cfg_ini->override_ht20_40_24g);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [ChannelBondingMode] Value = [%u]",
	     pHddCtx->cfg_ini->nChannelBondingMode5GHz);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [dot11Mode] Value = [%u]", pHddCtx->cfg_ini->dot11Mode);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [WmmMode] Value = [%u] ", pHddCtx->cfg_ini->WmmMode);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [UapsdMask] Value = [0x%x] ",
	     pHddCtx->cfg_ini->UapsdMask);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [PktClassificationBasis] Value = [%u] ",
	     pHddCtx->cfg_ini->PktClassificationBasis);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [ImplicitQosIsEnabled] Value = [%u]",
	     (int) pHddCtx->cfg_ini->bImplicitQosEnabled);

  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraUapsdVoSrvIntv] Value = [%u] ",
	     pHddCtx->cfg_ini->InfraUapsdVoSrvIntv);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraUapsdVoSuspIntv] Value = [%u] ",
	     pHddCtx->cfg_ini->InfraUapsdVoSuspIntv);

  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraUapsdViSrvIntv] Value = [%u] ",
	     pHddCtx->cfg_ini->InfraUapsdViSrvIntv);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraUapsdViSuspIntv] Value = [%u] ",
	     pHddCtx->cfg_ini->InfraUapsdViSuspIntv);

  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraUapsdBeSrvIntv] Value = [%u] ",
	     pHddCtx->cfg_ini->InfraUapsdBeSrvIntv);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraUapsdBeSuspIntv] Value = [%u] ",
	     pHddCtx->cfg_ini->InfraUapsdBeSuspIntv);

  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraUapsdBkSrvIntv] Value = [%u] ",
	     pHddCtx->cfg_ini->InfraUapsdBkSrvIntv);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraUapsdBkSuspIntv] Value = [%u] ",
	     pHddCtx->cfg_ini->InfraUapsdBkSuspIntv);
#ifdef FEATURE_WLAN_ESE
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraInactivityInterval] Value = [%u] ",
	     pHddCtx->cfg_ini->InfraInactivityInterval);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
  ...
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     ...
	     pHddCtx->cfg_ini->RoamRssiDiff);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [ImmediateRoamRssiDiff] Value = [%u] ",
	     pHddCtx->cfg_ini->nImmediateRoamRssiDiff);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
  ...
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     ...
	     pHddCtx->cfg_ini->InfraDirAcVo);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraNomMsduSizeAcVo] Value = [0x%x] ",
	     pHddCtx->cfg_ini->InfraNomMsduSizeAcVo);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraMeanDataRateAcVo] Value = [0x%x] ",
	     pHddCtx->cfg_ini->InfraMeanDataRateAcVo);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraMinPhyRateAcVo] Value = [0x%x] ",
	     pHddCtx->cfg_ini->InfraMinPhyRateAcVo);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraSbaAcVo] Value = [0x%x] ",
	     pHddCtx->cfg_ini->InfraSbaAcVo);

  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraDirAcVi] Value = [%u] ",
	     pHddCtx->cfg_ini->InfraDirAcVi);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraNomMsduSizeAcVi] Value = [0x%x] ",
	     pHddCtx->cfg_ini->InfraNomMsduSizeAcVi);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraMeanDataRateAcVi] Value = [0x%x] ",
	     pHddCtx->cfg_ini->InfraMeanDataRateAcVi);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraMinPhyRateAcVi] Value = [0x%x] ",
	     pHddCtx->cfg_ini->InfraMinPhyRateAcVi);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraSbaAcVi] Value = [0x%x] ",
	     pHddCtx->cfg_ini->InfraSbaAcVi);

  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraDirAcBe] Value = [%u] ",
	     pHddCtx->cfg_ini->InfraDirAcBe);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraNomMsduSizeAcBe] Value = [0x%x] ",
	     pHddCtx->cfg_ini->InfraNomMsduSizeAcBe);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraMeanDataRateAcBe] Value = [0x%x] ",
	     pHddCtx->cfg_ini->InfraMeanDataRateAcBe);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraMinPhyRateAcBe] Value = [0x%x] ",
	     pHddCtx->cfg_ini->InfraMinPhyRateAcBe);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraSbaAcBe] Value = [0x%x] ",
	     pHddCtx->cfg_ini->InfraSbaAcBe);

  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraDirAcBk] Value = [%u] ",
	     pHddCtx->cfg_ini->InfraDirAcBk);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraNomMsduSizeAcBk] Value = [0x%x] ",
	     pHddCtx->cfg_ini->InfraNomMsduSizeAcBk);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraMeanDataRateAcBk] Value = [0x%x] ",
	     pHddCtx->cfg_ini->InfraMeanDataRateAcBk);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraMinPhyRateAcBk] Value = [0x%x] ",
	     pHddCtx->cfg_ini->InfraMinPhyRateAcBk);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [InfraSbaAcBk] Value = [0x%x] ",
	     pHddCtx->cfg_ini->InfraSbaAcBk);

  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [WfqBkWeight] Value = [%u] ",
	     pHddCtx->cfg_ini->WfqBkWeight);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [WfqBeWeight] Value = [%u] ",
	     pHddCtx->cfg_ini->WfqBeWeight);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [WfqViWeight] Value = [%u] ",
	     pHddCtx->cfg_ini->WfqViWeight);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [WfqVoWeight] Value = [%u] ",
	     pHddCtx->cfg_ini->WfqVoWeight);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [DelayedTriggerFrmInt] Value = [%u] ",
	     pHddCtx->cfg_ini->DelayedTriggerFrmInt);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [BkReorderAgingTime] Value = [%u] ",
	     pHddCtx->cfg_ini->BkReorderAgingTime);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [BeReorderAgingTime] Value = [%u] ",
	     pHddCtx->cfg_ini->BeReorderAgingTime);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [ViReorderAgingTime] Value = [%u] ",
	     pHddCtx->cfg_ini->ViReorderAgingTime);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [VoReorderAgingTime] Value = [%u] ",
	     pHddCtx->cfg_ini->VoReorderAgingTime);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
  ...
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     ...
	     pHddCtx->cfg_ini->fFTResourceReqSupported);
#endif

  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gEnableRoamDelayStats] Value = [%u] ",
	     pHddCtx->cfg_ini->gEnableRoamDelayStats);
#ifdef WLAN_FEATURE_NEIGHBOR_ROAMING
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [nNeighborReassocRssiThreshold] Value = [%u] ",
  ...
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     ...
	     pHddCtx->cfg_ini->nMaxNeighborReqTries);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
  ...
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [nNeighborInitialForcedRoamTo5GhEnable] Value = [%u] ",
	     pHddCtx->cfg_ini->nNeighborInitialForcedRoamTo5GhEnable);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [nWeakZoneRssiThresholdForRoam] Value = [%u] ",
	     pHddCtx->cfg_ini->nWeakZoneRssiThresholdForRoam);
#endif
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [burstSizeDefinition] Value = [0x%x] ",
	     pHddCtx->cfg_ini->burstSizeDefinition);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [tsInfoAckPolicy] Value = [0x%x] ",
	     pHddCtx->cfg_ini->tsInfoAckPolicy);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [rfSettlingTimeUs] Value = [%u] ",
	     pHddCtx->cfg_ini->rfSettlingTimeUs);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [bSingleTidRc] Value = [%u] ",
	     pHddCtx->cfg_ini->bSingleTidRc);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gDynamicPSPollvalue] Value = [%u] ",
	     pHddCtx->cfg_ini->dynamicPsPollValue);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gAddTSWhenACMIsOff] Value = [%u] ",
	     pHddCtx->cfg_ini->AddTSWhenACMIsOff);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gValidateScanList] Value = [%u] ",
	     pHddCtx->cfg_ini->fValidateScanList);

  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gStaKeepAlivePeriod] Value = [%u] ",
	     pHddCtx->cfg_ini->infraStaKeepAlivePeriod);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gApDataAvailPollInterVal] Value = [%u] ",
	     pHddCtx->cfg_ini->apDataAvailPollPeriodInMs);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gEnableBtAmp] Value = [%u] ",
	     pHddCtx->cfg_ini->enableBtAmp);
#ifdef WLAN_BTAMP_FEATURE
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [BtAmpPreferredChannel] Value = [%u] ",
	     pHddCtx->cfg_ini->preferredChannel);
#endif //WLAN_BTAMP_FEATURE
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
  ...
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     ...
	     pHddCtx->cfg_ini->fEnableBeaconEarlyTermination);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [teleBcnWakeupEnable] Value = [%u] ",
	     pHddCtx->cfg_ini->teleBcnWakeupEn);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [transListenInterval] Value = [%u] ",
	     pHddCtx->cfg_ini->nTeleBcnTransListenInterval);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [transLiNumIdleBeacons] Value = [%u] ",
	     pHddCtx->cfg_ini->nTeleBcnTransLiNumIdleBeacons);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [maxListenInterval] Value = [%u] ",
	     pHddCtx->cfg_ini->nTeleBcnMaxListenInterval);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [maxLiNumIdleBeacons] Value = [%u] ",
	     pHddCtx->cfg_ini->nTeleBcnMaxLiNumIdleBeacons);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [bcnEarlyTermWakeInterval] Value = [%u] ",
	     pHddCtx->cfg_ini->bcnEarlyTermWakeInterval);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gEnableBypass11d] Value = [%u] ",
	     pHddCtx->cfg_ini->enableBypass11d);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gEnableDFSChnlScan] Value = [%u] ",
	     pHddCtx->cfg_ini->enableDFSChnlScan);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gEnableDFSPnoChnlScan] Value = [%u] ",
	     pHddCtx->cfg_ini->enableDFSPnoChnlScan);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gReportMaxLinkSpeed] Value = [%u] ",
	     pHddCtx->cfg_ini->reportMaxLinkSpeed);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
  ...
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     ...
	     pHddCtx->cfg_ini->enableFirstScan2GOnly);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [skipDfsChnlInP2pSearch] Value = [%u] ",
	     pHddCtx->cfg_ini->skipDfsChnlInP2pSearch);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [ignoreDynamicDtimInP2pMode] Value = [%u] ",
	     pHddCtx->cfg_ini->ignoreDynamicDtimInP2pMode);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [enableRxSTBC] Value = [%u] ",
	     pHddCtx->cfg_ini->enableRxSTBC);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gEnableLpwrImgTransition] Value = [%u] ",
	     pHddCtx->cfg_ini->enableLpwrImgTransition);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [enable_conc_bmiss] Value = [%u] ",
	     pHddCtx->cfg_ini->enable_conc_bmiss);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [enable_units_bwait] Value = [%u] ",
	     pHddCtx->cfg_ini->enable_units_bwait);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gEnableSSR] Value = [%u] ",
	     pHddCtx->cfg_ini->enableSSR);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gEnableVhtFor24GHzBand] Value = [%u] ",
	     pHddCtx->cfg_ini->enableVhtFor24GHzBand);

  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gEnableTrafficMonitor] Value = [%u] ",
	     pHddCtx->cfg_ini->enableTrafficMonitor);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gTrafficIdleTimeout] Value = [%u] ",
	     pHddCtx->cfg_ini->trafficIdleTimeout);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gFlexConnectPowerFactor] Value = [%u] ",
	     pHddCtx->cfg_ini->flexConnectPowerFactor);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gEnableIbssHeartBeatOffload] Value = [%u] ",
	     pHddCtx->cfg_ini->enableIbssHeartBeatOffload);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gAntennaDiversity] Value = [%u] ",
	     pHddCtx->cfg_ini->antennaDiversity);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gGoLinkMonitorPeriod] Value = [%u]",
	     pHddCtx->cfg_ini->goLinkMonitorPeriod);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gApLinkMonitorPeriod] Value = [%u]",
	     pHddCtx->cfg_ini->apLinkMonitorPeriod);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gGoKeepAlivePeriod] Value = [%u]",
	     pHddCtx->cfg_ini->goKeepAlivePeriod);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gApKeepAlivePeriod]Value = [%u]",
	     pHddCtx->cfg_ini->apKeepAlivePeriod);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gAmsduSupportInAMPDU] Value = [%u] ",
	     pHddCtx->cfg_ini->isAmsduSupportInAMPDU);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [nSelect5GHzMargin] Value = [%u] ",
	     pHddCtx->cfg_ini->nSelect5GHzMargin);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gCoalesingInIBSS] Value = [%u] ",
	     pHddCtx->cfg_ini->isCoalesingInIBSSAllowed);

  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [overrideCountryCode] Value = [%s] ",
	     pHddCtx->cfg_ini->overrideCountryCode);

  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gAsdProbeInterval] Value = [%u]",
	     pHddCtx->cfg_ini->gAsdProbeInterval);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gAsdTriggerThreshold] Value = [%hhd]",
	     pHddCtx->cfg_ini->gAsdTriggerThreshold);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gAsdRTTRssiHystThreshold]Value = [%u]",
	     pHddCtx->cfg_ini->gAsdRTTRssiHystThreshold);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
  ...
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gRoamtoDFSChannel] Value = [%u] ",
	     pHddCtx->cfg_ini->allowDFSChannelRoam);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gMaxConcurrentActiveSessions] Value = [%u] ",
	     pHddCtx->cfg_ini->gMaxConcurrentActiveSessions);
#ifdef WLAN_FEATURE_AP_HT40_24G
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gApHT4024G] Value = [%u]",
	     pHddCtx->cfg_ini->apHT40_24GEnabled);
#endif
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gAcsScanBandPreference] Value = [%u] ",
	     pHddCtx->cfg_ini->acsScanBandPreference);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gACSBandSwitchThreshold] value = [%u]",
	     pHddCtx->cfg_ini->acsBandSwitchThreshold);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gDeferScanTimeInterval] value = [%u]",
	     pHddCtx->cfg_ini->nDeferScanTimeInterval);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gEnableTDLSScan] value = [%u]\n",
	     pHddCtx->cfg_ini->fEnableTDLSScan);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gDxeReplenishRXTimerVal] Value = [%u] ",
	     pHddCtx->cfg_ini->dxeReplenishRXTimerVal);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gDxeSSREnable] Value = [%u] ",
	     pHddCtx->cfg_ini->dxeSSREnable);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [toggleArpBDRates] Value = [%u] ",
	     pHddCtx->cfg_ini->toggleArpBDRates);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [ExtScanConcMode] Value = [%u] ",
	     pHddCtx->cfg_ini->cfgExtScanConcMode);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gEnableForceTargetAssert] Value = [%u] ",
  ...
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [%s] Value = [%u] ",
	     CFG_ENABLE_EDCA_INI_NAME, pHddCtx->cfg_ini->enable_edca_params);

  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [%s] Value = [%u] ",
	     CFG_EDCA_VO_CWMIN_VALUE_NAME, pHddCtx->cfg_ini->edca_vo_cwmin);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [%s] Value = [%u] ",
	     CFG_EDCA_VI_CWMIN_VALUE_NAME, pHddCtx->cfg_ini->edca_vi_cwmin);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [%s] Value = [%u] ",
	     CFG_EDCA_BK_CWMIN_VALUE_NAME, pHddCtx->cfg_ini->edca_bk_cwmin);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [%s] Value = [%u] ",
	     CFG_EDCA_BE_CWMIN_VALUE_NAME, pHddCtx->cfg_ini->edca_be_cwmin);

  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [%s] Value = [%u] ",
	     CFG_EDCA_VO_CWMAX_VALUE_NAME, pHddCtx->cfg_ini->edca_vo_cwmax);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [%s] Value = [%u] ",
	     CFG_EDCA_VI_CWMAX_VALUE_NAME, pHddCtx->cfg_ini->edca_vi_cwmax);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [%s] Value = [%u] ",
	     CFG_EDCA_BK_CWMAX_VALUE_NAME, pHddCtx->cfg_ini->edca_bk_cwmax);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [%s] Value = [%u] ",
	     CFG_EDCA_BE_CWMAX_VALUE_NAME, pHddCtx->cfg_ini->edca_be_cwmax);

  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [%s] Value = [%u] ",
	     CFG_EDCA_VO_AIFS_VALUE_NAME, pHddCtx->cfg_ini->edca_vo_aifs);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [%s] Value = [%u] ",
	     CFG_EDCA_VI_AIFS_VALUE_NAME, pHddCtx->cfg_ini->edca_vi_aifs);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [%s] Value = [%u] ",
	     CFG_EDCA_BK_AIFS_VALUE_NAME, pHddCtx->cfg_ini->edca_bk_aifs);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [%s] Value = [%u] ",
	     CFG_EDCA_BE_AIFS_VALUE_NAME, pHddCtx->cfg_ini->edca_be_aifs);
  ...
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [gMDNSFqdn] Value = [%s]", pHddCtx->cfg_ini->mdns_fqdn);
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
  ...
  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_HIGH,
	     "Name = [%s] Value = [%s] ",
	     CFG_ENABLE_DEFAULT_SAP, pHddCtx->cfg_ini->enabledefaultSAP);
}
...
VOS_STATUS
hdd_cfg_get_config (hdd_context_t * pHddCtx, char *pBuf, int buflen)
{
  unsigned int idx;
  REG_TABLE_ENTRY *pRegEntry = g_registry_table;
  unsigned long cRegTableEntries =
    sizeof (g_registry_table) / sizeof (g_registry_table[0]);
  v_U32_t value;
...
static VOS_STATUS
find_cfg_item (tCfgIniEntry * iniTable, unsigned long entries,
...
static VOS_STATUS
hdd_apply_cfg_ini (hdd_context_t * pHddCtx, tCfgIniEntry * iniTable,
		   unsigned long entries)
{
  ...
  for (idx = 0; idx < cRegTableEntries; idx++, pRegEntry++)
    {
      ...
      match_status =
	find_cfg_item (iniTable, entries, pRegEntry->RegName, &value_str);
	  ...
	  (WLAN_PARAM_HexInteger == pRegEntry->RegType))
	{
	  ...
	  else if (match_status == VOS_STATUS_SUCCESS
		   && (WLAN_PARAM_HexInteger == pRegEntry->RegType))
	    {
	  ...
	  if (pRegEntry->Flags & VAR_FLAGS_RANGE_CHECK)
	      ...
	      if (value > pRegEntry->VarMax)
		{
		  hddLog (LOGE,
			  ...
			  __func__, pRegEntry->RegName, value,
			  pRegEntry->VarMax);
	  ...
	  // If this parameter needs range checking, do it here.
	      ...
	      if (value > pRegEntry->VarMax)
		{
		  hddLog (LOGE,
			  ...
			  __func__, pRegEntry->RegName, value,
			  pRegEntry->VarMax, pRegEntry->VarDefault);
		  value = pRegEntry->VarDefault;
		}
	      ...
	      if (value < pRegEntry->VarMin)
		{
		  hddLog (LOGE,
			  ...
			  __func__, pRegEntry->RegName, value,
			  pRegEntry->VarMin, pRegEntry->VarDefault);
		  value = pRegEntry->VarDefault;
		}
	    }
	  ...
	  if (match_status == VOS_STATUS_SUCCESS)
	      ...
	      if (len_value_str > (pRegEntry->VarSize - 1))
		{
		  hddLog (LOGE,
			  ...
			  __func__, value_str, pRegEntry->RegName,
			  WLAN_INI_FILE);
		  cbOutString =
		    utilMin (strlen ((char *) pRegEntry->VarDefault),
			     pRegEntry->VarSize - 1);
      ...
      else if (WLAN_PARAM_MacAddr == pRegEntry->RegType)
	{
	  ...
	  for (i = 0; i < VOS_MAC_ADDR_SIZE; i++)
	    {
	      ((char *) pField)[i] =
		(char) (parseHexDigit (candidate[i * 2]) * 16 +
			parseHexDigit (candidate[i * 2 + 1]));
	    }
	}
      else
	{
      ...
      if ((match_status == VOS_STATUS_SUCCESS) && (idx < MAX_CFG_INI_ITEMS))
	{
...
eCsrPhyMode
hdd_cfg_xlate_to_csr_phy_mode (eHddDot11Mode dot11Mode)
{
  switch (dot11Mode)
    {
...
static void
hdd_set_btc_config (hdd_context_t * pHddCtx)
{
  ...
  btcParams.btcConsBtSlotsToBlockDuringDhcp =
    pConfig->btcConsBtSlotsToBlockDuringDhcp;
  btcParams.btcA2DPBtSubIntervalsDuringDhcp =
    pConfig->btcA2DPBtSubIntervalsDuringDhcp;
...
static void
hdd_set_power_save_config (hdd_context_t * pHddCtx,
			   tSmeConfigParams * smeConfig)
{
  ...
  sme_GetConfigPowerSave (pHddCtx->hHal, ePMC_BEACON_MODE_POWER_SAVE,
			  &bmpsParams);
  ...
  if (strcmp (pConfig->PowerUsageControl, "Min") == 0)
    {
      ...
      bmpsParams.enableBeaconEarlyTermination =
	pConfig->fEnableBeaconEarlyTermination;
  ...
  if (sme_SetConfigPowerSave
      (pHddCtx->hHal, ePMC_BEACON_MODE_POWER_SAVE,
       &bmpsParams) == eHAL_STATUS_FAILURE)
    {
...
VOS_STATUS
hdd_string_to_u8_array (char *str, tANI_U8 * intArray,
...
#ifdef MDNS_OFFLOAD
int
hdd_string_to_string_array (char *data, uint8_t * datalist,
  ...
  while (str && ('\0' != *str) && (num < max_entries))
      ...
      if ('\0' == *str)
	{
      ...
      *str++ = '\0';
      ...
      strlcpy ((char *) (datalist + (num * max_len_entry)), field, len);
      num++;
    }
  *num_entries = num;
...
v_BOOL_t
hdd_update_config_dat (hdd_context_t * pHddCtx)
{
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_SHORT_GI_20MHZ,
		    pConfig->ShortGI20MhzEnable, NULL,
  ...
  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_CAL_PERIOD, pConfig->CalibrationPeriod, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
  ...
  if (0 != pConfig->Cfg1Id)
    {
      if (ccmCfgSetInt
	  (pHddCtx->hHal, pConfig->Cfg1Id, pConfig->Cfg1Value, NULL,
	   eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
	{
	  fStatus = FALSE;
  ...
  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_BA_AUTO_SETUP, pConfig->BlockAckAutoSetup, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_MAX_RX_AMPDU_FACTOR,
		    pConfig->MaxRxAmpduFactor, NULL,
		    eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_HT_AMPDU_PARAMS_MAX_RX_AMPDU_FACTOR to CCM");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_SHORT_PREAMBLE, pConfig->fIsShortPreamble, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
  ...
  if (pConfig->fIsAutoIbssBssid)
    {
      if (ccmCfgSetStr
	  (pHddCtx->hHal, WNI_CFG_BSSID, (v_U8_t *) "000000000000",
	   sizeof (v_BYTE_t) * VOS_MAC_ADDR_SIZE, NULL,
	   eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
	{
	  fStatus = FALSE;
      ...
      if (VOS_FALSE == vos_is_macaddr_group (&pConfig->IbssBssid))
	{
	  VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_MED,
		     "MAC Addr (IBSS BSSID) read from Registry is: "
		     MAC_ADDRESS_STR,
		     MAC_ADDR_ARRAY (pConfig->IbssBssid.bytes));
	  if (ccmCfgSetStr
	      (pHddCtx->hHal, WNI_CFG_BSSID, pConfig->IbssBssid.bytes,
	       sizeof (v_BYTE_t) * VOS_MAC_ADDR_SIZE, NULL,
	       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
	    {
	      fStatus = FALSE;
  ...
  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_BEACON_INTERVAL, pConfig->nBeaconInterval, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
  ...
  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_CURRENT_RX_ANTENNA, pConfig->nRxAnt, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_CURRENT_RX_ANTENNA configuration info to HAL");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_LOW_GAIN_OVERRIDE, pConfig->fIsLowGainOverride,
       NULL, eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
  ...
  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_RSSI_FILTER_PERIOD, pConfig->nRssiFilterPeriod,
       NULL, eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
  ...
  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_PS_ENABLE_HEART_BEAT,
       pConfig->fEnableFwHeartBeatMonitoring, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_PS_HEART_BEAT configuration info to CCM");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_PS_ENABLE_BCN_FILTER,
       pConfig->fEnableFwBeaconFiltering, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_PS_BCN_FILTER configuration info to CCM");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_PS_ENABLE_RSSI_MONITOR,
       pConfig->fEnableFwRssiMonitoring, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
  ...
  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_PS_DATA_INACTIVITY_TIMEOUT,
       pConfig->nDataInactivityTimeout, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_PS_DATA_INACTIVITY_TIMEOUT configuration info to CCM");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_NTH_BEACON_FILTER, pConfig->nthBeaconFilter,
       NULL, eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_NTH_BEACON_FILTER configuration info to CCM");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_ENABLE_LTE_COEX, pConfig->enableLTECoex, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
  ...
  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_ENABLE_PHY_AGC_LISTEN_MODE,
       pConfig->nEnableListenMode, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_ENABLE_PHY_AGC_LISTEN_MODE to CCM");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_AP_KEEP_ALIVE_TIMEOUT,
       pConfig->apKeepAlivePeriod, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
  ...
  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_GO_KEEP_ALIVE_TIMEOUT,
       pConfig->goKeepAlivePeriod, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
  ...
  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_AP_LINK_MONITOR_TIMEOUT,
       pConfig->apLinkMonitorPeriod, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_AP_LINK_MONITOR_TIMEOUT to CCM");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_GO_LINK_MONITOR_TIMEOUT,
       pConfig->goLinkMonitorPeriod, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_GO_LINK_MONITOR_TIMEOUT to CCM");
    }
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_RRM_ENABLED, pConfig->fRrmEnable,
		    NULL, eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_RRM_ENABLE configuration info to CCM");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_RRM_OPERATING_CHAN_MAX,
       pConfig->nInChanMeasMaxDuration, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_RRM_OPERATING_CHAN_MAX configuration info to CCM");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_RRM_NON_OPERATING_CHAN_MAX,
       pConfig->nOutChanMeasMaxDuration, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_RRM_OUT_CHAN_MAX configuration info to CCM");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_MCAST_BCAST_FILTER_SETTING,
       pConfig->mcastBcastFilterSetting, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
#endif

    if (ccmCfgSetInt
	(pHddCtx->hHal, WNI_CFG_SINGLE_TID_RC, pConfig->bSingleTidRc, NULL,
	 eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
      {
	fStatus = FALSE;
	hddLog (LOGE,
		"Failure: Could not pass on WNI_CFG_SINGLE_TID_RC configuration info to CCM");
      }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_TELE_BCN_WAKEUP_EN, pConfig->teleBcnWakeupEn,
       NULL, eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_TELE_BCN_WAKEUP_EN configuration info to CCM");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_TELE_BCN_TRANS_LI,
       pConfig->nTeleBcnTransListenInterval, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_TELE_BCN_TRANS_LI configuration info to CCM");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_TELE_BCN_MAX_LI,
       pConfig->nTeleBcnMaxListenInterval, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_TELE_BCN_MAX_LI configuration info to CCM");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_TELE_BCN_TRANS_LI_IDLE_BCNS,
       pConfig->nTeleBcnTransLiNumIdleBeacons, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_TELE_BCN_TRANS_LI_IDLE_BCNS configuration info to CCM");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_TELE_BCN_MAX_LI_IDLE_BCNS,
       pConfig->nTeleBcnMaxLiNumIdleBeacons, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_TELE_BCN_MAX_LI_IDLE_BCNS configuration info to CCM");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_RF_SETTLING_TIME_CLK, pConfig->rfSettlingTimeUs,
       NULL, eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_RF_SETTLING_TIME_CLK configuration info to CCM");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_INFRA_STA_KEEP_ALIVE_PERIOD,
       pConfig->infraStaKeepAlivePeriod, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_INFRA_STA_KEEP_ALIVE_PERIOD configuration info to CCM");
    }
  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_DYNAMIC_PS_POLL_VALUE,
       pConfig->dynamicPsPollValue, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_DYNAMIC_PS_POLL_VALUE configuration info to CCM");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_PS_NULLDATA_AP_RESP_TIMEOUT,
       pConfig->nNullDataApRespTimeout, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_PS_NULLDATA_DELAY_TIMEOUT configuration info to CCM");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_AP_DATA_AVAIL_POLL_PERIOD,
       pConfig->apDataAvailPollPeriodInMs, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_AP_DATA_AVAIL_POLL_PERIOD configuration info to CCM");
    }
  if (ccmCfgSetInt
       ...
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_FRAGMENTATION_THRESHOLD configuration info to CCM");
    }
  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_RTS_THRESHOLD, pConfig->RTSThreshold, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_RTS_THRESHOLD configuration info to CCM");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_11D_ENABLED, pConfig->Is11dSupportEnabled, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_11D_ENABLED configuration info to CCM");
    }
  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_HEART_BEAT_THRESHOLD,
       pConfig->HeartbeatThresh24, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_HEART_BEAT_THRESHOLD configuration info to CCM");
    }
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_TX_PWR_CTRL_ENABLE,
		    pConfig->enableAutomaticTxPowerControl, NULL,
		    eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_SHORT_GI_40MHZ,
		    pConfig->ShortGI40MhzEnable, NULL,
		    eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
  ...
  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_ENABLE_MC_ADDR_LIST, pConfig->fEnableMCAddrList,
       NULL, eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
  ...
  if ((pConfig->dot11Mode == eHDD_DOT11_MODE_AUTO) ||
	...
	if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_VHT_AMPDU_LEN_EXPONENT,
			  pConfig->gVhtMaxAmpduLenExp, NULL,
			  eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
	  {
	    fStatus = FALSE;
	    hddLog (LOGE,
		    "Could not pass on WNI_CFG_VHT_AMPDU_LEN_EXPONENT to CCM");
	  }
  ...
  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_NUM_BUFF_ADVERT, pConfig->numBuffAdvert, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_HT_CAP_INFO,
		    temp32, NULL, eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_ASSOC_STA_LIMIT, val,
		    NULL, eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Failure: Could not pass on WNI_CFG_ASSOC_STA_LIMIT configuration info to CCM");
    }
#endif
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_ENABLE_LPWR_IMG_TRANSITION,
		    pConfig->enableLpwrImgTransition, NULL,
		    eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_ENABLE_LPWR_IMG_TRANSITION to CCM");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_ENABLE_MCC_ADAPTIVE_SCHED,
       pConfig->enableMCCAdaptiveScheduler, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
  ...
  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_DISABLE_LDPC_WITH_TXBF_AP,
       pConfig->disableLDPCWithTxbfAP, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_DISABLE_LDPC_WITH_TXBF_AP to CCM");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_DYNAMIC_THRESHOLD_ZERO, pConfig->retryLimitZero,
       NULL, eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_DYNAMIC_THRESHOLD_ZERO to CCM");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_DYNAMIC_THRESHOLD_ONE, pConfig->retryLimitOne,
       NULL, eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
  ...
  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_DYNAMIC_THRESHOLD_TWO, pConfig->retryLimitTwo,
       NULL, eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
  ...
  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_MAX_MEDIUM_TIME, pConfig->cfgMaxMediumTime,
       NULL, eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_TDLS_QOS_WMM_UAPSD_MASK,
		    ...
		    eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_TDLS_QOS_WMM_UAPSD_MASK to CCM");
    }
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_TDLS_PUAPSD_INACT_TIME,
		    ...
		    eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_TDLS_PUAPSD_INACT_TIME to CCM");
    }
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_TDLS_RX_FRAME_THRESHOLD,
		    ...
		    eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_TDLS_RX_FRAME_THRESHOLD to CCM");
    }
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_TDLS_OFF_CHANNEL_ENABLED,
		    ...
		    eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_TDLS_BUF_STA_ENABLED to CCM\n");
    }
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_TDLS_WMM_MODE_ENABLED,
		    ...
		    eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_TDLS_WMM_MODE_ENABLED to CCM\n");
    }
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_ASD_RTT_RSSI_HYST_THRESHOLD,
		    ...
		    eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_ASD_RSSI_HYST_THRESHOLD to CCM");
    }
  ...
  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_RA_FILTER_ENABLE, pConfig->cfgRAFilterEnable,
       NULL, eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
  ...
  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_RA_RATE_LIMIT_INTERVAL,
       pConfig->cfgRARateLimitInterval, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE, "Could not pass on WNI_CFG_RA_FILTER_ENABLE to CCM");
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_PMF_SA_QUERY_RETRY_INTERVAL,
		    ...
		    eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_SA_QUERY_RETRY_INTERVAL to CCM");
    }
#endif
  ...
  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_MAX_UAPSD_CONSEC_TX_CNT_MEAS_WINDOW,
       pConfig->uapsdConsecTxCntMeasWindow, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_MAX_UAPSD_CONSEC_TX_CNT_MEAS_WINDOW");
    }

  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_MAX_UAPSD_CONSEC_RX_CNT_MEAS_WINDOW,
       pConfig->uapsdConsecRxCntMeasWindow, NULL,
       eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_MAX_UAPSD_CONSEC_RX_CNT_MEAS_WINDOW");
    }
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_MAX_PSPOLL_IN_WMM_UAPSD_PS_MODE,
		    ...
		    eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_MAX_PSPOLL_IN_WMM_UAPSD_PS_MODE");
    }
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_MAX_UAPSD_INACTIVITY_INTERVALS,
		    ...
		    eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_MAX_UAPSD_INACTIVITY_INTERVALS");
    }
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_ENABLE_RTSCTS_HTVHT,
    ...
    {
      fStatus = FALSE;
      hddLog (LOGE, "Could not pass on" "WNI_CFG_ENABLE_RTSCTS_HTVHT to CCM");
    }
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_TOGGLE_ARP_BDRATES,
    ...
    {
      fStatus = FALSE;
      hddLog (LOGE, "Could not pass on" "WNI_CFG_TOGGLE_ARP_BDRATES to CCM");
    }
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_BTC_STATIC_OPP_WLAN_IDLE_WLAN_LEN,
		    ...
		    eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_BTC_STATIC_OPP_WLAN_IDLE_WLAN_LEN ");
    }
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_BTC_STATIC_OPP_WLAN_IDLE_BT_LEN,
		    ...
		    eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_BTC_STATIC_OPP_WLAN_IDLE_BT_LEN ");
    }
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_BTC_DISABLE_WLAN_LINK_CRITICAL,
		    ...
		    eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_BTC_DISABLE_WLAN_LINK_CRITICAL");
    }
  ...
  if (ccmCfgSetInt
      (pHddCtx->hHal, WNI_CFG_EXT_SCAN_CONC_MODE, pConfig->cfgExtScanConcMode,
       NULL, eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_DISABLE_BAR_WAKE_UP_HOST,
		    ...
		    NULL, eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_DISABLE_BAR_WAKE_UP_HOST to CCM");
    }
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_SAR_BOFFSET_SET_CORRECTION,
		    ...
		    NULL, eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_SAR_BOFFSET_SET_CORRECTION to CCM");
    }
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_DISABLE_SCAN_DURING_SCO,
		    ...
		    NULL, eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Could not pass on WNI_CFG_DISABLE_SCAN_DURING_SCO to CCM");
    }
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_TRIGGER_NULLFRAME_BEFORE_HB,
		    ...
		    eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Couldn't pass WNI_CFG_TRIGGER_NULLFRAME_BEFORE_HB to CCM");
    }
  ...
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_BTC_2M_DYN_LONG_BT_EXT_LEN,
		    ...
		    eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Couldn't pass WNI_CFG_BTC_2M_DYN_LONG_BT_EXT_LEN to CCM");
    }
  if (ccmCfgSetInt (pHddCtx->hHal, WNI_CFG_BTC_2M_DYN_LONG_NUM_BT_EXT,
		    ...
		    eANI_BOOLEAN_FALSE) == eHAL_STATUS_FAILURE)
    {
      fStatus = FALSE;
      hddLog (LOGE,
	      "Couldn't pass WNI_CFG_BTC_2M_DYN_LONG_NUM_BT_EXT to CCM");
    }
...
VOS_STATUS
hdd_set_sme_config (hdd_context_t * pHddCtx)
{
  ...
  if (NULL == smeConfig)
    {
      VOS_TRACE (VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_ERROR,
		 "%s smeConfig allocation failed", __func__);
  ...
  smeConfig->csrConfig.fEnforceCountryCodeMatch =
    pConfig->fEnforceCountryCodeMatch;
  ...
  smeConfig->csrConfig.min_chntime_btc_esco = pConfig->min_chntime_btc_esco;
  smeConfig->csrConfig.min_chntime_btc_sco = pConfig->min_chntime_btc_sco;
  smeConfig->csrConfig.max_chntime_btc_sco = pConfig->max_chntime_btc_sco;
  ...
  smeConfig->csrConfig.nImmediateRoamRssiDiff =
    pConfig->nImmediateRoamRssiDiff;
  ...
  smeConfig->csrConfig.isPERRoamEnabled = pConfig->isPERRoamEnabled;
...
#ifdef WLAN_FEATURE_NEIGHBOR_ROAMING
  smeConfig->csrConfig.neighborRoamConfig.nNeighborReassocRssiThreshold =
    pConfig->nNeighborReassocRssiThreshold;
  ...
  if ((pConfig->enableFirstScan2GOnly)
      && (pConfig->nBandCapability != eCSR_BAND_24))
    {
      smeConfig->csrConfig.neighborRoamConfig.
	nNeighborInitialForcedRoamTo5GhEnable =
  ...
  hdd_string_to_u8_array (pConfig->neighborScanChanList,
			  ...
			  WNI_CFG_VALID_CHANNEL_LIST_LEN, ",", false);
#endif
  ...
  smeConfig->csrConfig.roamDelayStatsEnabled =
    pHddCtx->cfg_ini->gEnableRoamDelayStats;
  ...
  smeConfig->csrConfig.edca_vo_cwmin = pHddCtx->cfg_ini->edca_vo_cwmin;
  smeConfig->csrConfig.edca_vi_cwmin = pHddCtx->cfg_ini->edca_vi_cwmin;
  smeConfig->csrConfig.edca_bk_cwmin = pHddCtx->cfg_ini->edca_bk_cwmin;
  smeConfig->csrConfig.edca_be_cwmin = pHddCtx->cfg_ini->edca_be_cwmin;

  smeConfig->csrConfig.edca_vo_cwmax = pHddCtx->cfg_ini->edca_vo_cwmax;
  smeConfig->csrConfig.edca_vi_cwmax = pHddCtx->cfg_ini->edca_vi_cwmax;
  smeConfig->csrConfig.edca_bk_cwmax = pHddCtx->cfg_ini->edca_bk_cwmax;
  smeConfig->csrConfig.edca_be_cwmax = pHddCtx->cfg_ini->edca_be_cwmax;

  smeConfig->csrConfig.edca_vo_aifs = pHddCtx->cfg_ini->edca_vo_aifs;
  smeConfig->csrConfig.edca_vi_aifs = pHddCtx->cfg_ini->edca_vi_aifs;
  smeConfig->csrConfig.edca_bk_aifs = pHddCtx->cfg_ini->edca_bk_aifs;
  smeConfig->csrConfig.edca_be_aifs = pHddCtx->cfg_ini->edca_be_aifs;

  smeConfig->csrConfig.sta_auth_retries_for_code17 =
    pHddCtx->cfg_ini->sta_auth_retries_for_code17;
  if (hdd_string_to_u8_array (pHddCtx->cfg_ini->enable_aggr_btc_sco_oui,
			      smeConfig->csrConfig.agg_btc_sco_oui, &i,
			      VENDOR_AP_OUI_SIZE, "-",
  ...
  smeConfig->csrConfig.num_ba_buff = pHddCtx->cfg_ini->numBuffAdvert;
...
VOS_STATUS
hdd_execute_config_command (hdd_context_t * pHddCtx, char *command)
{
  ...
  if ('\0' == *pCmd)
    {
      // did not find '='
      hddLog (LOGE, "%s: invalid command, no '=':[%s]", __func__, command);
      goto done;
    }
  ...
  *pCmd++ = '\0';
  ...
  if ('\0' == *name)
    {
      // did not find a name
      hddLog (LOGE, "%s: invalid command, no <name>:[%s]", __func__, command);
      goto done;
    }
...
tANI_BOOLEAN
hdd_is_okc_mode_enabled (hdd_context_t * pHddCtx)
{
  if (NULL == pHddCtx)
    {
