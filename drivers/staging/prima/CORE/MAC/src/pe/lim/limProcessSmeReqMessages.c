...
static tANI_BOOLEAN __limProcessSmeStartBssReq (tpAniSirGlobal,
						tpSirMsgQ pMsg);
...
static tANI_BOOLEAN __limProcessSmeStopBssReq (tpAniSirGlobal,
					       tpSirMsgQ pMsg);
  ...
  if (TX_TIMER_VALID (pMac->lim.limTimers.gLimBackgroundScanTimer))
    {
      ...
      if (tx_timer_activate (&pMac->lim.limTimers.gLimBackgroundScanTimer) !=
	  TX_SUCCESS)
...
__limFreshScanReqd (tpAniSirGlobal pMac, tANI_U8 returnFreshResults)
{
  ...
  for (i = 0; i < pMac->lim.maxBssId; i++)
	...
	{
	  if (!
	      ((((pMac->lim.gpSession[i].bssType == eSIR_INFRASTRUCTURE_MODE)
...
__limIsDeferedMsgForLearn (tpAniSirGlobal pMac, tpSirMsgQ pMsg)
{
  if (limIsSystemInScanState (pMac))
	...
	{
	  PELOGE (limLog
		  (pMac, LOGE, FL ("Could not defer Msg = %d"), pMsg->type);
	    )return eANI_BOOLEAN_FALSE;
	}
      PELOG1 (limLog
	      (pMac, LOG1, FL ("Defer the message, in learn mode type = %d"),
	       pMsg->type);
	)
...
__limProcessSmeStartReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  limGetSessionInfo (pMac, (tANI_U8 *) pMsgBuf, &smesessionId,
		     &smetransactionId);
  ...
  if (pMac->lim.gLimSmeState == eLIM_SME_OFFLINE_STATE)
      ...
      if (((tSirSmeStartReq *) pMsgBuf)->sendNewBssInd)
	{
	  ...
	  if (cfgSetInt
	      (pMac, WNI_CFG_NEW_BSS_FOUND_IND,
	       ((tSirSmeStartReq *) pMsgBuf)->sendNewBssInd) != eSIR_SUCCESS)
	    {
	      limLog (pMac, LOGP,
		      FL ("could not set NEIGHBOR_BSS_IND at CFG"));
	...
	/**
         ...
         */
      limLog (pMac, LOGE,
	      FL ("Invalid SME_START_REQ received in SME state %d"),
	      pMac->lim.gLimSmeState);
      limPrintSmeState (pMac, LOGE, pMac->lim.gLimSmeState);
      retCode = eSIR_SME_UNEXPECTED_REQ_RESULT_CODE;
    }
  limSendSmeRsp (pMac, eWNI_SME_START_RSP, retCode, smesessionId,
		 smetransactionId);
...
tANI_U32
limGetCenterChannel (tpAniSirGlobal pMac, tANI_U8 primarychanNum,
		     ePhyChanBondState secondaryChanOffset, tANI_U8 chanWidth)
{
  ...
  if ((pMac->lim.gLimSmeState == eLIM_SME_OFFLINE_STATE) ||
      ...
      if (NULL == pSmeStartBssReq)
	{
	  PELOGE (limLog
		  (pMac, LOGE,
		   FL
		   ("AllocateMemory failed for pMac->lim.gpLimStartBssReq"));
	    )
      ...
      if ((limStartBssReqSerDes (pMac, pSmeStartBssReq, (tANI_U8 *) pMsgBuf)
	   == eSIR_FAILURE)
	  || (!limIsSmeStartBssReqValid (pMac, pSmeStartBssReq)))
	{
      ...
      PELOG3 (limLog (pMac, LOG3,
		      FL
		      ("Parsed START_BSS_REQ fields are bssType=%d, channelId=%d"),
	...
	if ((psessionEntry =
	     peFindSessionByBssid (pMac, pSmeStartBssReq->bssId,
				   &sessionId)) != NULL)
	{
	...
	{
	  if ((psessionEntry =
	       peCreateSession (pMac, pSmeStartBssReq->bssId, &sessionId,
				pMac->lim.maxStation)) == NULL)
	    {
	...
	pSmeStartBssReq->beaconInterval;
	...
	pSmeStartBssReq->txLdpcIniFeatureEnabled;
      ...
      limLog (pMac, LOG1, FL ("Session RMF enabled: %d"),
	...
	case eSIR_INFRA_AP_MODE:
	  ...
	  psessionEntry->fwdWPSPBCProbeReq =
	    pSmeStartBssReq->fwdWPSPBCProbeReq;
	  ...
	  || (pSmeStartBssReq->bssType == eSIR_INFRA_AP_MODE))
	{
	  psessionEntry->parsedAssocReq =
	    vos_mem_malloc (psessionEntry->dph.dphHashTable.size *
			    sizeof (tpSirAssocReq));
	  ...
	  vos_mem_set (psessionEntry->parsedAssocReq,
		       (psessionEntry->dph.dphHashTable.size *
      ...
      if (pSmeStartBssReq->channelId)
	{
	  ...
	  psessionEntry->htSecondaryChannelOffset =
	    limGetHTCBState (pSmeStartBssReq->cbMode);
#endif
	}
      else
	{
      ...
      else			// ibss mode
	{
	  ...
	  if ((retStatus =
	       wlan_cfgGetInt (pMac, WNI_CFG_IBSS_AUTO_BSSID,
			       &autoGenBssId)) != eSIR_SUCCESS)
	    {
	      limLog (pMac, LOGP,
	  ...
	  if (!autoGenBssId)
	      ...
	      if (pMlmStartReq->bssId[0] & 0x01)
		{
		  PELOGE (limLog
			  (pMac, LOGE,
			   FL
			   ("Request to start IBSS with group BSSID\n Autogenerating the BSSID"));
		    )autoGenBssId = TRUE;
		}
	    }
	...
	{
	  if (wlan_cfgGetInt (pMac, WNI_CFG_DTIM_PERIOD, &val) !=
	      eSIR_SUCCESS)
	...
	limLog (pMac, LOGP, FL ("could not retrieve Beacon interval"));
      ...
      if (wlan_cfgGetInt (pMac, WNI_CFG_CFP_MAX_DURATION, &val) !=
	  eSIR_SUCCESS)
	limLog (pMac, LOGP, FL ("could not retrieve CFPMaxDuration"));
      ...
      vos_mem_copy ((void *) &pMlmStartReq->rateSet,
		    (void *) &psessionEntry->rateSet,
		    sizeof (tSirMacRateSet));
      ...
      pMlmStartReq->txChannelWidthSet =
	psessionEntry->htRecommendedTxWidthSet;
	...
	{
	  if (cfgSetInt (pMac, WNI_CFG_LOCAL_POWER_CONSTRAINT, 0) !=
	      eSIR_SUCCESS)
	    limLog (pMac, LOGP, FL ("Fail to get WNI_CFG_11H_ENABLED "));
	}
...
free:
  ...
  limSendSmeStartBssRsp (pMac, eWNI_SME_START_BSS_RSP, retCode, psessionEntry,
 ...
 */
void
limGetRandomBssid (tpAniSirGlobal pMac, tANI_U8 * data)
{
  tANI_U32 random[2];
...
static eHalStatus
limSendHalStartScanOffloadReq (tpAniSirGlobal pMac, tpSirSmeScanReq pScanReq)
{
  ...
  len =
    sizeof (tSirScanOffloadReq) + (pScanReq->channelList.numChannels - 1) +
    pScanReq->uIEFieldLen;

  pScanOffloadReq = vos_mem_malloc (len);
  if (NULL == pScanOffloadReq)
    {
      limLog (pMac, LOGE, FL ("AllocateMemory failed for pScanOffloadReq"));
  ...
  vos_mem_copy ((tANI_U8 *) pScanOffloadReq->bssId,
		(tANI_U8 *) pScanReq->bssId, sizeof (tSirMacAddr));
  ...
  vos_mem_copy ((tANI_U8 *) pScanOffloadReq->selfMacAddr,
		(tANI_U8 *) pScanReq->selfMacAddr, sizeof (tSirMacAddr));
...
__limProcessSmeScanReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if (!limIsSmeScanReqValid (pMac, pScanReq) ||
      limIsLinkSuspended (pMac) || limIsChanSwitchRunning (pMac))
    {
      limLog (pMac, LOGE,
	      FL
	      ("Received SME_SCAN_REQ with invalid params or link is suspended %d limIsChanSwitchRunning %d"),
	      limIsLinkSuspended (pMac), limIsChanSwitchRunning (pMac));
      ...
      if (pMac->lim.gLimRspReqd)
	{
	  ...
	  limSendSmeScanRsp (pMac, sizeof (tSirSmeScanRsp),
			     eSIR_SME_INVALID_PARAMETERS, pScanReq->sessionId,
  ...
  if ((pMac->lim.fScanDisabled)
      || (!pMac->lim.gScanInPowersave && !limIsSystemInActiveState (pMac)))
    {
      limLog (pMac, LOGE, FL ("SCAN is disabled or SCAN in power save"
			      " is disabled and system is in power save."));
      limSendSmeScanRsp (pMac, offsetof (tSirSmeScanRsp, bssDescription[0]),
			 eSIR_SME_INVALID_PARAMETERS, pScanReq->sessionId,
			 pScanReq->transactionId);
      return;
    }
  ...
  if (__limFreshScanReqd (pMac, pScanReq->returnFreshResults))
      ...
      if (pScanReq->returnFreshResults & SIR_BG_SCAN_PURGE_RESUTLS)
	{
      ...
      pMac->lim.gLimReturnAfterFirstMatch = pScanReq->returnAfterFirstMatch;
      pMac->lim.gLimBackgroundScanMode = pScanReq->backgroundScanMode;
      ...
      else
	{
	  ...
	  MTRACE (macTrace
		  (pMac, TRACE_CODE_SME_STATE, pScanReq->sessionId,
		   pMac->lim.gLimSmeState));
	  ...
	  if (pScanReq->channelList.numChannels == 0)
		...
		(sizeof (pScanReq->channelList.channelNumber) *
		 (WNI_CFG_VALID_CHANNEL_LIST_LEN - 1)) +
		pScanReq->uIEFieldLen;
	      ...
	      if (NULL == pMlmScanReq)
		{
		  ...
		  limLog (pMac, LOGP,
			  FL
			  ("call to AllocateMemory failed for mlmScanReq (%d)"),
			  len);
	    ...
	    {
	      len =
		sizeof (tLimMlmScanReq) -
		sizeof (pScanReq->channelList.channelNumber) +
		(sizeof (pScanReq->channelList.channelNumber) *
		 pScanReq->channelList.numChannels) + pScanReq->uIEFieldLen;
	  ...
	  if (pScanReq->uIEFieldLen)
	    {
	      vos_mem_copy ((tANI_U8 *) pMlmScanReq +
			    pMlmScanReq->uIEFieldOffset,
			    (tANI_U8 *) pScanReq + (pScanReq->uIEFieldOffset),
			    pScanReq->uIEFieldLen);
	    }
	  ...
	  vos_mem_copy (pMlmScanReq->bssId,
			pScanReq->bssId, sizeof (tSirMacAddr));
	  ...
	  if (pMac->miracast_mode)
	    {
	      pMlmScanReq->minChannelTime =
		DEFAULT_MIN_CHAN_TIME_DURING_MIRACAST;
	      pMlmScanReq->maxChannelTime =
	  ...
	  pMlmScanReq->min_chntime_btc_esco = pScanReq->min_chntime_btc_esco;
	  pMlmScanReq->max_chntime_btc_esco = pScanReq->max_chntime_btc_esco;
	  ...
	  limPostMlmMessage (pMac, LIM_MLM_SCAN_REQ,
			     (tANI_U32 *) pMlmScanReq);
	}
    }				// if ((pMac->lim.gLimSmeState == eLIM_SME_IDLE_STATE) || ...
...
#ifdef WLAN_FEATURE_ROAM_SCAN_OFFLOAD
	  if (pScanReq->
	      returnFreshResults & SIR_BG_SCAN_RETURN_LFR_CACHED_RESULTS)
	    {
...
static void
__limProcessSmeOemDataReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
 ...
 */
static void
__limProcessClearDfsChannelList (tpAniSirGlobal pMac, tpSirMsgQ pMsg)
{
...
//    tSirMacAddr         currentBssId;
  ...
  if (pMac->lim.gLimSmeState == eLIM_SME_IDLE_STATE)
      ...
      if (NULL == pSmeJoinReq)
	{
      ...
      if ((limJoinReqSerDes (pMac, pSmeJoinReq, (tANI_U8 *) pMsgBuf) ==
	   eSIR_FAILURE) || (!limIsSmeJoinReqValid (pMac, pSmeJoinReq)))
	{
      ...
      if (pSmeJoinReq->bsstype == eSIR_BTAMP_AP_MODE)
	{
      ...
      if ((psessionEntry =
	   peFindSessionByBssid (pMac, pSmeJoinReq->bssDescription.bssId,
				 &sessionId)) != NULL)
	{
	  limLog (pMac, LOGE, FL ("Session(%d) Already exists for BSSID: "
				  MAC_ADDRESS_STR " in limSmeState = %d"),
		  sessionId,
	  ...
	  if (psessionEntry->limSmeState == eLIM_SME_LINK_EST_STATE)
	    {
	      ...
	      PELOGW (limLog (pMac, LOGW, FL ("SessionId:%d Received"
					      "SME_JOIN_REQ for currently joined BSS"),
			      sessionId);
		)
      ...
      else			/* Session Entry does not exist for given BSSId */
	{
	  ...
	  if ((psessionEntry =
	       peCreateSession (pMac, pSmeJoinReq->bssDescription.bssId,
      ...
      if (psessionEntry != NULL
	  && pSmeJoinReq->operationalRateSet.numRates > 0)
	{
      ...
      psessionEntry->isAmsduSupportInAMPDU =
	pSmeJoinReq->isAmsduSupportInAMPDU;
      ...
      psessionEntry->beaconParams.beaconInterval =
	pSmeJoinReq->bssDescription.beaconInterval;
      ...
      vendorIE = limGetVendorIEOuiPtr (pMac, SIR_MAC_CISCO_OUI,
				       ...
				       ((tANI_U8 *) & pSmeJoinReq->
					bssDescription.ieFields), ieLen);

      if (NULL != vendorIE)
	{
	  limLog (pMac, LOGE, FL ("DUT is trying to connect to Cisco AP"));
      ...
      if (psessionEntry->vhtCapability)
	{
	  psessionEntry->txBFIniFeatureEnabled =
	    pSmeJoinReq->txBFIniFeatureEnabled;
	  ...
	  if (psessionEntry->txBFIniFeatureEnabled)
	    {
	      ...
	      if (cfgSetInt
		  (pMac, WNI_CFG_VHT_CSN_BEAMFORMEE_ANT_SUPPORTED,
		   pSmeJoinReq->txBFCsnValue) != eSIR_SUCCESS)
		{
		  limLog (pMac, LOGP, FL ("could not set "
	  ...
	  if (cfgSetInt
	      (pMac, WNI_CFG_VHT_MU_BEAMFORMEE_CAP,
	       psessionEntry->txMuBformee) != eSIR_SUCCESS)
	    {
      ...
      else
	{
      ...
      VOS_TRACE (VOS_MODULE_ID_PE, VOS_TRACE_LEVEL_INFO,
		 ...
		 psessionEntry->pePersona,
		 pSmeJoinReq->cbMode, psessionEntry->force_24ghz_in_ht20);
	...
	pSmeJoinReq->isFastTransitionEnabled;
#endif

#ifdef FEATURE_WLAN_LFR
      psessionEntry->isFastRoamIniFeatureEnabled =
	pSmeJoinReq->isFastRoamIniFeatureEnabled;
#endif
      psessionEntry->txLdpcIniFeatureEnabled =
	pSmeJoinReq->txLdpcIniFeatureEnabled;
      ...
      if (NULL == pMlmJoinReq)
	{
	  ...
	  (pMac, WNI_CFG_JOIN_FAILURE_TIMEOUT,
	   (tANI_U32 *) & pMlmJoinReq->joinFailureTimeout) != eSIR_SUCCESS)
	{
      ...
      vos_mem_copy ((void *) &psessionEntry->rateSet,
		    (void *) &pSmeJoinReq->operationalRateSet,
		    sizeof (tSirMacRateSet));
      vos_mem_copy ((void *) &psessionEntry->extRateSet,
		    (void *) &pSmeJoinReq->extendedRateSet,
		    sizeof (tSirMacRateSet));
      //this may not be needed anymore now, as rateSet is now included in the session entry and MLM has session context.
      vos_mem_copy ((void *) &pMlmJoinReq->operationalRateSet,
		    (void *) &psessionEntry->rateSet,
		    sizeof (tSirMacRateSet));
	...
	psessionEntry->pLimJoinReq->bssDescription.length;
      ...
      vos_mem_copy ((tANI_U8 *) & pMlmJoinReq->bssDescription.bssId,
		    (tANI_U8 *) & psessionEntry->pLimJoinReq->bssDescription.
		    bssId,
		    psessionEntry->pLimJoinReq->bssDescription.length + 2);
	...
	cfgGetRegulatoryMaxTransmitPower (pMac,
      ...
      limExtractApCapability (pMac,
			      (tANI_U8 *) psessionEntry->pLimJoinReq->
			      bssDescription.ieFields,
			      GET_IE_LEN_IN_BSS (psessionEntry->pLimJoinReq->
						 bssDescription.length),
			      &psessionEntry->limCurrentBssQosCaps,
			      &psessionEntry->limCurrentBssPropCap,
			      &pMac->lim.gLimCurrentBssUapsd,
			      &localPowerConstraint, psessionEntry);
	...
	psessionEntry->lim11hEnable = 0;
      ...
      limLog (pMac, LOG1,
	      FL ("SME JoinReq:Sessionid %d SSID len %d SSID : %s "
		  "Channel %d, BSSID " MAC_ADDRESS_STR),
	      pMlmJoinReq->sessionId, psessionEntry->ssId.length,
	      psessionEntry->ssId.ssId, psessionEntry->currentOperChannel,
	      MAC_ADDR_ARRAY (psessionEntry->bssId));

      /* Indicate whether spectrum management is enabled */
      psessionEntry->spectrumMgtEnabled = pSmeJoinReq->spectrumMgtIndicator;
	  ...
	  limIsconnectedOnDFSChannel (psessionEntry->currentOperChannel))
	{
      ...
      limLog (pMac, LOG1,
	      FL ("SessionId: %d LIM_MLM_JOIN_REQ is posted to MLM " "SM"),
  ...
  limLog (pMac, LOG1, FL ("Sending failure status limSendSmeJoinReassocRsp"
			  "on sessionid: %d with retCode = %d"), smesessionId,
	  retCode);
  limSendSmeJoinReassocRsp (pMac, eWNI_SME_JOIN_RSP, retCode,
...
tANI_U8
limGetMaxTxPower (tPowerdBm regMax, tPowerdBm apTxPower, tANI_U8 iniTxPower)
{
...
__limProcessSmeReassocReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((limJoinReqSerDes (pMac, (tpSirSmeJoinReq) pReassocReq,
      ...
      (!limIsSmeJoinReqValid (pMac, (tpSirSmeJoinReq) pReassocReq)))
    {
      /// Received invalid eWNI_SME_REASSOC_REQ
      // Log the event
      limLog (pMac, LOGW, FL ("received SME_REASSOC_REQ with invalid data"));
  ...
  if ((psessionEntry =
       peFindSessionByBssid (pMac, pReassocReq->bssDescription.bssId,
			     &sessionId)) == NULL)
    {
  ...
  localPowerConstraint = regMax;
  limExtractApCapability (pMac,
			  (tANI_U8 *) psessionEntry->pLimReAssocReq->
			  bssDescription.ieFields,
			  GET_IE_LEN_IN_BSS (psessionEntry->pLimReAssocReq->
					     bssDescription.length),
...
#endif
  {
    ...
    vos_mem_copy (pMac->lim.gLimReassocSSID.ssId,
		  psessionEntry->ssId.ssId, psessionEntry->ssId.length);
#endif
    psessionEntry->limReassocSSID.length = pReassocReq->ssId.length;
    vos_mem_copy (psessionEntry->limReassocSSID.ssId,
		  pReassocReq->ssId.ssId,
		  psessionEntry->limReassocSSID.length);
  ...
  if (teleBcnEn)
    ...
    {
      if (wlan_cfgGetInt (pMac, WNI_CFG_LISTEN_INTERVAL, &val) !=
	  eSIR_SUCCESS)
	{
	 /**
            * Could not get ListenInterval value
  ...
  limPostMlmMessage (pMac, LIM_MLM_REASSOC_REQ, (tANI_U32 *) pMlmReassocReq);
  return;
...
__limProcessSmeDisassocReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionByBssid (pMac, smeDisassocReq.bssId, &sessionId)) == NULL)
    {
      limLog (pMac, LOGE,
  ...
  limLog (pMac, LOG1, FL ("received DISASSOC_REQ message on sessionid %d"
			  "Systemrole %d Reason: %u SmeState: %d from: "
			  MAC_ADDRESS_STR), smesessionId,
	  psessionEntry->limSystemRole, smeDisassocReq.reasonCode,
	  pMac->lim.gLimSmeState,
	  MAC_ADDR_ARRAY (smeDisassocReq.peerMacAddr));

#ifdef FEATURE_WLAN_DIAG_SUPPORT_LIM	//FEATURE_WLAN_DIAG_SUPPORT
  limDiagEventReport (pMac, WLAN_PE_DIAG_DISASSOC_REQ_EVENT, psessionEntry, 0,
	...
	case eLIM_SME_WT_DEAUTH_STATE:
	  ...
	  MTRACE (macTrace
		  (pMac, TRACE_CODE_SME_STATE, psessionEntry->peSessionId,
		   psessionEntry->limSmeState));
	  limLog (pMac, LOG1,
		  FL ("Rcvd SME_DISASSOC_REQ while in "
		      "SME_WT_DEAUTH_STATE. "));
	  break;
  ...
  limPostMlmMessage (pMac,
		     LIM_MLM_DISASSOC_REQ, (tANI_U32 *) pMlmDisassocReq);
  return;
...
__limProcessSmeDisassocCnf (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionByBssid (pMac, smeDisassocCnf.bssId, &sessionId)) == NULL)
    {
      limLog (pMac, LOGE, FL ("session does not exist for given bssId"));
      return;
    }
  ...
  if (smeDisassocCnf.messageType == eWNI_SME_DISASSOC_CNF)
    limDiagEventReport (pMac, WLAN_PE_DIAG_DISASSOC_CNF_EVENT, psessionEntry,
			(tANI_U16) smeDisassocCnf.statusCode, 0);
  else if (smeDisassocCnf.messageType == eWNI_SME_DEAUTH_CNF)
    limDiagEventReport (pMac, WLAN_PE_DIAG_DEAUTH_CNF_EVENT, psessionEntry,
			(tANI_U16) smeDisassocCnf.statusCode, 0);
#endif //FEATURE_WLAN_DIAG_SUPPORT
  ...
  if ((psessionEntry->limSmeState == eLIM_SME_WT_DISASSOC_STATE) ||
    ...
    {
      pStaDs =
	dphLookupHashEntry (pMac, smeDisassocCnf.peerMacAddr, &aid,
			    &psessionEntry->dph.dphHashTable);
      if (pStaDs == NULL)
	{
	  PELOGE (limLog
		  (pMac, LOGE,
		   FL ("received DISASSOC_CNF for a STA that "
      ...
      if (aid != smeDisassocCnf.assocId)
	{
	  PELOGE (limLog
		  (pMac, LOGE,
		   FL ("same peerMacAddr but assocId is different "
		       "aid=%d, assocId=%d, addr= " MAC_ADDRESS_STR), aid,
		   smeDisassocCnf.assocId,
		   MAC_ADDR_ARRAY (smeDisassocCnf.peerMacAddr));
	    )return;
	}
      /*
	...
	{
	  limLog (pMac, LOGE,
		  FL ("No need to cleanup for addr:" MAC_ADDRESS_STR
		      "as Mlm state is %d"),
		  MAC_ADDR_ARRAY (smeDisassocCnf.peerMacAddr),
      ...
      limCleanUpDisassocDeauthReq (pMac, (char *) &smeDisassocCnf.peerMacAddr,
				   0);
    }
...
__limProcessSmeDeauthReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionByBssid (pMac, smeDeauthReq.bssId, &sessionId)) == NULL)
    {
      limLog (pMac, LOGE, FL ("session does not exist for given bssId"));
      retCode = eSIR_SME_INVALID_PARAMETERS;
  ...
  if ((status == eSIR_FAILURE)
      || (!limIsSmeDeauthReqValid (pMac, &smeDeauthReq, psessionEntry)))
    {
  ...
  limLog (pMac, LOG1, FL ("received DEAUTH_REQ message on sessionid %d "
			  ...
			  MAC_ADDRESS_STR), smesessionId,
	  psessionEntry->limSystemRole, smeDeauthReq.reasonCode,
	  psessionEntry->limSmeState,
	  MAC_ADDR_ARRAY (smeDeauthReq.peerMacAddr));
#ifdef FEATURE_WLAN_DIAG_SUPPORT_LIM	//FEATURE_WLAN_DIAG_SUPPORT
  limDiagEventReport (pMac, WLAN_PE_DIAG_DEAUTH_REQ_EVENT, psessionEntry, 0,
		      smeDeauthReq.reasonCode);
#endif //FEATURE_WLAN_DIAG_SUPPORT
  ...
  if (smeDeauthReq.reasonCode == eLIM_LINK_MONITORING_DEAUTH)
    {
      ...
      limLog (pMac, LOG1, FL ("Set Trigger to eLIM_LINK_MONITORING_DEAUTH"));
      limLog (pMac, LOG1,
  ...
  vos_mem_copy ((tANI_U8 *) & pMlmDeauthReq->peerMacAddr,
		(tANI_U8 *) & smeDeauthReq.peerMacAddr, sizeof (tSirMacAddr));
  ...
  limPostMlmMessage (pMac, LIM_MLM_DEAUTH_REQ, (tANI_U32 *) pMlmDeauthReq);
  return;

sendDeauth:
  limSendSmeDeauthNtf (pMac, smeDeauthReq.peerMacAddr,
		       retCode,
		       deauthTrigger, 1, smesessionId, smetransactionId);
...
__limProcessSmeSetContextReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  PELOG1 (limLog (pMac, LOG1, FL ("received SETCONTEXT_REQ message"));
    );
  ...
  pSetContextReq =
    vos_mem_malloc (sizeof (tSirKeys) * SIR_MAC_MAX_NUM_OF_DEFAULT_KEYS);
  if (NULL == pSetContextReq)
    {
      limLog (pMac, LOGP,
	      FL ("call to AllocateMemory failed for pSetContextReq"));
      return;
    }

  if ((limSetContextReqSerDes (pMac, pSetContextReq, (tANI_U8 *) pMsgBuf) ==
       eSIR_FAILURE) || (!limIsSmeSetContextReqValid (pMac, pSetContextReq)))
    {
  ...
  if (pSetContextReq->keyMaterial.numKeys > SIR_MAC_MAX_NUM_OF_DEFAULT_KEYS)
	...
	)limSendSmeSetContextRsp (pMac, pSetContextReq->peerMacAddr, 1,
  ...
  if ((psessionEntry =
       peFindSessionByBssid (pMac, pSetContextReq->bssId,
			     &sessionId)) == NULL)
    {
...
#ifdef FEATURE_WLAN_DIAG_SUPPORT_LIM	//FEATURE_WLAN_DIAG_SUPPORT
  limDiagEventReport (pMac, WLAN_PE_DIAG_SETCONTEXT_REQ_EVENT, psessionEntry,
		      0, 0);
#endif //FEATURE_WLAN_DIAG_SUPPORT
  ...
  if ((((psessionEntry->limSystemRole == eLIM_STA_ROLE)
      ...
      if (NULL == pMlmSetKeysReq)
	{
	  // Log error
	  limLog (pMac, LOGP,
		  FL ("call to AllocateMemory failed for mlmSetKeysReq"));
	  goto end;
	}
	...
	{
	  limLog (pMac, LOGP,
		  FL ("Num of keys exceeded max num of default keys limit"));
	  goto end;
	}
      ...
      vos_mem_copy ((tANI_U8 *) & pMlmSetKeysReq->key,
		    ...
		    sizeof (tSirKeys) *
		    (pMlmSetKeysReq->numKeys ? pMlmSetKeysReq->numKeys : 1));
	  ...
	  if (pSetContextReq->keyMaterial.key[0].keyLength)
	    {
	      ...
	      vos_mem_copy ((tANI_U8 *) & psessionEntry->
			    WEPKeyMaterial[keyId],
			    (tANI_U8 *) & pSetContextReq->keyMaterial,
	      ...
	      for (i = 0; i < SIR_MAC_MAX_NUM_OF_DEFAULT_KEYS; i++)
		{
		  vos_mem_copy ((tANI_U8 *) & pMlmSetKeysReq->key[i],
				(tANI_U8 *) psessionEntry->WEPKeyMaterial[i].
				key, sizeof (tSirKeys));
		}
	    }
	}

      limPostMlmMessage (pMac, LIM_MLM_SETKEYS_REQ,
  ...
  else
    {
      limLog (pMac, LOGE,
	      FL
	      ("received unexpected SME_SETCONTEXT_REQ for role %d, state=%d"),
...
__limProcessSmeRemoveKeyReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  limLog (pMac, LOG1, FL ("received SME_REMOVEKEY_REQ message"));
  ...
  if ((limRemoveKeyReqSerDes (pMac,
			      ...
			      (tANI_U8 *) pMsgBuf) == eSIR_FAILURE))
    {
      ...
      if ((psessionEntry =
	   peFindSessionByBssid (pMac, pRemoveKeyReq->bssId,
				 &sessionId)) == NULL)
	{
	  limLog (pMac, LOGE, FL ("session does not exist for given bssId"));
	  //goto end;
  ...
  if ((psessionEntry =
       peFindSessionByBssid (pMac, pRemoveKeyReq->bssId, &sessionId)) == NULL)
    {
      limLog (pMac, LOGE, FL ("session does not exist for given bssId"));
      limSendSmeRemoveKeyRsp (pMac,
			      pRemoveKeyReq->peerMacAddr,
  ...
  if ((((psessionEntry->limSystemRole == eLIM_STA_ROLE)
  ...
  else
    {
      limLog (pMac, LOGE,
	      FL
	      ("received unexpected SME_REMOVEKEY_REQ for role %d, state=%d"),
	      psessionEntry->limSystemRole, psessionEntry->limSmeState);
      limPrintSmeState (pMac, LOGE, psessionEntry->limSmeState);
...
void
limProcessSmeGetScanChannelInfo (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  tSirMsgQ mmhMsg;
  ...
  limLog (pMac, LOG1,
	  ...
	  limMsgStr (eWNI_SME_GET_SCANNED_CHANNEL_RSP),
	  pMac->lim.scanChnInfo.numChnInfo);

  len =
    sizeof (tSmeGetScanChnRsp) + (pMac->lim.scanChnInfo.numChnInfo -
				  1) * sizeof (tLimScanChn);
  ...
  if (pMac->fScanOffload)
    {
      limGetSessionInfo (pMac, (tANI_U8 *) pMsgBuf, &sessionId,
			 &transactionId);
...
void
limProcessSmeGetAssocSTAsInfo (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if (!limIsSmeGetAssocSTAsReqValid
      (pMac, &getAssocSTAsReq, (tANI_U8 *) pMsgBuf))
    {
      limLog (pMac, LOGE,
...
*/
  ...
  if ((psessionEntry =
       peFindSessionByBssid (pMac, getAssocSTAsReq.bssId,
			     &sessionId)) == NULL)
    {
      limLog (pMac, LOGE, FL ("session does not exist for given bssId"));
      goto limAssocStaEnd;
  ...
  for (assocId = 0; assocId < psessionEntry->dph.dphHashTable.size; assocId++)	// Softap dphHashTable.size = 8
    {
      pStaDs =
	dphGetHashEntry (pMac, assocId, &psessionEntry->dph.dphHashTable);
	...
	continue;
      ...
      if (pStaDs->valid)
	{
	  vos_mem_copy ((tANI_U8 *) & pAssocStasTemp->staMac, (tANI_U8 *) & pStaDs->staAddr, sizeof (v_MACADDR_t));	// Mac address
  ...
  if (pSapEventCallback != NULL)
    {
      ...
      sapEvent.sapevt.sapAssocStaListEvent.pAssocStas =
	(tpSap_AssocMacAddr) getAssocSTAsReq.pAssocStasArray;
 ...
 */
void
limProcessSmeGetWPSPBCSessions (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if (limIsSmeGetWPSPBCSessionsReqValid
      (pMac, &GetWPSPBCSessionsReq, (tANI_U8 *) pMsgBuf) != eSIR_SUCCESS)
    {
      limLog (pMac, LOGE,
	      FL ("received invalid eWNI_SME_GET_ASSOC_STAS_REQ message"));
      return;
  ...
  // Find PE session Entry
  if ((psessionEntry =
       peFindSessionByBssid (pMac, GetWPSPBCSessionsReq.bssId,
			     &sessionId)) == NULL)
    {
      limLog (pMac, LOGE, FL ("session does not exist for given bssId"));
  ...
  if (vos_mem_compare
      (zeroMac, GetWPSPBCSessionsReq.pRemoveMac, sizeof (tSirMacAddr)))
    {				//This is GetWpsSession call

      limGetWPSPBCSessions (pMac,
			    sapEvent.sapevt.sapGetWPSPBCSessionEvent.addr.
			    bytes,
			    sapEvent.sapevt.sapGetWPSPBCSessionEvent.UUID_E,
			    &sapEvent.sapevt.sapGetWPSPBCSessionEvent.
			    wpsPBCOverlap, psessionEntry);
    }
  else
    {
      limRemovePBCSessions (pMac, GetWPSPBCSessionsReq.pRemoveMac,
			    psessionEntry);
  ...
  PELOG4 (limLog
	  (pMac, LOGE, FL ("wpsPBCOverlap %d"),
	   sapEvent.sapevt.sapGetWPSPBCSessionEvent.wpsPBCOverlap);
    )PELOG4 (limPrintMacAddr
	     (pMac, sapEvent.sapevt.sapGetWPSPBCSessionEvent.addr.bytes,
	      LOG4);
    )sapEvent.sapevt.sapGetWPSPBCSessionEvent.status = VOS_STATUS_SUCCESS;

limGetWPSPBCSessionsEnd:
  pSapEventCallback =
    (tpWLAN_SAPEventCB) GetWPSPBCSessionsReq.pSapEventCallback;
...
limProcessTkipCounterMeasures (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if (limTkipCntrMeasReqSerDes (pMac, &tkipCntrMeasReq, (tANI_U8 *) pMsgBuf)
      != eSIR_SUCCESS)
    {
      limLog (pMac, LOGE,
...
__limHandleSmeStopBssRequest (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((limStopBssReqSerDes (pMac, &stopBssReq, (tANI_U8 *) pMsgBuf) !=
       eSIR_SUCCESS) || !limIsSmeStopBssReqValid (pMsgBuf))
    {
      PELOGW (limLog
	      (pMac, LOGW, FL ("received invalid SME_STOP_BSS_REQ message"));
	)
	/// Send Stop BSS response to host
	limSendSmeRsp (pMac, eWNI_SME_STOP_BSS_RSP,
  ...
  if ((psessionEntry =
       peFindSessionByBssid (pMac, stopBssReq.bssId, &sessionId)) == NULL)
    {
      limLog (pMac, LOGW, FL ("session does not exist for given BSSID "));
...
#ifdef FEATURE_WLAN_DIAG_SUPPORT_LIM	//FEATURE_WLAN_DIAG_SUPPORT
  limDiagEventReport (pMac, WLAN_PE_DIAG_STOP_BSS_REQ_EVENT, psessionEntry, 0,
		      0);
#endif //FEATURE_WLAN_DIAG_SUPPORT
  ...
  if ((psessionEntry->limSmeState != eLIM_SME_NORMAL_STATE) ||	/* Added For BT -AMP Support */
	...
	/**
      ...
      limLog (pMac, LOGE,
	      FL
	      ("received unexpected SME_STOP_BSS_REQ in state %d, for role %d"),
	      psessionEntry->limSmeState, psessionEntry->limSystemRole);
      limPrintSmeState (pMac, LOGE, psessionEntry->limSmeState);
      /// Send Stop BSS response to host
      limSendSmeRsp (pMac, eWNI_SME_STOP_BSS_RSP,
  ...
  PELOGW (limLog
	  (pMac, LOGW, FL ("RECEIVED STOP_BSS_REQ with reason code=%d"),
  ...
  if (((eLIM_STA_IN_IBSS_ROLE != psessionEntry->limSystemRole) &&
	...
	limSendDisassocMgmtFrame (pMac, eSIR_MAC_DEAUTH_LEAVING_BSS_REASON,
  ...
  for (i = 1; i < pMac->lim.gLimAssocStaLimit; i++)
	...
	{
	  limLog (pMac, LOGE, FL ("limDelSta failed with Status : %d"),
		  status);
	  VOS_ASSERT (0);
	}
    }
...
void
limProcessSmeDelBssRsp (tpAniSirGlobal pMac,
...
void
__limProcessSmeAssocCnfNew (tpAniSirGlobal pMac, tANI_U32 msgType,
			    tANI_U32 * pMsgBuf)
{
  ...
  if ((limAssocCnfSerDes (pMac, &assocCnf, (tANI_U8 *) pMsgBuf) ==
       eSIR_FAILURE) || !__limIsSmeAssocCnfValid (&assocCnf))
    {
  ...
  if ((psessionEntry =
       peFindSessionByBssid (pMac, assocCnf.bssId, &sessionId)) == NULL)
    {
      limLog (pMac, LOGE, FL ("session does not exist for given bssId"));
      goto end;
    }
  ...
  if (((psessionEntry->limSystemRole != eLIM_AP_ROLE)
    ...
    {
      limLog (pMac, LOGE,
	      FL ("Received unexpected message %X in state %d, in role %d"),
  ...
  pStaDs =
    dphGetHashEntry (pMac, assocCnf.aid, &psessionEntry->dph.dphHashTable);

  if (pStaDs == NULL)
    {
      limLog (pMac, LOGE,
	      FL ("Received invalid message %X due to no STA context, "
		  "for aid %d, peer "), msgType, assocCnf.aid);
       ...
       **/
      limSendSmeDisassocNtf (pMac, assocCnf.peerMacAddr,
			     eSIR_SME_STA_NOT_ASSOCIATED,
			     eLIM_PEER_ENTITY_DISASSOC, assocCnf.aid,
			     psessionEntry->smeSessionId,
			     psessionEntry->transactionId, psessionEntry);
      goto end;
    }
  if ((pStaDs && ((!vos_mem_compare ((tANI_U8 *) pStaDs->staAddr, (tANI_U8 *) assocCnf.peerMacAddr, sizeof (tSirMacAddr))) || (pStaDs->mlmStaContext.mlmState != eLIM_MLM_WT_ASSOC_CNF_STATE) || ((pStaDs->mlmStaContext.subType == LIM_ASSOC) && (msgType != eWNI_SME_ASSOC_CNF)) || ((pStaDs->mlmStaContext.subType == LIM_REASSOC) && (msgType != eWNI_SME_ASSOC_CNF)))))	// since softap is passing this as ASSOC_CNF and subtype differs
    {
      limLog (pMac, LOGE,
  ...
  if (assocCnf.statusCode == eSIR_SME_SUCCESS)
	...
	pStaDs->mlmStaContext.updateContext = 1;
      limLog (pMac, LOG1,
	      FL ("Receive Assoc Cnf with status Code : %d(assoc id=%d) "),
	      assocCnf.statusCode, pStaDs->assocId);
      limRejectAssociation (pMac, pStaDs->staAddr,
			    pStaDs->mlmStaContext.subType, true,
			    pStaDs->mlmStaContext.authType, pStaDs->assocId,
			    true, eSIR_MAC_UNSPEC_FAILURE_STATUS,
			    psessionEntry);
    }
...
__lim_process_sme_assoc_offload_cnf (tpAniSirGlobal pmac,
				     tANI_U32 msg_type, tANI_U32 * pmsg_buf)
{
  ...
  sta_ds = dphGetHashEntry (pmac,
			    assoc_cnf.aid, &psession_entry->dph.dphHashTable);
  if (sta_ds != NULL)
    {
      aid = sta_ds->assocId;
      /* Deactivate/delete CNF_WAIT timer since ASSOC_CNF
       * has been received */
      limDeactivateAndChangePerStaIdTimer (pmac, eLIM_CNF_WAIT_TIMER, aid);
    }
  else
    {
  ...
  if (assoc_cnf.statusCode == eSIR_SME_SUCCESS)
    {
      ...
      limLog (pmac, LOG1,
	      FL ("Set mlmState to eLIM_MLM_LINK_ESTABLISHED_STATE"));
    }
  ...
  if ((psession_entry != NULL) && (sta_ds != NULL))
      ...
      if (psession_entry->parsedAssocReq[aid] != NULL)
	{
	  if (((tpSirAssocReq)
	    ...
	    {
	      vos_mem_free (((tpSirAssocReq)
			     (psession_entry->parsedAssocReq[aid]))->
			    assocReqFrame);
	      ((tpSirAssocReq) (psession_entry->parsedAssocReq[aid]))->
		assocReqFrame = NULL;
	    }
...
__limProcessSmeAddtsReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionByBssid (pMac, pSirAddts->bssId, &sessionId)) == NULL)
    {
      limLog (pMac, LOGE, "Session Does not exist for given bssId");
      return;
    }
#ifdef FEATURE_WLAN_DIAG_SUPPORT_LIM	//FEATURE_WLAN_DIAG_SUPPORT
  limDiagEventReport (pMac, WLAN_PE_DIAG_ADDTS_REQ_EVENT, psessionEntry, 0,
		      0);
#endif //FEATURE_WLAN_DIAG_SUPPORT
  ...
  if ((psessionEntry->limSystemRole != eLIM_STA_ROLE)
	...
	)limSendSmeAddtsRsp (pMac, pSirAddts->rspReqd, eSIR_FAILURE,
  ...
  pStaDs =
    dphGetHashEntry (pMac, DPH_STA_HASH_INDEX_PEER,
		     &psessionEntry->dph.dphHashTable);
  ...
  if ((psessionEntry->limSmeState != eLIM_SME_ASSOCIATED_STATE) &&
      (psessionEntry->limSmeState != eLIM_SME_LINK_EST_STATE))
    {
      limLog (pMac, LOGE, "AddTs received in invalid LIMsme state (%d)",
	      psessionEntry->limSmeState);
      limSendSmeAddtsRsp (pMac, pSirAddts->rspReqd, eSIR_FAILURE,
			  psessionEntry, pSirAddts->req.tspec, smesessionId,
			  smetransactionId);
      return;
    }
  ...
  vos_mem_copy ((tANI_U8 *) & pMac->lim.gLimAddtsReq, (tANI_U8 *) pSirAddts,
		sizeof (tSirAddtsReq));

  // ship out the message now
  limSendAddtsReqActionFrame (pMac, peerMac, &pSirAddts->req, psessionEntry);
  ...
  else if (wlan_cfgGetInt (pMac, WNI_CFG_ADDTS_RSP_TIMEOUT, &timeout) !=
	   eSIR_SUCCESS)
    {
      limLog (pMac, LOGP,
...
__limProcessSmeDeltsReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionByBssid (pMac, pDeltsReq->bssId, &sessionId)) == NULL)
    {
      limLog (pMac, LOGE, "Session Does not exist for given bssId");
      status = eSIR_FAILURE;
      goto end;
...
#ifdef FEATURE_WLAN_DIAG_SUPPORT_LIM	//FEATURE_WLAN_DIAG_SUPPORT
  limDiagEventReport (pMac, WLAN_PE_DIAG_DELTS_REQ_EVENT, psessionEntry, 0,
		      0);
#endif //FEATURE_WLAN_DIAG_SUPPORT


  if (eSIR_SUCCESS !=
      limValidateDeltsReq (pMac, pDeltsReq, peerMacAddr, psessionEntry))
    {
      PELOGE (limLog (pMac, LOGE, FL ("limValidateDeltsReq failed"));
	)status = eSIR_FAILURE;
      limSendSmeDeltsRsp (pMac, pDeltsReq, eSIR_FAILURE, psessionEntry,
			  smesessionId, smetransactionId);
      return;
    }
  ...
  limSendDeltsReqActionFrame (pMac, peerMacAddr,
			      pDeltsReq->req.wmeTspecPresent,
			      &pDeltsReq->req.tsinfo, &pDeltsReq->req.tspec,
			      psessionEntry);
  ...
  limSendSmeDeltsRsp (pMac, pDeltsReq, eSIR_SUCCESS, psessionEntry,
		      smesessionId, smetransactionId);
}
...
limProcessSmeAddtsRspTimeout (tpAniSirGlobal pMac, tANI_U32 param)
{
  ...
  limSendSmeAddtsRsp (pMac, true, eSIR_SME_ADDTS_RSP_TIMEOUT, psessionEntry,
		      pMac->lim.gLimAddtsReq.req.tspec,
		      psessionEntry->smeSessionId,
...
__limProcessSmeStatsRequest (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionByBssid (pMac, pStatsReq->bssId, &sessionId)) == NULL)
    {
      limLog (pMac, LOGE, FL ("session does not exist for given bssId"));
...
__limProcessSmeUpdateAPWPSIEs (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  PELOG1 (limLog (pMac, LOG1, FL ("received UPDATE_APWPSIEs_REQ message"));
    );
  ...
  if (NULL == pUpdateAPWPSIEsReq)
    {
      limLog (pMac, LOGP,
	      FL ("call to AllocateMemory failed for pUpdateAPWPSIEsReq"));
      return;
    }

  if ((limUpdateAPWPSIEsReqSerDes
       (pMac, pUpdateAPWPSIEsReq, (tANI_U8 *) pMsgBuf) == eSIR_FAILURE))
    {
      limLog (pMac, LOGW, FL ("received invalid SME_SETCONTEXT_REQ message"));
      goto end;
    }
  ...
  if ((psessionEntry =
       peFindSessionByBssid (pMac, pUpdateAPWPSIEsReq->bssId,
			     &sessionId)) == NULL)
    {
      limLog (pMac, LOGW, FL ("Session does not exist for given BSSID"));
      goto end;
  ...
  vos_mem_copy (&psessionEntry->APWPSIEs, &pUpdateAPWPSIEsReq->APWPSIEs,
		sizeof (tSirAPWPSIEs));
...
__limProcessSmeHideSSID (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  limLog (pMac, LOG1, FL ("received SME_HIDE_SSID message"));
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pUpdateParams->sessionId)) == NULL)
    {
...
__limProcessSmeSetWPARSNIEs (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if (NULL == pUpdateAPWPARSNIEsReq)
    {
      limLog (pMac, LOGP,
	      FL ("call to AllocateMemory failed for pUpdateAPWPARSNIEsReq"));
      return;
    }

  if ((limUpdateAPWPARSNIEsReqSerDes
       (pMac, pUpdateAPWPARSNIEsReq, (tANI_U8 *) pMsgBuf) == eSIR_FAILURE))
    {
      limLog (pMac, LOGW, FL ("received invalid SME_SETCONTEXT_REQ message"));
      goto end;
    }
  ...
  if ((psessionEntry =
       peFindSessionByBssid (pMac, pUpdateAPWPARSNIEsReq->bssId,
			     &sessionId)) == NULL)
    {
      limLog (pMac, LOGW, FL ("Session does not exist for given BSSID"));
      goto end;
...
__limProcessSmeChangeBI (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  PELOG1 (limLog (pMac, LOG1, FL ("received Update Beacon Interval message"));
    );
  ...
  if ((psessionEntry =
       peFindSessionByBssid (pMac, pChangeBIParams->bssId,
			     &sessionId)) == NULL)
    {
      limLog (pMac, LOGE, FL ("Session does not exist for given BSSID"));
      return;
  ...
  if (pMac->sch.schObject.gSchBeaconInterval !=
	...
	pChangeBIParams->beaconInterval;
      ...
      PELOG1 (limLog (pMac, LOG1,
		      FL ("LIM send update BeaconInterval Indication : %d"),
		      pChangeBIParams->beaconInterval);
	);
...
#ifdef WLAN_FEATURE_AP_HT40_24G
static void
__limProcessSmeSetHT2040Mode (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  PELOG1 (limLog (pMac, LOGRW, FL ("received Set HT 20/40 mode message"));
    );
...
limProcessSmeDelBaPeerInd (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionByBssid (pMac, pSmeDelBAPeerInd->bssId,
			     &sessionId)) == NULL)
    {
      limLog (pMac, LOGE, FL ("session does not exist for given bssId"));
      return;
  ...
  limLog (pMac, LOGW,
	  FL ("called with staId = %d, tid = %d, baDirection = %d"),
	  pSmeDelBAPeerInd->staIdx, pSmeDelBAPeerInd->baTID,
	  pSmeDelBAPeerInd->baDirection);

  pSta =
    dphLookupAssocId (pMac, pSmeDelBAPeerInd->staIdx, &assocId,
		      &psessionEntry->dph.dphHashTable);
  if (eSIR_SUCCESS !=
      limPostMlmDelBAReq (pMac, pSta, pSmeDelBAPeerInd->baDirection,
			  pSmeDelBAPeerInd->baTID,
			  eSIR_MAC_UNSPEC_FAILURE_REASON, psessionEntry))
    {
      limLog (pMac, LOGW, FL ("Failed to post LIM_MLM_DELBA_REQ to "));
      if (pSta)
...
void
__limProcessReportMessage (tpAniSirGlobal pMac, tpSirMsgQ pMsg)
{
#ifdef WLAN_FEATURE_VOWIFI
  switch (pMsg->type)
    {
	...
	if ((psessionEntry =
	     peFindSessionByBssid (pMac, pBcnReport->bssId,
				   &sessionId)) == NULL)
	  {
	    limLog (pMac, LOGE, "Session Does not exist for given bssId");
	    return;
 ...
 */
tSirRetStatus
limSendSetMaxTxPowerReq (tpAniSirGlobal pMac, tPowerdBm txPower,
			 tpPESession pSessionEntry)
{
  ...
  if (pSessionEntry == NULL)
    {
      PELOGE (limLog
	      (pMac, LOGE, "%s:%d: Inavalid parameters", __func__, __LINE__);
	)return eSIR_FAILURE;
    }
  ...
  limLog (pMac, LOG1,
	  FL
	  (" Allocated memory for pMaxTxParams, which will be freed in other module"));
#endif
  ...
  vos_mem_copy (pMaxTxParams->bssId, pSessionEntry->bssId,
		sizeof (tSirMacAddr));
  vos_mem_copy (pMaxTxParams->selfStaMacAddr, pSessionEntry->selfMacAddr,
		sizeof (tSirMacAddr));
...
__limProcessSmeAddStaSelfReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  vos_mem_copy (pAddStaSelfParams->selfMacAddr, pSmeReq->selfMacAddr,
		sizeof (tSirMacAddr));
  ...
  PELOGW (limLog
	  (pMac, LOG1, FL ("sending SIR_HAL_ADD_STA_SELF_REQ msg to HAL"));
    )MTRACE (macTraceMsgTx (pMac, NO_SESSION, msg.type));
...
__limProcessSmeDelStaSelfReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  vos_mem_copy (pDelStaSelfParams->selfMacAddr, pSmeReq->selfMacAddr,
		sizeof (tSirMacAddr));
  ...
  PELOGW (limLog
	  (pMac, LOG1, FL ("sending SIR_HAL_DEL_STA_SELF_REQ msg to HAL"));
    )MTRACE (macTraceMsgTx (pMac, NO_SESSION, msg.type));
...
__limProcessSmeRegisterMgmtFrameReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if (pSmeReq->registerFrame)
    {
      pLimMgmtRegistration =
	vos_mem_malloc (sizeof (tLimMgmtFrameRegistration) +
			pSmeReq->matchLen);
...
__limInsertSingleShotNOAForScan (tpAniSirGlobal pMac, tANI_U32 noaDuration)
{
  ...
  if (tx_timer_activate (&pMac->lim.limTimers.gLimP2pSingleShotNoaInsertTimer)
      == TX_TIMER_ERROR)
    {
      ...
      limLog (pMac, LOGP,
	      FL
	      ("could not activate Insert Single Shot NOA during scan timer"));

      // send the scan response back with status failure and do not even call insert NOA
      limSendSmeScanRsp (pMac, sizeof (tSirSmeScanRsp), eSIR_SME_SCAN_FAILED,
			 pMac->lim.gSmeSessionId, pMac->lim.gTransactionId);
...
static void
__limRegisterDeferredSmeReqForNOAStart (tpAniSirGlobal pMac, tANI_U16 msgType,
					tANI_U32 * pMsgBuf)
{
...
static void
__limDeregisterDeferredSmeReqAfterNOAStart (tpAniSirGlobal pMac)
{
...
static tANI_U32
limCalculateNOADuration (tpAniSirGlobal pMac, tANI_U16 msgType,
			 tANI_U32 * pMsgBuf, tANI_BOOLEAN isPassiveScan)
{
  ...
  switch (msgType)
	...
	if (wlan_cfgGetInt (pMac, WNI_CFG_PASSIVE_MAXIMUM_CHANNEL_TIME, &val)
	    != eSIR_SUCCESS)
	  {
	    /*
	     ...
	     */
	    limLog (pMac, LOGP,
		    FL ("could not retrieve passive max channel value"));
	...
	for (i = 0; i < pScanReq->channelList.numChannels; i++)
	  {
		...
		noaDuration +=
		  pScanReq->minChannelTime + pScanReq->maxChannelTime;
	      }
	  }
...
void
limProcessRegdDefdSmeReqAfterNOAStart (tpAniSirGlobal pMac)
{
  ...
  if ((pMac->lim.gDeferMsgTypeForNOA != 0)
  ...
  else
    {
      limLog (pMac, LOGW,
	      FL
	      ("start received from FW when no sme deferred msg pending. Do nothing."
...
__limProcessSmeSpoofMacAddrRequest (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  vos_mem_copy (pSpoofMacAddrParams->macAddr, pSmeReq->macAddr,
		sizeof (tSirMacAddr));

  vos_mem_copy (pMac->lim.spoofMacAddr, pSmeReq->macAddr,
		VOS_MAC_ADDRESS_LEN);
  ...
  limLog (pMac, LOG1, FL ("Recieved Spoofed Mac Addr request with Addr:"
			  MAC_ADDRESS_STR),
	  MAC_ADDR_ARRAY (pMac->lim.spoofMacAddr));
  ...
  if (eSIR_SUCCESS != wdaPostCtrlMsg (pMac, &msg))
    {
      limLog (pMac, LOGE,
	      FL ("wdaPostCtrlMsg failed for SIR_HAL_SPOOF_MAC_ADDR_REQ"));
 ...
 */
static void
lim_register_mgmt_frame_ind_cb (tpAniSirGlobal pMac, tANI_U32 * msg_buf)
{
  ...
  if (sme_req->callback)
    pMac->mgmt_frame_ind_cb = (sir_mgmt_frame_ind_callback) sme_req->callback;
  else
    limLog (pMac, LOGE, FL ("sme_req->callback is null"));
}

void
lim_send_chan_switch_action_frame (tpAniSirGlobal mac_ctx,
 ...
 */
static void
lim_process_sme_set_csa_ie_request (tpAniSirGlobal mac_ctx,
				    uint32_t * msg_buf)
{
  ...
  session_entry->gLimChannelSwitch.switchCount = CHANNEL_SWITCH_BEACON_COUNT;
  session_entry->gLimChannelSwitch.secondarySubBand = csa_ie_req->cb_mode;
  ...
  if (session_entry->vhtCapability &&
      (csa_ie_req->cb_mode >= PHY_QUADRUPLE_CHANNEL_20MHZ_LOW_40MHZ_CENTERED))
    {
 ...
 */
static void
lim_process_sme_channel_change_request (tpAniSirGlobal mac_ctx,
					uint32_t * msg_buf)
{
  ...
  if (session_entry->currentOperChannel == ch_change_req->new_chan)
    {
  ...
  if (session_entry->vhtCapability)
      ...
      if (cb_mode <= PHY_DOUBLE_CHANNEL_HIGH_PRIMARY)
	{
	  ...
	  if (cfgSetInt (mac_ctx, WNI_CFG_VHT_CHANNEL_WIDTH,
	    ...
	    limLog (mac_ctx, LOGE,
		    FL ("could not set channel width to 20/40 in CFG"));
      ...
      else
	{
	  ...
	  if (cfgSetInt (mac_ctx, WNI_CFG_VHT_CHANNEL_CENTER_FREQ_SEGMENT1,
	    ...
	    limLog (mac_ctx, LOGE,
		    FL ("could not set center freq seg 0 in beacon"));
	  if (cfgSetInt
	      (mac_ctx, WNI_CFG_VHT_CHANNEL_WIDTH,
	       WNI_CFG_VHT_CHANNEL_WIDTH_80MHZ) != eSIR_SUCCESS)
	    limLog (mac_ctx, LOGE,
		    FL ("could not set channel width to 80 in CFG"));
	}
    }
...
limProcessSmeReqMessages (tpAniSirGlobal pMac, tpSirMsgQ pMsg)
{
  ...
  switch (pMsg->type)
    ...
    case eWNI_SME_SCAN_REQ:
      pScanReq = (tpSirSmeScanReq) pMsgBuf;
      isPassiveScan =
	(pScanReq->scanType == eSIR_PASSIVE_SCAN) ? TRUE : FALSE;
    case eWNI_SME_REMAIN_ON_CHANNEL_REQ:
    ...
    case eWNI_SME_JOIN_REQ:
      ...
      if ((limIsNOAInsertReqd (pMac) == TRUE)
	  && IS_FEATURE_SUPPORTED_BY_FW (P2P_GO_NOA_DECOUPLE_INIT_SCAN))
	{
	  ...
	  noaDuration =
	    limCalculateNOADuration (pMac, pMsg->type, pMsgBuf,
				     isPassiveScan);
