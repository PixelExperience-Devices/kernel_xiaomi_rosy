...
static void limHandleSmeJoinResult (tpAniSirGlobal, tSirResultCodes, tANI_U16,
				    tpPESession);
static void limHandleSmeReaasocResult (tpAniSirGlobal, tSirResultCodes,
...
static void limHandleDelBssInReAssocContext (tpAniSirGlobal pMac,
					     tpDphHashNode pStaDs,
					     tpPESession psessionEntry);
void limGetSessionInfo (tpAniSirGlobal pMac, tANI_U8 *, tANI_U8 *,
			tANI_U16 *);
static void limProcessBtampAddBssRsp (tpAniSirGlobal pMac, tpSirMsgQ limMsgQ,
				      tpPESession psessionEntry);
 ...
 */
void
limProcessMlmRspMessages (tpAniSirGlobal pMac, tANI_U32 msgType,
			  tANI_U32 * pMsgBuf)
{
  ...
  if ((pMac->lim.gLimRspReqd) || pMac->lim.gLimReportBackgroundScanResults)
    {
      ...
      if ((((tLimMlmScanCnf *) pMsgBuf)->resultCode ==
	   eSIR_SME_SUCCESS) || pMac->lim.gLimSmeScanResultLength)
	{
	  scanRspLen = sizeof (tSirSmeScanRsp) +
	    pMac->lim.gLimSmeScanResultLength - sizeof (tSirBssDescription);
	}
      else
	{
	  scanRspLen = sizeof (tSirSmeScanRsp);
      ...
      else
	{
	  limSendSmeScanRsp (pMac, scanRspLen,
			     eSIR_SME_SUCCESS, pMac->lim.gSmeSessionId,
			     pMac->lim.gTransactionId);
	}
...
void
limProcessMlmOemDataReqCnf (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
...
limProcessMlmStartCnf (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pLimMlmStartCnf->sessionId)) == NULL)
    {
      PELOGE (limLog
	      (pMac, LOGE,
	       FL ("Session does Not exist with given sessionId "));
	)return;
    }
...
limProcessMlmJoinCnf (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pLimMlmJoinCnf->sessionId)) == NULL)
    {
  ...
  if (resultCode == eSIR_SME_SUCCESS)
    {
      ...
      if (limStaSendAddBssPreAssoc (pMac, false, psessionEntry) ==
	  eSIR_SUCCESS)
	return;
      else
    ...
    }
  {
    ...
    limHandleSmeJoinResult (pMac, resultCode,
			    ((tLimMlmJoinCnf *) pMsgBuf)->protStatusCode,
			    psessionEntry);
  }
...
//    tANI_U8         sessionId;
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pMlmAuthCnf->sessionId)) == NULL)
    {
      PELOGE (limLog (pMac, LOGE, FL ("SessionId:%d session does not exist"),
  ...
  if (((psessionEntry->limSmeState != eLIM_SME_WT_AUTH_STATE) &&
	...
	/**
      ...
      PELOGE (limLog (pMac, LOGE,
		      FL
		      ("SessionId:%d received unexpected MLM_AUTH_CNF in state %d"),
  ...
  limDiagEventReport (pMac, WLAN_PE_DIAG_AUTH_COMP_EVENT, psessionEntry,
		      pMlmAuthCnf->resultCode, pMlmAuthCnf->protStatusCode);
#endif
  ...
  if (pMlmAuthCnf->resultCode != eSIR_SME_SUCCESS)
      ...
      if (psessionEntry->limSmeState == eLIM_SME_WT_AUTH_STATE)
	{
	  if (wlan_cfgGetInt (pMac, WNI_CFG_AUTHENTICATION_TYPE,
		...
		/**
                 ...
                 * Log error.
                 */
	      limLog (pMac, LOGP, FL ("could not retrieve AuthType value"));
	    }
	}
      else
	      ...
	      (pMlmAuthCnf->resultCode == eSIR_SME_AUTH_TIMEOUT_RESULT_CODE)))
	{
	  ...
	  limPostMlmMessage (pMac,
			     LIM_MLM_AUTH_REQ, (tANI_U32 *) pMlmAuthReq);
	  return;
	}
      else
	{
	  ...
	  if (psessionEntry->limSmeState == eLIM_SME_WT_AUTH_STATE)
		...
		/**
	      ...
	      limSendSmeAuthRsp (pMac, pMlmAuthCnf->resultCode,
				 pMlmAuthCnf->peerMacAddr,
				 pMlmAuthCnf->authType,
      ...
      if (psessionEntry->limSmeState == eLIM_SME_WT_AUTH_STATE)
	{
	  ...
	  if (cfgGetStr (pMac, WNI_CFG_BSSID,
			 pMlmAssocReq->peerMacAddr, &val) != eSIR_SUCCESS)
	    {
	  ...
	  if (wlan_cfgGetInt (pMac, WNI_CFG_ASSOCIATION_FAILURE_TIMEOUT,
			      ...
			      assocFailureTimeout) != eSIR_SUCCESS)
	    {
		/**
	  ...
	  if (cfgGetCapabilityInfo (pMac, &caps, psessionEntry) !=
	      eSIR_SUCCESS)
	    {
		/**
	  ...
	  if (!
	      (psessionEntry->pLimJoinReq->bssDescription.
	       capabilityInfo & LIM_SPECTRUM_MANAGEMENT_BIT_MASK))
	    {
	  ...
	  PELOG3 (limLog (pMac, LOG3,
			  FL
			  ("Capabilities to be used in AssocReq=0x%X, privacy bit=%x shortSlotTime %x"),
			  caps,
			  ((tpSirMacCapabilityInfo) & pMlmAssocReq->
			   capabilityInfo)->privacy,
			  ((tpSirMacCapabilityInfo) & pMlmAssocReq->
			   capabilityInfo)->shortSlotTime);
	    )
	  ...
	  MTRACE (macTrace
		  (pMac, TRACE_CODE_SME_STATE, psessionEntry->peSessionId,
		   psessionEntry->limSmeState));
	  limLog (pMac, LOG1,
		  "SessionId:%d PostMLMMessage: LIM_MLM_ASSOC_REQ",
		  psessionEntry->peSessionId);
	  limPostMlmMessage (pMac, LIM_MLM_ASSOC_REQ,
			     (tANI_U32 *) pMlmAssocReq);
	}
      else
	{
...
limProcessMlmAssocCnf (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pLimMlmAssocCnf->sessionId)) == NULL)
    {
      PELOGE (limLog (pMac, LOGE, FL ("SessionId:%d Session does not exist"),
  ...
  if (psessionEntry->limSmeState != eLIM_SME_WT_ASSOC_STATE ||
	...
	/**
      ...
      PELOGE (limLog (pMac, LOGE,
		      FL
		      ("SessionId:%d Received unexpected MLM_ASSOC_CNF in state %d"),
		      psessionEntry->peSessionId, psessionEntry->limSmeState);
	)return;
    }
...
limProcessMlmReassocCnf (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pLimMlmReassocCnf->sessionId)) == NULL)
    {
      PELOGE (limLog
	      (pMac, LOGE,
  ...
  if ((psessionEntry->limSmeState != eLIM_SME_WT_REASSOC_STATE) ||
	...
	/**
      ...
      PELOGE (limLog (pMac, LOGE,
		      FL
		      ("Rcv unexpected MLM_REASSOC_CNF in role %d, sme state %d"),
  ...
  PELOGE (limLog
	  (pMac, LOG1, FL ("Rcv MLM_REASSOC_CNF with result code %d"),
	...
	/**
      ...
      limSendSmeJoinReassocRsp (pMac, eWNI_SME_REASSOC_RSP,
				pLimMlmReassocCnf->resultCode,
				pLimMlmReassocCnf->protStatusCode,
	...
	/**
         ...
         * Association failure to Host.
         */
      limHandleSmeReaasocResult (pMac, pLimMlmReassocCnf->resultCode,
				 pLimMlmReassocCnf->protStatusCode,
				 psessionEntry);
    }
...
limProcessMlmReassocInd (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionByBssid (pMac,
			     ((tpLimMlmReassocInd) pMsgBuf)->peerMacAddr,
			     &sessionId)) == NULL)
    {
      PELOGE (limLog
  ...
  if (limReassocIndSerDes (pMac, (tpLimMlmReassocInd) pMsgBuf,
    ...
    {
      limLog (pMac, LOGE,
	      FL (" Received SME message with invalid rem length"));
      vos_mem_free (pSirSmeReassocInd);
      return;
    }

  // Required for indicating the frames to upper layer
  pSirSmeReassocInd->assocReqLength =
    ((tpLimMlmReassocInd) pMsgBuf)->assocReqLength;
  pSirSmeReassocInd->assocReqPtr =
    ((tpLimMlmReassocInd) pMsgBuf)->assocReqPtr;
  ...
  msgQ.bodyval = 0;
  MTRACE (macTrace
	  (pMac, TRACE_CODE_TX_SME_MSG, psessionEntry->peSessionId,
	   msgQ.type));
#ifdef FEATURE_WLAN_DIAG_SUPPORT_LIM	//FEATURE_WLAN_DIAG_SUPPORT
  limDiagEventReport (pMac, WLAN_PE_DIAG_REASSOC_IND_EVENT, psessionEntry, 0,
		      0);
#endif //FEATURE_WLAN_DIAG_SUPPORT
  pStaDs =
    dphGetHashEntry (pMac, ((tpLimMlmReassocInd) pMsgBuf)->aid,
		     &psessionEntry->dph.dphHashTable);
  if (!pStaDs)
    {
      limLog (pMac, LOGP,
	      FL ("MLM ReAssocInd: Station context no longer valid (aid %d)"),
		       ...
		       tpPESession psessionEntry)
{
  ...
  // Fill in bssId
  vos_mem_copy (pSirSmeAssocInd->bssId, psessionEntry->bssId,
		sizeof (tSirMacAddr));
  // Fill in staId
  ...
  if (pAssocInd->spectrumMgtIndicator == eSIR_TRUE)
    {
      ...
      pSirSmeAssocInd->supportedChannels.numChnl =
	pAssocInd->supportedChannels.numChnl;
      vos_mem_copy ((tANI_U8 *) & pSirSmeAssocInd->supportedChannels.
		    channelList,
...
limProcessMlmAssocInd (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac,
				 ((tpLimMlmAssocInd) pMsgBuf)->sessionId)) ==
      NULL)
    {
  ...
  limFillAssocIndParams (pMac, (tpLimMlmAssocInd) pMsgBuf, pSirSmeAssocInd,
			 psessionEntry);
  ...
  if (!pStaDs)
    {				// good time to panic...
      limLog (pMac, LOGE,
	      FL ("MLM AssocInd: Station context no longer valid (aid %d)"),
	      ((tpLimMlmAssocInd) pMsgBuf)->aid);
  ...
  MTRACE (macTrace (pMac, TRACE_CODE_TX_SME_MSG, psessionEntry->peSessionId,
		    msg.type));
#ifdef FEATURE_WLAN_DIAG_SUPPORT_LIM	//FEATURE_WLAN_DIAG_SUPPORT
  limDiagEventReport (pMac, WLAN_PE_DIAG_ASSOC_IND_EVENT, psessionEntry, 0,
		      0);
#endif //FEATURE_WLAN_DIAG_SUPPORT
  limLog (pMac, LOG1,
...
//tANI_U32 *pMsgBuf;
  {
    ...
    if (NULL == pSmeAssoccnf)
      PELOGE (limLog
	      (pMac, LOGE, FL ("AllocateMemory failed for pSmeAssoccnf "));
...
limProcessMlmDisassocInd (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pMlmDisassocInd->sessionId)) == NULL)
    {
...
limProcessMlmDisassocCnf (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pMlmDisassocCnf->sessionId)) == NULL)
    {
      PELOGE (limLog
	      (pMac, LOGE,
	       FL ("session Does not exist for given session Id"));
  ...
  resultCode = (tSirResultCodes)
    ...
    eSIR_SME_LOST_LINK_WITH_PEER_RESULT_CODE : pMlmDisassocCnf->resultCode;
  if ((psessionEntry->limSystemRole == eLIM_STA_ROLE)
	  ...
	  (psessionEntry->limSmeState != eLIM_SME_WT_DEAUTH_STATE))
	{
	  ...
	  PELOGE (limLog (pMac, LOGE,
			  FL
			  ("received unexpected MLM_DISASSOC_CNF in state %d"),
			  psessionEntry->limSmeState);
	    )return;
	}
      if (pMac->lim.gLimRspReqd)
	pMac->lim.gLimRspReqd = false;
      if (pMlmDisassocCnf->disassocTrigger == eLIM_PROMISCUOUS_MODE_DISASSOC)
	{
      ...
      else
	{
	  ...
	  limSendSmeDisassocNtf (pMac, pMlmDisassocCnf->peerMacAddr,
				 resultCode, pMlmDisassocCnf->disassocTrigger,
				 pMlmDisassocCnf->aid,
				 psessionEntry->smeSessionId,
...
limProcessMlmDeauthInd (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionByBssid (pMac, pMlmDeauthInd->peerMacAddr,
			     &sessionId)) == NULL)
    {
      PELOGE (limLog
	      (pMac, LOGE,
...
limProcessMlmDeauthCnf (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pMlmDeauthCnf->sessionId)) == NULL)
    {
      PELOGE (limLog
	      (pMac, LOGE,
	       FL ("session does not exist for given session Id "));
  ...
  resultCode = (tSirResultCodes)
    ...
    eSIR_SME_LOST_LINK_WITH_PEER_RESULT_CODE : pMlmDeauthCnf->resultCode;
  ...
  if ((psessionEntry->limSystemRole == eLIM_STA_ROLE)
	  ...
	  (psessionEntry->limSmeState != eLIM_SME_WT_DEAUTH_STATE))
	{
	  ...
	  PELOGE (limLog (pMac, LOGE,
			  FL
			  ("received unexpected MLM_DEAUTH_CNF in state %d"),
			  psessionEntry->limSmeState);
	    )return;
	}
	...
	{
	  psessionEntry->limSmeState = eLIM_SME_IDLE_STATE;
	  limLog (pMac, LOG1, FL ("*** Deauthenticated with BSS ***"));
	}
      else
	psessionEntry->limSmeState = psessionEntry->limPrevSmeState;
      MTRACE (macTrace
...
limProcessMlmPurgeStaInd (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pMlmPurgeStaInd->sessionId)) == NULL)
    {
      PELOGE (limLog
	      (pMac, LOGE, FL ("session does not exist for given bssId"));
	)return;
  ...
  switch (psessionEntry->limSystemRole)
	  ...
	  (psessionEntry->limSmeState != eLIM_SME_WT_DEAUTH_STATE))
	{
	  ...
	  PELOGE (limLog (pMac, LOGE,
			  FL
			  ("received unexpected MLM_PURGE_STA_IND in state %d"),
			  psessionEntry->limSmeState);
	    )break;
	}
...
limProcessMlmSetKeysCnf (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pMlmSetKeysCnf->sessionId)) == NULL)
    {
      PELOGE (limLog
	      (pMac, LOGE,
  ...
  if (eSIR_SME_SUCCESS == pMlmSetKeysCnf->resultCode)
	...
	{
	  pStaDs =
	    dphLookupHashEntry (pMac, pMlmSetKeysCnf->peerMacAddr, &aid,
				&psessionEntry->dph.dphHashTable);
	  if (pStaDs != NULL)
...
limProcessMlmRemoveKeyCnf (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pMlmRemoveKeyCnf->sessionId)) == NULL)
    {
      PELOGE (limLog
	      (pMac, LOGE,
	       FL ("session Does not exist for given session Id"));
 ...
 */
static void
limHandleSmeJoinResult (tpAniSirGlobal pMac, tSirResultCodes resultCode,
...
error:
  ...
  limSendSmeJoinReassocRsp (pMac, eWNI_SME_JOIN_RSP, resultCode,
 ...
 */
static void
limHandleSmeReaasocResult (tpAniSirGlobal pMac, tSirResultCodes resultCode,
			   tANI_U16 protStatusCode, tpPESession psessionEntry)
{
  tpDphHashNode pStaDs = NULL;
  tANI_U8 smesessionId;
...
error:
  ...
  limSendSmeJoinReassocRsp (pMac, eWNI_SME_REASSOC_RSP, resultCode,
			    protStatusCode, psessionEntry, smesessionId,
			    smetransactionId);
 ...
 */
void
limProcessMlmAddStaRsp (tpAniSirGlobal pMac, tpSirMsgQ limMsgQ,
			tpPESession psessionEntry)
{
...
void
limProcessStaMlmAddStaRsp (tpAniSirGlobal pMac, tpSirMsgQ limMsgQ,
			   tpPESession psessionEntry)
{
  ...
  if (eHAL_STATUS_SUCCESS == pAddStaParams->status)
...
#endif
	{
      ...
      pMac->lim.limTimers.gLimKeepaliveTimer.sessionId =
	psessionEntry->peSessionId;
      if (tx_timer_activate (&pMac->lim.limTimers.gLimKeepaliveTimer) !=
	  TX_SUCCESS)
...
void
limProcessMlmDelBssRsp (tpAniSirGlobal pMac, tpSirMsgQ limMsgQ,
			tpPESession psessionEntry)
{
  //we need to process the deferred message since the initiating req. there might be nested request.
  //in the case of nested request the new request initiated from the response will take care of resetting
  ...
  if (psessionEntry->limRmfEnabled)
    {
      if (eSIR_SUCCESS !=
	  limSendExcludeUnencryptInd (pMac, TRUE, psessionEntry))
	{
	  limLog (pMac, LOGE,
...
void
limProcessStaMlmDelBssRsp (tpAniSirGlobal pMac, tpSirMsgQ limMsgQ,
			   tpPESession psessionEntry)
{
  ...
  if (eHAL_STATUS_SUCCESS == pDelBssParams->status)
	...
	{
	  PELOGE (limLog
		  (pMac, LOGE, FL ("Failure in setting link state to IDLE"));
...
void
limProcessBtAmpApMlmDelBssRsp (tpAniSirGlobal pMac, tpSirMsgQ limMsgQ,
			       tpPESession psessionEntry)
{
  ...
  limSendSmeRsp (pMac, eWNI_SME_STOP_BSS_RSP, rc, psessionEntry->smeSessionId,
		 psessionEntry->transactionId);
  peDeleteSession (pMac, psessionEntry);
...
void
limProcessMlmDelStaRsp (tpAniSirGlobal pMac, tpSirMsgQ limMsgQ)
{
  //we need to process the deferred message since the initiating req. there might be nested request.
  //in the case of nested request the new request initiated from the response will take care of resetting
  //the deffered flag.
  ...
  if (NULL == pDeleteStaParams ||
      NULL == (psessionEntry =
	       peFindSessionBySessionId (pMac, pDeleteStaParams->sessionId)))
    {
      limLog (pMac, LOGP,
	      FL
	      ("Session Does not exist or invalid body pointer in message"));
...
void
limProcessBtAmpApMlmDelStaRsp (tpAniSirGlobal pMac, tpSirMsgQ limMsgQ,
			       tpPESession psessionEntry)
{
  ...
  if (limMsgQ->bodyptr == NULL)
    {
      limLog (pMac, LOGE, FL ("limMsgQ->bodyptry NULL"));
      return;
    }
  pStaDs =
    dphGetHashEntry (pMac, pDelStaParams->assocId,
		     &psessionEntry->dph.dphHashTable);
  if (pStaDs == NULL)
    {
      limLog (pMac, LOGE,
  ...
  if (eHAL_STATUS_SUCCESS == pDelStaParams->status)
    {
      limLog (pMac, LOGW,
	      FL ("AP received the DEL_STA_RSP for assocID: %d."),
	   ...
	   pStaDs->mlmStaContext.mlmState))
	{
	  limLog (pMac, LOGE,
		  FL
		  ("Received unexpected WDA_DEL_STA_RSP in state %s for staId %d assocId %d "),
		  limMlmStateStr (pStaDs->mlmStaContext.mlmState),
	  ...
	  pStaDs->mlmStaContext.mlmState)
	{
	  ...
	  if (limAddSta (pMac, pStaDs, false, psessionEntry) != eSIR_SUCCESS)
		...
		if (pStaDs->qos.addtsPresent)
		{
		  ...
		  if (eSIR_SUCCESS ==
		      limTspecFindByAssocId (pMac, pStaDs->assocId,
					     &pStaDs->qos.addts.tspec,
					     &pMac->lim.tspecInfo[0],
					     &pTspecInfo))
		    {
		      limAdmitControlDeleteTS (pMac, pStaDs->assocId,
					       &pStaDs->qos.addts.tspec.
					       tsinfo, NULL,
					       &pTspecInfo->idx);
		    }
    ...
    {
      limLog (pMac, LOGW, FL ("DEL STA failed!"));
      statusCode = eSIR_SME_REFUSED;
    }
end:
  vos_mem_free (pDelStaParams);
...
void
limProcessStaMlmDelStaRsp (tpAniSirGlobal pMac, tpSirMsgQ limMsgQ,
			   tpPESession psessionEntry)
{
  tSirResultCodes statusCode = eSIR_SME_SUCCESS;
  tpDeleteStaParams pDelStaParams = (tpDeleteStaParams) limMsgQ->bodyptr;
...
void
limProcessBtAmpApMlmAddStaRsp (tpAniSirGlobal pMac, tpSirMsgQ limMsgQ,
			       tpPESession psessionEntry)
{
  tpAddStaParams pAddStaParams = (tpAddStaParams) limMsgQ->bodyptr;
  tpDphHashNode pStaDs = NULL;
  ...
  pStaDs =
    dphGetHashEntry (pMac, pAddStaParams->assocId,
		     &psessionEntry->dph.dphHashTable);
  if (pStaDs == NULL)
    {
      //TODO: any response to be sent out here ?
...
//    tANI_U8     sessionId;
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pAddBssParams->sessionId)) == NULL)
    {
      PELOGE (limLog
  ...
  if (eHAL_STATUS_SUCCESS == pAddBssParams->status)
    {
      limLog (pMac, LOG1,
	      FL ("WDA_ADD_BSS_RSP returned with eHAL_STATUS_SUCCESS"));
      if (limSetLinkState
	...
	limCreateTimers (pMac);
      ...
      // Start OLBC timer
      if (tx_timer_activate (&pMac->lim.limTimers.gLimUpdateOlbcCacheTimer) !=
	  TX_SUCCESS)
	{
      ...
      if (eSIR_SUCCESS !=
	  wlan_cfgGetInt (pMac, WNI_CFG_TRIG_STA_BK_SCAN, &val))
				   ...
				   tpPESession psessionEntry)
{
  ...
  if (eHAL_STATUS_SUCCESS == pAddBssParams->status)
    {
      if ((pStaDs =
	   dphAddHashEntry (pMac, pAddBssParams->staContext.staMac,
			    DPH_STA_HASH_INDEX_PEER,
			  ...
			  (tANI_U32 *) & cfgAuthType) != eSIR_SUCCESS)
	{
		/**
                 ...
                 * Log error.
                 */
	  limLog (pMac, LOGP, FL ("could not retrieve AuthType"));
	}
      if (cfgAuthType == eSIR_AUTO_SWITCH)
      ...
      if (NULL == pMlmAuthReq)
	{
      ...
      if (wlan_cfgGetStr (pMac, WNI_CFG_BSSID,
			  pMlmAuthReq->peerMacAddr, &val) != eSIR_SUCCESS)
	{
	  /// Could not get BSSID from CFG. Log error.
	  limLog (pMac, LOGP, FL ("could not retrieve BSSID"));
	}
	  ...
	  != eSIR_SUCCESS)
	{
      ...
      MTRACE (macTrace (pMac, TRACE_CODE_SME_STATE,
			...
			psessionEntry->limSmeState));
      limLog (pMac, LOG1,
	      "SessionId:%d limPostMlmMessage LIM_MLM_AUTH_REQ"
...
joinFailure:
  {
    ...
    /// Send Join response to Host
    limHandleSmeJoinResult (pMac, eSIR_SME_REFUSED,
			    eSIR_MAC_UNSPEC_FAILURE_STATUS, psessionEntry);
 ...
 */
static inline void
limProcessStaMlmAddBssRspFT (tpAniSirGlobal pMac, tpSirMsgQ limMsgQ,
			     tpPESession psessionEntry)
{
  ...
  if ((pStaDs =
       dphAddHashEntry (pMac, pAddBssParams->bssId, DPH_STA_HASH_INDEX_PEER,
			&psessionEntry->dph.dphHashTable)) == NULL)
    {
      // Could not add hash table entry
      PELOGE (limLog
  ...
  if (pMlmReassocReq == NULL)
    {
      limLog (pMac, LOGE, FL ("Invalid pMlmReassocReq"));
      goto end;
    }
  ...
  if (tx_timer_deactivate (&pMac->lim.limTimers.gLimReassocFailureTimer) !=
      TX_SUCCESS)
    {
      limLog (pMac, LOGP, FL ("unable to deactivate Reassoc failure timer"));
    }
  ...
  if (tx_timer_activate (&pMac->lim.limTimers.gLimReassocFailureTimer) !=
      TX_SUCCESS)
    {
      ...
      // Log error
      limLog (pMac, LOGP, FL ("could not start Reassociation failure timer"));
  ...
  if (NULL == pMac->lim.pSessionEntry->pLimMlmReassocRetryReq)
	...
	vos_mem_malloc (sizeof (tLimMlmReassocReq));
      if (NULL == pMac->lim.pSessionEntry->pLimMlmReassocRetryReq)
	goto end;
      vos_mem_set (pMac->lim.pSessionEntry->pLimMlmReassocRetryReq,
...
#endif
  limSendReassocReqWithFTIEsMgmtFrame (pMac, psessionEntry->pLimMlmReassocReq,
				       psessionEntry);
  if (pMac->roam.configParam.roamDelayStatsEnabled)
    {
  ...
  pAddStaParams->shortPreambleSupported =
    (tANI_U8) psessionEntry->beaconParams.fShortPreamble;
#ifdef WLAN_FEATURE_11AC
  limPopulatePeerRateSet (pMac, &pAddStaParams->supportedRates, NULL, false,
			  psessionEntry, NULL);
#else
  limPopulatePeerRateSet (pMac, &pAddStaParams->supportedRates, NULL, false,
			  psessionEntry);
  ...
  if (psessionEntry->htCapability)
	...
	psessionEntry->vhtTxChannelWidthSet;
	  ...
	  && (!psessionEntry->pLimJoinReq->bssDescription.aniIndicator))
	{
	  limLog (pMac, LOGE,
	...
	limGetHTCapability (pMac, eHT_MIMO_POWER_SAVE, psessionEntry);
  ...
  if (wlan_cfgGetInt (pMac, WNI_CFG_LISTEN_INTERVAL, &listenInterval) !=
      eSIR_SUCCESS)
			   ...
			   tpPESession psessionEntry)
{
  ...
  if ((eLIM_MLM_WT_ADD_BSS_RSP_REASSOC_STATE == psessionEntry->limMlmState) ||
       ...
       psessionEntry->limMlmState))
    {
      ...
      if (sirCompareMacAddr
	  (psessionEntry->bssId, psessionEntry->limReAssocbssId))
	{
...
#if defined WLAN_FEATURE_VOWIFI
	  rrmCacheMgmtTxPower (pMac, pAddBssParams->txMgmtPower,
			       psessionEntry);
#endif

	  if (subType == LIM_REASSOC)
	    limDeactivateAndChangeTimer (pMac, eLIM_KEEPALIVE_TIMER);
	  if (limAddStaSelf (pMac, staIdx, updateSta, psessionEntry) !=
	      eSIR_SUCCESS)
	    {
	      // Add STA context at HW
	      PELOGE (limLog
		      (pMac, LOGE,
		       FL ("SessionId:%d could not Add Self"
			   "Entry for the station"),
		       psessionEntry->peSessionId);
		)mlmAssocCnf.resultCode = (tSirResultCodes) eSIR_SME_REFUSED;
	    }
 ...
 */
void
limProcessMlmAddBssRsp (tpAniSirGlobal pMac, tpSirMsgQ limMsgQ)
{
  tLimMlmStartCnf mlmStartCnf;
  tpPESession psessionEntry;
  tpAddBssParams pAddBssParams = (tpAddBssParams) (limMsgQ->bodyptr);
  ...
  if (eSIR_IBSS_MODE == psessionEntry->bssType)
      ...
      if (eLIM_SME_WT_START_BSS_STATE == psessionEntry->limSmeState)
	{
	  if (eLIM_MLM_WT_ADD_BSS_RSP_STATE != psessionEntry->limMlmState)
	      ...
	      if (0 != limMsgQ->bodyptr)
		{
	      ...
	      limPostSmeMessage (pMac, LIM_MLM_START_CNF,
				 (tANI_U32 *) & mlmStartCnf);
	    }
	  else if ((psessionEntry->bssType == eSIR_BTAMP_AP_MODE)
		   || (psessionEntry->bssType == eSIR_BTAMP_STA_MODE))
	    {
      ...
      else
	{
	  ...
	  if (pMac->ft.ftSmeContext.is_preauth_lfr_mbb)
	    {
	      lim_process_sta_mlm_add_bss_rsp_mbb (pMac, limMsgQ,
						   psessionEntry);
	      return;
	    }
#endif
  ...
  if (psessionEntry->limRmfEnabled)
    {
      if (eSIR_SUCCESS !=
	  limSendExcludeUnencryptInd (pMac, FALSE, psessionEntry))
	{
	  limLog (pMac, LOGE,
		  FL ("Could not send down Exclude Unencrypted Indication!"));
	}
 ...
 */
void
limProcessMlmSetStaKeyRsp (tpAniSirGlobal pMac, tpSirMsgQ limMsgQ)
{
  tANI_U8 respReqd = 1;
  ...
  if (eLIM_MLM_WT_SET_STA_KEY_STATE != psessionEntry->limMlmState)
    ...
    mlmSetKeysCnf.resultCode =
      (tANI_U16) (((tpSetStaKeyParams) limMsgQ->bodyptr)->status);
  ...
  if (respReqd)
	...
	(tpLimMlmSetKeysReq) pMac->lim.gpLimMlmSetKeysReq;
      ...
      if (NULL != lpLimMlmSetKeysReq)
	{
      ...
      limPostSmeMessage (pMac, LIM_MLM_SETKEYS_CNF,
			 (tANI_U32 *) & mlmSetKeysCnf);
    }
}

void
limProcessMlmSetBssKeyRsp (tpAniSirGlobal pMac, tpSirMsgQ limMsgQ)
{
  tLimMlmSetKeysCnf mlmSetKeysCnf;
  tANI_U16 resultCode;
  tANI_U8 sessionId = 0;
  ...
  if (NULL != lpLimMlmSetKeysReq)
    {
      vos_mem_copy ((tANI_U8 *) & mlmSetKeysCnf.peerMacAddr,
		    ...
		    sizeof (tSirMacAddr));
      /* Free the buffer cached for the global pMac->lim.gpLimMlmSetKeysReq */
      vos_mem_zero (pMac->lim.gpLimMlmSetKeysReq, sizeof (tLimMlmSetKeysReq));
      vos_mem_free (pMac->lim.gpLimMlmSetKeysReq);
      pMac->lim.gpLimMlmSetKeysReq = NULL;
    }
 ...
 */
void
limProcessMlmRemoveKeyRsp (tpAniSirGlobal pMac, tpSirMsgQ limMsgQ)
{
  tANI_U8 respReqd = 1;
  ...
  if (eLIM_MLM_WT_REMOVE_BSS_KEY_STATE == psessionEntry->limMlmState)
    resultCode =
      (tANI_U16) (((tpRemoveBssKeyParams) limMsgQ->bodyptr)->status);
  else
    resultCode =
      (tANI_U16) (((tpRemoveStaKeyParams) limMsgQ->bodyptr)->status);
  ...
  if (respReqd)
    {
      tpLimMlmRemoveKeyReq lpLimMlmRemoveKeyReq =
	(tpLimMlmRemoveKeyReq) pMac->lim.gpLimMlmRemoveKeyReq;
      ...
      if (NULL != lpLimMlmRemoveKeyReq)
	{
      ...
      limPostSmeMessage (pMac, LIM_MLM_REMOVEKEY_CNF,
...
\ ----------------------------------------------------------------------- */
void
limProcessInitScanRsp (tpAniSirGlobal pMac, void *body)
{
  ...
  if (pMac->lim.abortScan &&
	...
	{
	  PELOGW (limLog
		  (pMac, LOGW, FL ("InitScnRsp failed status=%d"), status);
  ...
  switch (pMac->lim.gLimHalScanState)
	...
	{
	  PELOGW (limLog
		  (pMac, LOGW, FL ("InitScanRsp with failed status= %d"),
		   status);
	    )pMac->lim.gLimHalScanState = eLIM_HAL_IDLE_SCAN_STATE;
	  pMac->lim.gLimNumOfConsecutiveBkgndScanFailure += 1;
	  /*
...
//WLAN_SUSPEND_LINK Related
      ...
      if (pMac->lim.gpLimSuspendCallback)
	{
	  ...
	  pMac->lim.gpLimSuspendCallback (pMac, status,
	...
	{
	  limLog (pMac, LOGP,
		  "No suspend link callback set but station is in suspend state");
	  return;
	}
      break;
//end WLAN_SUSPEND_LINK Related
    default:
      limLog (pMac, LOGW,
	      FL
	      ("limProcessInitScanRsp: Rcvd InitScanRsp not in WAIT State, state %d"),
 ...
 */
static void
limProcessSwitchChannelReAssocReq (tpAniSirGlobal pMac,
				   tpPESession psessionEntry,
				   eHalStatus status)
{
 ...
 */
static void
limProcessSwitchChannelJoinReq (tpAniSirGlobal pMac,
				tpPESession psessionEntry, eHalStatus status)
{
  tANI_U32 val;
  tSirMacSSid ssId;
  ...
  if (psessionEntry->bssType == eSIR_BTAMP_AP_MODE)
    {
      if (limSetLinkState
	  (pMac, eSIR_LINK_BTAMP_PREASSOC_STATE, psessionEntry->bssId,
	   psessionEntry->selfMacAddr, NULL, NULL) != eSIR_SUCCESS)
	{
  ...
  if (wlan_cfgGetInt (pMac, WNI_CFG_TRIG_STA_BK_SCAN, &val) != eSIR_SUCCESS)
    limLog (pMac, LOGP,
	    FL ("failed to get WNI_CFG_TRIG_STA_BK_SCAN cfg value!"));
  pMac->lim.gLimTriggerBackgroundScanDuringQuietBss = (val) ? 1 : 0;
  // Apply previously set configuration at HW
  limApplyConfiguration (pMac, psessionEntry);
	  ...
	  if (vos_mem_compare
	       ...
	       pMac->lim.gLimHeartBeatApMac[apCount], sizeof (tSirMacAddr)))
	    {
	      limLog (pMac, LOGE, FL ("Index %d Sessionid: %d Send deauth on "
				      "channel %d to BSSID: "
				      MAC_ADDRESS_STR), apCount,
		      psessionEntry->peSessionId,
		      psessionEntry->currentOperChannel,
		      MAC_ADDR_ARRAY (psessionEntry->pLimMlmJoinReq->
				      bssDescription.bssId));

	      limSendDeauthMgmtFrame (pMac, eSIR_MAC_UNSPEC_FAILURE_REASON,
				      psessionEntry->pLimMlmJoinReq->
				      bssDescription.bssId, psessionEntry,
				      FALSE);
  ...
  limSendProbeReqMgmtFrame (pMac, &ssId,
			    ...
			    bssId,
			    psessionEntry->currentOperChannel /*chanNum */ ,
			    psessionEntry->selfMacAddr,
			    psessionEntry->dot11mode,
			    psessionEntry->pLimJoinReq->addIEScan.length,
			    psessionEntry->pLimJoinReq->addIEScan.addIEdata);
 ...
 */
void
limProcessSwitchChannelRsp (tpAniSirGlobal pMac, void *body)
{
  ...
  limLog (pMac, LOGE, FL ("channelChangeReasonCode %d status %d"),
	  channelChangeReasonCode, pChnlParams->status);
  switch (channelChangeReasonCode)
      ...
      if (pMac->lim.gpchangeChannelCallback)
	{
	  ...
	  pMac->lim.gpchangeChannelCallback (pMac, status,
					     pMac->lim.gpchangeChannelData,
					     psessionEntry);
	}
      break;
    case LIM_SWITCH_CHANNEL_SAP_ECSA:
      lim_send_sme_ap_channel_switch_resp (pMac, psessionEntry, pChnlParams);
      break;
    default:
      break;
    }
...
void
limProcessStartScanRsp (tpAniSirGlobal pMac, void *body)
{
  ...
  switch (pMac->lim.gLimHalScanState)
	...
	{
	  PELOGW (limLog
		  (pMac, LOGW, FL ("StartScanRsp with failed status= %d"),
		   status);
	    )
	    //
    ...
    default:
      limLog (pMac, LOGW,
	      FL ("Rcvd StartScanRsp not in WAIT State, state %d"),
	      pMac->lim.gLimHalScanState);
      break;
    }
  return;
...
void
limProcessEndScanRsp (tpAniSirGlobal pMac, void *body)
{
  ...
  switch (pMac->lim.gLimHalScanState)
	...
	{
	  PELOGW (limLog
		  (pMac, LOGW, FL ("EndScanRsp with failed status= %d"),
		   status);
	    )pMac->lim.gLimHalScanState = eLIM_HAL_IDLE_SCAN_STATE;
	  limCompleteMlmScan (pMac, eSIR_SME_HAL_SCAN_INIT_FAILED);
	}
  ...
  for (i = 0; i < pMac->lim.maxBssId; i++)
    {
	  ...
	  pMac->lim.gpSession[i].gLimSpecMgmt.dot11hChanSwState ==
...
void
limProcessFinishScanRsp (tpAniSirGlobal pMac, void *body)
{
  ...
  limLog (pMac, LOG1, FL ("Rcvd FinishScanRsp in state %d channel %d "),
	  pMac->lim.gLimHalScanState, pFinishScanParam->currentOperChannel);
...
//WLAN_SUSPEND_LINK Related
	...
	{
	  pMac->lim.gLimHalScanState = eLIM_HAL_IDLE_SCAN_STATE;
	  pMac->lim.gpLimResumeCallback (pMac, status,
	...
	{
	  limLog (pMac, LOGP,
		  "No Resume link callback set but station is in suspend state");
	  vos_mem_free (body);
	  body = NULL;
	  return;
	}
...
//end WLAN_SUSPEND_LINK Related
    ...
    default:
      limLog (pMac, LOGE,
	      FL ("Rcvd FinishScanRsp not in WAIT State, state %d"),
	      pMac->lim.gLimHalScanState);
      break;
    }
  vos_mem_free (body);
 ...
 */
void
limProcessMlmHalAddBARsp (tpAniSirGlobal pMac, tpSirMsgQ limMsgQ)
{
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pAddBAParams->sessionId)) == NULL)
    {
      PELOGE (limLog
	      (pMac, LOGE,
 ...
 */
void
limProcessMlmAddBACnf (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  tpLimMlmAddBACnf pMlmAddBACnf;
  tpDphHashNode pSta;
  tANI_U16 aid;
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pMlmAddBACnf->sessionId)) == NULL)
    {
      PELOGE (limLog
	      (pMac, LOGE, FL ("session does not exist for given BSSId"));
  ...
  pSta =
    dphLookupHashEntry (pMac, pMlmAddBACnf->peerMacAddr, &aid,
			&psessionEntry->dph.dphHashTable);
  if (NULL == pSta)
    {
      PELOGE (limLog (pMac, LOGE,
		      FL
		      ("STA context not found - ignoring ADDBA CNF from HAL"));
	)vos_mem_free (pMsgBuf);
      return;
    }
  ...
  if (eLIM_BA_STATE_WT_ADD_RSP != curBaState)
    {
      PELOGE (limLog (pMac, LOGE,
		      FL
		      ("Received unexpected ADDBA CNF when STA BA state is %d"),
		      curBaState);
	)vos_mem_free (pMsgBuf);
      return;
    }
  ...
  if (eSIR_MAC_SUCCESS_STATUS == pMlmAddBACnf->addBAResultCode)
      ...
      if (eBA_RECIPIENT == pMlmAddBACnf->baDirection)
	{
	  ...
	  pSta->tcCfg[pMlmAddBACnf->baTID].fRxBApolicy =
	    pMlmAddBACnf->baPolicy;
	  pSta->tcCfg[pMlmAddBACnf->baTID].rxBufSize =
	    pMlmAddBACnf->baBufferSize;
	  pSta->tcCfg[pMlmAddBACnf->baTID].tuRxBAWaitTimeout =
      ...
      else
	{
	  ...
	  pSta->tcCfg[pMlmAddBACnf->baTID].fTxBApolicy =
	    pMlmAddBACnf->baPolicy;
	  pSta->tcCfg[pMlmAddBACnf->baTID].txBufSize =
	    pMlmAddBACnf->baBufferSize;
	  pSta->tcCfg[pMlmAddBACnf->baTID].tuTxBAWaitTimeout =
	    pMlmAddBACnf->baTimeout;
	}
    }
 ...
 */
void
limProcessMlmDelBACnf (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
...
//    tANI_U8             sessionId;
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pMlmDelBACnf->sessionId)) == NULL)
    {
  ...
  // First, extract the DPH entry
  pSta =
    dphLookupHashEntry (pMac, pMlmDelBACnf->peerMacAddr, &aid,
			&psessionEntry->dph.dphHashTable);
  if (NULL == pSta)
    {
      limLog (pMac, LOGE,
 ...
 */
void
limProcessMlmHalBADeleteInd (tpAniSirGlobal pMac, tpSirMsgQ limMsgQ)
{
  ...
  if ((psessionEntry =
       peFindSessionByBssid (pMac, pBADeleteParams->bssId,
			     &sessionId)) == NULL)
    {
      PELOGE (limLog
	      (pMac, LOGE, FL ("session does not exist for given BSSId"));
  ...
  // First, extract the DPH entry
  pSta =
    dphLookupHashEntry (pMac, pBADeleteParams->peerMacAddr, &aid,
			&psessionEntry->dph.dphHashTable);
  if (NULL == pSta)
    {
      limLog (pMac, LOGE,
  ...
  if (eLIM_BA_STATE_IDLE != curBaState)
    {
      limLog (pMac, LOGE,
	      FL
	      ("Received unexpected BA Delete IND when STA BA state is %d"),
	      curBaState);
      goto returnAfterCleanup;
    }
  ...
  if (eSIR_SUCCESS !=
  ...
  else
    {
      limLog (pMac, LOGE,
	      FL
	      ("BA Delete - Reason 0x%08x. Attempting to delete BA session for TID %d with peer STA "),
...
limProcessSetMimoRsp (tpAniSirGlobal pMac, tpSirMsgQ limMsg)
{
  ...
  do
      ...
      if (NULL == pMIMO_PSParams)
	{
	  PELOGE (limLog
		  (pMac, LOGE,
		   "Received the WDA_SET_MIMOPS_RSP with NULL as the PS param");
	    )return;
	}
	...
	{
	  limLog (pMac, LOGP,
		  FL ("Update HAL / SW Mac for MIMO State has Failed"));
	  break;
	}
	...
	break;
      ...
      MTRACE (macTrace
	      (pMac, TRACE_CODE_MLM_STATE, 0, pMac->lim.gLimMlmState));
	...
	/** In the Case of Exiting out of the Powersave (changing from Dynamic/Static mode to SM Enabled)
          ...
          */
      if (!isEnteringMimoPS
	  (pMac->lim.gHTMIMOPSState, pMIMO_PSParams->htMIMOPSState))
	{
	  tSirMacAddr macAddr;

	    /** Obtain the AP's Mac Address */
	  vos_mem_copy ((tANI_U8 *) macAddr, pMac->lim.gLimBssid,
			sizeof (tSirMacAddr));

	    /** Send Action Frame with the corresponding mode */
	  retStatus =
	    limSendSMPowerStateFrame (pMac, macAddr,
				      pMIMO_PSParams->htMIMOPSState);
	  if (retStatus != eSIR_SUCCESS)
	    {
	      PELOGE (limLog
		      (pMac, LOGE, FL ("Sending Action Frame has failed"));
		)break;
	    }
	}
      PELOG1 (limLog
	      (pMac, LOG1,
	       FL ("The Setting up of LimGlobals is successful for MIMOPS"));
    )}
  while (0);
  ...
  switch (psessionEntry->limSmeState)
	...
	limDeleteDphHashEntry (pMac, psessionEntry->bssId,
       ...
       /**
             ...
             */
	if ((pStaDs =
	     dphAddHashEntry (pMac, psessionEntry->limReAssocbssId,
			      DPH_STA_HASH_INDEX_PEER,
			      &psessionEntry->dph.dphHashTable)) == NULL)
	  {
	    // Could not add hash table entry
	...
	limExtractApCapabilities (pMac,
				  ...
				  GET_IE_LEN_IN_BSS (psessionEntry->
						     pLimReAssocReq->
	...
	if (eSIR_SUCCESS != limStaSendAddBss (pMac, assocRsp, pBeaconStruct,
					      &psessionEntry->pLimReAssocReq->
					      bssDescription, false,
					      psessionEntry))
	  {
	    limLog (pMac, LOGE,
		    FL ("Posting ADDBSS in the ReAssocContext has Failed "));
...
static void
limProcessBtampAddBssRsp (tpAniSirGlobal pMac, tpSirMsgQ limMsgQ,
			  tpPESession psessionEntry)
{
  tLimMlmStartCnf mlmStartCnf;
  tANI_U32 val;
  ...
  if (eHAL_STATUS_SUCCESS == pAddBssParams->status)
	...
	{
	  if (limSetLinkState
	      (pMac, eSIR_LINK_BTAMP_AP_STATE, psessionEntry->bssId,
	       psessionEntry->selfMacAddr, NULL, NULL) != eSIR_SUCCESS)
	    goto end;
	}
				 ...
				 tpPESession psessionEntry)
{
  ...
  switch (psessionEntry->limSmeState)
	...
	if (NULL == pBeaconStruct)
	  {
	    limLog (pMac, LOGE,
		    FL
		    ("Unable to allocate memory in limHandleAddBssInReAssocContext"));
	    mlmReassocCnf.resultCode = eSIR_SME_RESOURCES_UNAVAILABLE;
	...
	if (pStaDs == NULL)
	  {
	    PELOGE (limLog
		    (pMac, LOGE, FL ("Fail to get STA PEER entry from hash"));
	      )mlmReassocCnf.resultCode = eSIR_SME_RESOURCES_UNAVAILABLE;
	    mlmReassocCnf.protStatusCode = eSIR_SME_SUCCESS;
...
#if 0
static void
limProcessSmeAssocCnfNew (tpAniSirGlobal pMac, tANI_U32 msgType,
			  tANI_U32 * pMsgBuf)
{
  ...
  if (pStaDs == NULL)
    {
      limLog (pMac, LOG1,
	      FL
	      ("Received invalid message %X due to no STA context, for aid %d, peer "),
  ...
  if ((pStaDs &&
	 ...
	 (msgType != eWNI_SME_REASSOC_CNF)))))
    {
      limLog (pMac, LOG1,
	      FL
	      ("Received invalid message %X due to peerMacAddr mismatched or not in eLIM_MLM_WT_ASSOC_CNF_STATE state, for aid %d, peer "),
	      msgType, assocCnf.aid);
      limPrintMacAddr (pMac, assocCnf.peerMacAddr, LOG1);
      goto end;
    }
  ...
  if (psessionEntry->parsedAssocReq[pStaDs->assocId] != NULL)
    {
      if (((tpSirAssocReq) (psessionEntry->parsedAssocReq[pStaDs->assocId]))->
	  assocReqFrame)
	{
...
limSendBeaconInd (tpAniSirGlobal pMac, tpPESession psessionEntry)
{
  ...
  vos_mem_copy ((void *) pBeaconGenParams->bssId,
		(void *) psessionEntry->bssId, SIR_MAC_ADDR_LENGTH);
...
void
limSendScanOffloadComplete (tpAniSirGlobal pMac, tSirResultCodes reasonCode)
{
  tANI_U16 scanRspLen = 0;

  pMac->lim.gLimSmeScanResultLength += pMac->lim.gLimMlmScanResultLength;
  pMac->lim.gLimRspReqd = false;
  if ((reasonCode == eSIR_SME_SUCCESS) || pMac->lim.gLimSmeScanResultLength)
    {
      scanRspLen = sizeof (tSirSmeScanRsp) +
	pMac->lim.gLimSmeScanResultLength - sizeof (tSirBssDescription);
    }
...
void
limProcessRxScanEvent (tpAniSirGlobal pMac, void *buf)
{
...
void
limProcessMlmSpoofMacAddrRsp (tpAniSirGlobal pMac, tSirRetStatus rspStatus)
{
  tANI_U32 val;
  ...
  if (wlan_cfgGetInt (pMac, WNI_CFG_ENABLE_MAC_ADDR_SPOOFING, &val)
    ...
    {
      limLog (pMac, LOGP,
	      FL ("fail to Get WNI_CFG_ENABLE_MAC_ADDR_SPOOFING"));
...
void
limSwitchChannelResumeLinkRsp (tpAniSirGlobal pMac,
