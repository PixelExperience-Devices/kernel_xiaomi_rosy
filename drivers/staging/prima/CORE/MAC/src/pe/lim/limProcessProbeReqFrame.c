...
limSendSmeProbeReqInd (tpAniSirGlobal pMac,
		       ...
		       tANI_U8 * pProbeReqIE,
		       tANI_U32 ProbeReqIELen, tpPESession psessionEntry);
...
void
limGetWPSPBCSessions (tpAniSirGlobal pMac, tANI_U8 * addr,
		      ...
		      tpPESession psessionEntry)
{
  ...
  curTime =
    (tANI_TIMESTAMP) (palGetTickCount (pMac->hHdd) / PAL_TICKS_PER_SECOND);
  ...
  for (pbc = psessionEntry->pAPWPSPBCSession; pbc; pbc = pbc->next)
	...
	break;
      ...
      vos_mem_copy ((tANI_U8 *) addr, (tANI_U8 *) pbc->addr,
		    sizeof (tSirMacAddr));
      vos_mem_copy ((tANI_U8 *) uuid_e, (tANI_U8 *) pbc->uuid_e,
		    SIR_WPS_UUID_LEN);
    }
  ...
  PELOGE (limLog (pMac, LOGE, FL ("overlap = %d"), *overlap);
	     ...
	     (pMac, SIR_LIM_MODULE_ID, LOGE, addr, sizeof (tSirMacAddr));
    )PELOGE (sirDumpBuf
	     (pMac, SIR_LIM_MODULE_ID, LOGE, uuid_e, SIR_WPS_UUID_LEN);
    )return;
}
 ...
 */
static void
limRemoveTimeoutPBCsessions (tpAniSirGlobal pMac, tSirWPSPBCSession * pbc)
{
...
void
limRemovePBCSessions (tpAniSirGlobal pMac, tSirMacAddr pRemoveMac,
		      tpPESession psessionEntry)
{
...
static void
limUpdatePBCSessionEntry (tpAniSirGlobal pMac,
			  ...
			  tpPESession psessionEntry)
{
  ...
  while (pbc)
    {
      if (vos_mem_compare
	  ((tANI_U8 *) pbc->addr, (tANI_U8 *) addr, sizeof (tSirMacAddr))
	  && vos_mem_compare ((tANI_U8 *) pbc->uuid_e, (tANI_U8 *) uuid_e,
			      SIR_WPS_UUID_LEN))
	{
	  if (prev)
	    prev->next = pbc->next;
  ...
  if (!pbc)
      ...
      if (NULL == pbc)
	{
      ...
      vos_mem_copy ((tANI_U8 *) pbc->addr, (tANI_U8 *) addr,
		    sizeof (tSirMacAddr));

      if (uuid_e)
	vos_mem_copy ((tANI_U8 *) pbc->uuid_e, (tANI_U8 *) uuid_e,
		      SIR_WPS_UUID_LEN);
    }
...
void
limWPSPBCTimeout (tpAniSirGlobal pMac, tpPESession psessionEntry)
{
  tANI_TIMESTAMP curTime;
  tSirWPSPBCSession *pbc, *prev = NULL;
  ...
  PELOG3 (limLog
	  (pMac, LOG3, FL ("WPS PBC cleanup timeout curTime=%d"), curTime);
...
void
limWPSPBCClose (tpAniSirGlobal pMac, tpPESession psessionEntry)
{
...
tANI_BOOLEAN
limCheck11bRates (tANI_U8 rate)
{
  if ((0x02 == (rate))
      || (0x04 == (rate)) || (0x0b == (rate)) || (0x16 == (rate)))
    {
      return TRUE;
    }
  return FALSE;
...
void
limProcessProbeReqFrame (tpAniSirGlobal pMac, tANI_U8 * pRxPacketInfo,
			 tpPESession psessionEntry)
{
  tANI_U8 *pBody;
  ...
  do
	   ...
	   (WDA_GET_RX_BEACON_SENT (pRxPacketInfo))))
	{
	  ...
	  PELOG3 (limLog
		  (pMac, LOG3, FL ("Received Probe Request %d bytes from "),
		   frameLen); limPrintMacAddr (pMac, pHdr->sa, LOG3);
	    )
	    // Get pointer to Probe Request frame body
	    pBody = WDA_GET_RX_MPDU_DATA (pRxPacketInfo);

	  // Parse Probe Request frame
	  if (sirConvertProbeReqFrame2Struct
	      (pMac, pBody, frameLen, &probeReq) == eSIR_FAILURE)
	    {
	      PELOGW (limLog (pMac, LOGE, FL ("Parse error ProbeRequest,"
					      " length=%d, SA is:"
					      MAC_ADDRESS_STR), frameLen,
	      ...
	      if (psessionEntry->pePersona == VOS_P2P_GO_MODE)
		{
		  ...
		  for (i = 0; i < probeReq.supportedRates.numRates; i++)
		    {
		      if (limCheck11bRates
			  (probeReq.supportedRates.rate[i] & 0x7f))
			{
			  rate_11b++;
			}
		      else
		  ...
		  for (i = 0; i < probeReq.extendedRates.numRates; i++)
		    {
		      if (limCheck11bRates
			  (probeReq.extendedRates.rate[i] & 0x7f))
			{
			  rate_11b++;
			}
		      else
		  ...
		  if ((rate_11b > 0) && (other_rates == 0))
		    {
		      PELOG3 (limLog (pMac, LOG3,
				      FL
				      ("Received a probe request frame with only 11b rates, SA is: "));
		  ...
		  if ((psessionEntry->APWPSIEs.SirWPSProbeRspIE.FieldPresent &
		      ...
		      if (psessionEntry->fwdWPSPBCProbeReq)
			{
			  PELOG4 (sirDumpBuf (pMac, SIR_LIM_MODULE_ID, LOG4,
				     ...
				     (pMac, SIR_LIM_MODULE_ID, LOG4, pBody,
				      frameLen);
			    )limSendSmeProbeReqInd (pMac, pHdr->sa, pBody,
						    frameLen, psessionEntry);
			}
		      else
			{
		...
		{
		  if (wlan_cfgGetInt
		      (pMac, (tANI_U16) WNI_CFG_WPS_ENABLE,
		       &tmp) != eSIR_SUCCESS)
		  ...
		  if ((wpsApEnable) &&
		       ...
		       WSC_PASSWD_ID_PUSH_BUTTON))
		    {
		      ...
		      pSirSmeProbeReq =
			vos_mem_malloc (sizeof (tSirSmeProbeReq));
		      if (NULL == pSirSmeProbeReq)
			{
			  // Log error
			  limLog (pMac, LOGP,
				  FL
				  ("call to AllocateMemory failed for eWNI_SME_PROBE_REQ"));
			  return;
			}
		      ...
		      pSirSmeProbeReq->sessionId =
			psessionEntry->smeSessionId;
		      vos_mem_copy (pSirSmeProbeReq->peerMacAddr, pHdr->sa,
				    sizeof (tSirMacAddr));
		      pSirSmeProbeReq->devicePasswdId =
			probeReq.probeReqWscIeInfo.DevicePasswordID.id;
		      MTRACE (macTrace
			      (pMac, TRACE_CODE_TX_SME_MSG,
			       psessionEntry->peSessionId, msgQ.type));
		      if (limSysProcessMmhMsgApi (pMac, &msgQ, ePROT) !=
			  eSIR_SUCCESS)
			{
			  PELOG3 (limLog
				  (pMac, LOG3,
				   FL ("couldnt send the probe req to wsm "));
			)}
		    }
		}
	    }
	  ...
	  if (probeReq.ssId.length)
	      ...
	      else if (psessionEntry->pePersona == VOS_P2P_GO_MODE)
		{
		  ...
		  if (vos_mem_compare ((tANI_U8 *) & direct_ssid,
				       (tANI_U8 *) & (probeReq.ssId.ssId),
				       (tANI_U8) (direct_ssid_len)))
		    {
		      limSendProbeRspMgmtFrame (pMac, pHdr->sa, &ssId,
						DPH_USE_MGMT_STAID,
						DPH_NON_KEEPALIVE_FRAME,
	      ...
	      else
		{
		  PELOG3 (limLog (pMac, LOG3,
				  FL
				  ("Ignoring ProbeReq frame with unmatched SSID received from "));
			  limPrintMacAddr (pMac, pHdr->sa, LOG3);
...
void
limProcessProbeReqFrame_multiple_BSS (tpAniSirGlobal pMac, tANI_U8 * pBd,
				      tpPESession psessionEntry)
{
  tANI_U8 i;
		       ...
		       tANI_U32 ProbeReqIELen, tpPESession psessionEntry)
{
  ...
  vos_mem_copy (pSirSmeProbeReqInd->bssId, psessionEntry->bssId,
		sizeof (tSirMacAddr));
  vos_mem_copy (pSirSmeProbeReqInd->WPSPBCProbeReq.peerMacAddr, peerMacAddr,
		sizeof (tSirMacAddr));
  ...
  vos_mem_copy (pSirSmeProbeReqInd->WPSPBCProbeReq.probeReqIE, pProbeReqIE,
		ProbeReqIELen);
...
#ifdef WLAN_FEATURE_P2P_INTERNAL
void
limSendP2PProbeResponse (tpAniSirGlobal pMac, tANI_U8 * pBd,
			 tpPESession psessionEntry)
{
  ...
  if ((pBody[0] == 0) && (pBody[1] == ssId.length) &&
      (vos_mem_compare (ssId.ssId, pBody + 2, ssId.length)))
    {
      // Parse Probe Request frame
      frameLen = WDA_GET_RX_PAYLOAD_LEN (pBd);
      if (eSIR_FAILURE ==
	  sirConvertProbeReqFrame2Struct (pMac, pBody, frameLen, &probeReq))
	{
	  PELOGW (limLog
		  (pMac, LOGW,
	...
	{
	  ssId.length = psessionEntry->ssId.length;
	  vos_mem_copy (ssId.ssId, psessionEntry->ssId.ssId,
