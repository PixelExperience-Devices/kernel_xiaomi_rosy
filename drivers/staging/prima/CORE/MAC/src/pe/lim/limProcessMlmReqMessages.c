...
static void limProcessMlmRemoveKeyReq (tpAniSirGlobal pMac,
				       tANI_U32 * pMsgBuf);
void limSetChannel (tpAniSirGlobal pMac, tANI_U8 channel,
		    tANI_U8 secChannelOffset, tPowerdBm maxTxPower,
		    tANI_U8 peSessionId);
...
limProcessMlmReqMessages (tpAniSirGlobal pMac, tpSirMsgQ Msg)
{
  switch (Msg->type)
    ...
    case SIR_LIM_SAP_ECSA_TIMEOUT:
      lim_process_ap_ecsa_timeout (pMac);
      break;
 ...
 */
void
limSuspendLink (tpAniSirGlobal pMac, tSirLinkTrafficCheck trafficCheck,
		SUSPEND_RESUME_LINK_CALLBACK callback, tANI_U32 * data)
{
  if (NULL == callback)
    {
  ...
  if (pMac->lim.gpLimSuspendCallback || pMac->lim.gLimSystemInScanLearnMode)
    {
      limLog (pMac, LOGE, FL ("Something is wrong, SuspendLinkCbk:%pK "
			      "IsSystemInScanLearnMode:%d"),
	      pMac->lim.gpLimSuspendCallback,
  ...
  limSendHalInitScanReq (pMac, eLIM_HAL_SUSPEND_LINK_WAIT_STATE,
			 trafficCheck);
 ...
 */
void
limResumeLink (tpAniSirGlobal pMac, SUSPEND_RESUME_LINK_CALLBACK callback,
	       tANI_U32 * data)
{
  if (NULL == callback)
    {
  ...
  if (pMac->lim.gpLimResumeCallback)
    {
      limLog (pMac, LOGE,
	      "%s:%d: gLimResumeLink callback is not NULL...something is wrong",
	      __func__, __LINE__);
      callback (pMac, eHAL_STATUS_FAILURE, data);
      return;
    }
  ...
  if (eLIM_HAL_IDLE_SCAN_STATE != pMac->lim.gLimHalScanState)
    ...
    {
      limLog (pMac, LOGW,
	      FL ("Init Scan failed, we will not call finish scan."
		  " calling the callback with failure status"));
      pMac->lim.gpLimResumeCallback (pMac, eSIR_FAILURE,
				     pMac->lim.gpLimResumeData);
 ...
 */
static bool
lim_is_spoofing_needed (tpAniSirGlobal mac)
{
  /*
...
void
limContinuePostChannelScan (tpAniSirGlobal pMac)
{
  tANI_U8 channelNum;
  tANI_U8 i = 0;
  ...
  if ((pMac->lim.gpLimMlmScanReq->scanType == eSIR_ACTIVE_SCAN) &&
      ...
      do
	{
	  ...
	  limLog (pMac, LOG1,
		  FL (" Mac Addr " MAC_ADDRESS_STR
		      " used in sending ProbeReq number %d, for SSID %s on channel: %d"),
		  MAC_ADDR_ARRAY (gSelfMacAddr), i,
		  pMac->lim.gpLimMlmScanReq->ssId[i].ssId, channelNum);
	  // include additional IE if there is
	  status =
	    limSendProbeReqMgmtFrame (pMac,
				      &pMac->lim.gpLimMlmScanReq->ssId[i],
				      pMac->lim.gpLimMlmScanReq->bssId,
				      channelNum, gSelfMacAddr,
	...
	if (pMac->lim.gpLimMlmScanReq->minChannelTime != 0)
	...
	else
	  {
	    ...
	    PELOGE (limLog
		    (pMac, LOGE,
		     "Min channel time == 0, Use only max chan timer");
	      )
#endif
      ...
      else
	{
	  if (pMac->miracast_mode)
		...
		/**
	      ...
	      limLog (pMac, LOGE,
		      FL
		      ("could not retrieve passive max chan value, Use Def val"));
...
void
limCovertChannelScanType (tpAniSirGlobal pMac, tANI_U8 channelNum,
			  tANI_BOOLEAN passiveToActive)
{
  ...
  for (i = 0; (i + 1) < len; i += 2)
	...
	{
	  if ((eSIR_PASSIVE_SCAN == channelPair[i + 1])
	      && TRUE == passiveToActive)
	    {
	      limLog (pMac, LOG1,
		      FL ("Channel %d changed from Passive to Active"),
		      channelNum);
	      channelPair[i + 1] = eSIR_ACTIVE_SCAN;
	      break;
	    }
	  if ((eSIR_ACTIVE_SCAN == channelPair[i + 1])
	      && FALSE == passiveToActive)
	    {
	      limLog (pMac, LOG1,
		      FL ("Channel %d changed from Active to Passive"),
		      channelNum);
  ...
  cfgSetStrNotify (pMac, WNI_CFG_SCAN_CONTROL_LIST, (tANI_U8 *) channelPair,
		   len, FALSE);
  return;
}
...
void
limSetDFSChannelList (tpAniSirGlobal pMac, tANI_U8 channelNum,
  ...
  if ((1 <= channelNum) && (165 >= channelNum))
      ...
      if (eANI_BOOLEAN_TRUE == limIsconnectedOnDFSChannel (channelNum))
	{
	  ...
	  if (!pMac->fActiveScanOnDFSChannels &&
	    ...
	    limLog (pMac, LOGE,
		    FL
		    ("Received beacon on DFS channel %d with dfs time stamp %lu, but channel is still DFS"),
...
void
limDoSendAuthMgmtFrame (tpAniSirGlobal pMac, tpPESession psessionEntry)
{
  ...
  if (tx_timer_activate (&pMac->lim.limTimers.gLimAuthFailureTimer)
      != TX_SUCCESS)
    {
      ...
      //Log error
      limLog (pMac, LOGP, FL ("could not start Auth failure timer"));
  ...
  else
    {
	  ...
	  (&pMac->lim.limTimers.gLimPeriodicAuthRetryTimer) != TX_SUCCESS)
	{
...
*/
static void
__limCreateInitScanRawFrame (tpAniSirGlobal pMac,
			     tpInitScanParams pInitScanParam)
{
  ...
  for (i = 0; i < pMac->lim.maxBssId; i++)
	...
	{
	  if (pMac->lim.gpSession[i].limMlmState ==
	      eLIM_MLM_LINK_ESTABLISHED_STATE)
	    {
	      if ((pMac->lim.gpSession[i].limSystemRole !=
		   eLIM_BT_AMP_STA_ROLE)
		  && (pInitScanParam->scanEntry.activeBSScnt < HAL_NUM_BSSID))
		{
		  pInitScanParam->scanEntry.bssIdx[pInitScanParam->scanEntry.
						   activeBSScnt] =
...
*/
static void
__limCreateFinishScanRawFrame (tpAniSirGlobal pMac,
	  ...
	  if (pMac->lim.gpSession[i].limMlmState ==
	      eLIM_MLM_LINK_ESTABLISHED_STATE)
	    {
		  ...
		  && (pFinishScanParam->scanEntry.activeBSScnt <
		      HAL_NUM_BSSID))
		{
		  pFinishScanParam->scanEntry.bssIdx[pFinishScanParam->
						     scanEntry.activeBSScnt] =
		    pMac->lim.gpSession[i].bssIdx;
		  pFinishScanParam->scanEntry.activeBSScnt++;
		}
	    }
...
void
limSendHalInitScanReq (tpAniSirGlobal pMac, tLimLimHalScanState nextState,
		       tSirLinkTrafficCheck trafficCheck)
{
  ...
  vos_mem_set ((tANI_U8 *) & pInitScanParam->macMgmtHdr,
	       sizeof (tSirMacMgmtHdr), 0);
  ...
  if (rc == eSIR_SUCCESS)
    {
      PELOG3 (limLog
	      (pMac, LOG3,
	       FL
	       ("wdaPostCtrlMsg() return eSIR_SUCCESS pMac=%x nextState=%d"),
  ...
  PELOGW (limLog
	  (pMac, LOGW, FL ("wdaPostCtrlMsg failed, error code %d"), rc);
)error:
  switch (nextState)
    {
	...
	{
	  pMac->lim.gpLimSuspendCallback (pMac, rc,
					  pMac->lim.gpLimSuspendData);
...
void
limSendHalStartScanReq (tpAniSirGlobal pMac, tANI_U8 channelNum,
			tLimLimHalScanState nextState)
{
  tSirMsgQ msg;
  ...
  if (pMac->lim.gLimHalScanState != eLIM_HAL_START_SCAN_WAIT_STATE)
    ...
    {
      PELOGW (limLog
	      (pMac, LOGW, FL ("Invalid state for START_SCAN_REQ message=%d"),
...
void
limSendHalEndScanReq (tpAniSirGlobal pMac, tANI_U8 channelNum,
		      tLimLimHalScanState nextState)
{
  tSirMsgQ msg;
  ...
  if ((pMac->lim.gLimHalScanState != eLIM_HAL_END_SCAN_WAIT_STATE) &&
	...
	default:
	  PELOGW (limLog
		  (pMac, LOGW,
		   FL ("wdaPostCtrlMsg Rcvd invalid nextState %d"),
		   nextState);
	    )break;
	}
      pMac->lim.gLimHalScanState = eLIM_HAL_IDLE_SCAN_STATE;
    ...
    {
      PELOGW (limLog
	      (pMac, LOGW, FL ("Invalid state for END_SCAN_REQ message=%d"),
	       pMac->lim.gLimHalScanState);
    )}
...
void
limSendTLPauseInd (tpAniSirGlobal pMac, uint16_t staId)
{
  tSirMsgQ msg;
  tSirRetStatus rc = eSIR_SUCCESS;
 ...
 */
void
limSendHalFinishScanReq (tpAniSirGlobal pMac, tLimLimHalScanState nextState)
{
  ...
  if (pMac->lim.gLimHalScanState == nextState)
    {
      ...
      PELOGW (limLog
	      (pMac, LOGW,
	       FL ("Next Scan State is same as the current state: %d "),
	       nextState);
	)return;
    }
  ...
  peGetResumeChannel (pMac, &pFinishScanParam->currentOperChannel,
		      &pFinishScanParam->cbState);

  vos_mem_set ((tANI_U8 *) & pFinishScanParam->macMgmtHdr,
	       sizeof (tSirMacMgmtHdr), 0);
...
limContinueChannelScan (tpAniSirGlobal pMac)
{
  ...
  if ((pMac->lim.gLimReturnAfterFirstMatch & 0x40) ||
      (pMac->lim.gLimReturnAfterFirstMatch & 0x80))
    {
      while (pMac->lim.gLimCurrentScanChannelId <=
	     (tANI_U32) (pMac->lim.gpLimMlmScanReq->channelList.numChannels -
			 1))
	{
  ...
  channelNum = limGetCurrentScanChannel (pMac);
  limLog (pMac, LOG1, FL ("Current Channel to be scanned is %d"), channelNum);
  ...
  if (((ePMM_STATE_BMPS_WAKEUP == pMac->pmm.gPmmState) ||
      ...
      for (i = 0; i < pMac->lim.maxBssId; i++)
	{
	  if ((peFindSessionBySessionId (pMac, i) != NULL) &&
	    ...
	    {
	      limReactivateHeartBeatTimer (pMac,
...
void
limSendHalOemDataReq (tpAniSirGlobal pMac)
{
  tSirMsgQ msg;
  ...
  if (NULL == pStartOemDataReq)
    {
      PELOGE (limLog
	      (pMac, LOGE,
	       FL
	       ("OEM_DATA: Could not allocate memory for pStartOemDataReq"));
	)goto error;
    }
  ...
  vos_mem_copy (pStartOemDataReq->oemDataReq,
		pMac->lim.gpLimMlmOemDataReq->oemDataReq, OEM_DATA_REQ_SIZE);
  ...
  PELOGE (limLog
	  (pMac, LOGE,
	   FL ("OEM_DATA: posting WDA_START_OEM_DATA_REQ to HAL failed"));
)error:
  pMac->lim.gLimMlmState = pMac->lim.gLimPrevMlmState;
  MTRACE (macTrace
	  (pMac, TRACE_CODE_MLM_STATE, NO_SESSION, pMac->lim.gLimMlmState));
  ...
  if (NULL == pMlmOemDataRsp)
    {
      limLog (pMac->hHdd, LOGP,
	      FL
	      ("OEM_DATA: memory allocation for pMlmOemDataRsp failed under suspend link failure"));
      return;
    }
...
void
limSetOemDataReqModeFailed (tpAniSirGlobal pMac, eHalStatus status,
			    tANI_U32 * data)
{
  tpLimMlmOemDataRsp pMlmOemDataRsp;
...
void
limSetOemDataReqMode (tpAniSirGlobal pMac, eHalStatus status, tANI_U32 * data)
{
  if (status != eHAL_STATUS_SUCCESS)
    ...
    {
      PELOGE (limLog
	      (pMac, LOGE, FL ("OEM_DATA: Calling limSendHalOemDataReq"));
...
static void
mlm_add_sta (tpAniSirGlobal pMac, tpAddStaParams pSta, tANI_U8 * pBssid, tANI_U8 htCapable, tpPESession psessionEntry)	//psessionEntry  may required in future
{
  tANI_U32 val;
  int i;
  ...
  vos_mem_copy (pSta->staMac, psessionEntry->selfMacAddr,
		sizeof (tSirMacAddr));
  ...
  pSta->htLdpcCapable = (psessionEntry->txLdpcIniFeatureEnabled & 0x01);
  pSta->vhtLdpcCapable =
  ...
  if (IS_DOT11_MODE_HT (psessionEntry->dot11mode))
    {
      ...
      pSta->greenFieldCapable =
	limGetHTCapability (pMac, eHT_GREENFIELD, psessionEntry);
      pSta->txChannelWidthSet =
	limGetHTCapability (pMac, eHT_SUPPORTED_CHANNEL_WIDTH_SET,
			    psessionEntry);
      pSta->mimoPS =
	(tSirMacHTMIMOPowerSaveState) limGetHTCapability (pMac,
							  eHT_MIMO_POWER_SAVE,
							  psessionEntry);
      pSta->rifsMode =
	limGetHTCapability (pMac, eHT_RIFS_MODE, psessionEntry);
      pSta->lsigTxopProtection =
	limGetHTCapability (pMac, eHT_LSIG_TXOP_PROTECTION, psessionEntry);
      pSta->delBASupport =
	limGetHTCapability (pMac, eHT_DELAYED_BA, psessionEntry);
      pSta->maxAmpduDensity =
	limGetHTCapability (pMac, eHT_MPDU_DENSITY, psessionEntry);
      pSta->maxAmsduSize =
	limGetHTCapability (pMac, eHT_MAX_AMSDU_LENGTH, psessionEntry);
      pSta->fDsssCckMode40Mhz =
	limGetHTCapability (pMac, eHT_DSSS_CCK_MODE_40MHZ, psessionEntry);
      pSta->fShortGI20Mhz =
	limGetHTCapability (pMac, eHT_SHORT_GI_20MHZ, psessionEntry);
      pSta->fShortGI40Mhz =
	limGetHTCapability (pMac, eHT_SHORT_GI_40MHZ, psessionEntry);
    }
...
#endif
#ifdef WLAN_FEATURE_11AC
  limPopulateOwnRateSet (pMac, &pSta->supportedRates, NULL, false,
			 psessionEntry, NULL);
#else
  limPopulateOwnRateSet (pMac, &pSta->supportedRates, NULL, false,
			 psessionEntry);
#endif
  limFillSupportedRatesInfo (pMac, NULL, &pSta->supportedRates,
			     psessionEntry);

  limLog (pMac, LOGE,
	  FL
	  ("GF: %d, ChnlWidth: %d, MimoPS: %d, lsigTXOP: %d, dsssCCK: %d, SGI20: %d, SGI40%d"),
	  pSta->greenFieldCapable, pSta->txChannelWidthSet, pSta->mimoPS,
	  pSta->lsigTxopProtection, pSta->fDsssCckMode40Mhz,
	  pSta->fShortGI20Mhz, pSta->fShortGI40Mhz);
...
//
tSirResultCodes
limMlmAddBss (tpAniSirGlobal pMac,
	      tLimMlmStartReq * pMlmStartReq, tpPESession psessionEntry)
{
  tSirMsgQ msgQ;
  ...
  pAddBssParams->cfParamSet.cfpMaxDuration =
    pMlmStartReq->cfParamSet.cfpMaxDuration;
  pAddBssParams->cfParamSet.cfpDurRemaining =
    pMlmStartReq->cfParamSet.cfpDurRemaining;
  ...
  if (pAddBssParams->ssId.length > SIR_MAC_MAX_SSID_LENGTH)
    {
      limLog (pMac, LOGE,
	      ...
	      pAddBssParams->ssId.length, SIR_MAC_MAX_SSID_LENGTH);
      vos_mem_free (pAddBssParams);
      return eSIR_SME_INVALID_PARAMETERS;
    }
  vos_mem_copy (pAddBssParams->ssId.ssId,
		pMlmStartReq->ssId.ssId, pAddBssParams->ssId.length);
...
limProcessMlmStartReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pMlmStartReq->sessionId)) == NULL)
    {
...
#if 0
  if (cfgSetInt (pMac, WNI_CFG_CURRENT_CHANNEL, pMlmStartReq->channelNumber)
      != eSIR_SUCCESS)
  ...
  if (cfgSetStr
      (pMac, WNI_CFG_BSSID, (tANI_U8 *) pMlmStartReq->bssId,
  ...
  vos_mem_copy (pMac->lim.gLimCurrentBssId,
		pMlmStartReq->bssId, sizeof (tSirMacAddr));
#endif //TO SUPPORT BT-AMP

#if 0
  if (cfgSetStr
      (pMac, WNI_CFG_SSID, (tANI_U8 *) & pMlmStartReq->ssId.ssId,
       pMlmStartReq->ssId.length) != eSIR_SUCCESS)
  ...
  if (psessionEntry->limSystemRole == eLIM_AP_ROLE)
    {
      if (pMac->lim.limTimers.gLimWPSOverlapTimerObj.isTimerCreated ==
	  eANI_BOOLEAN_FALSE)
	{
	  if (tx_timer_create (&pMac->lim.limTimers.gLimWPSOverlapTimerObj.gLimWPSOverlapTimer, "PS OVERLAP Timer", limWPSOverlapTimerHandler, SIR_LIM_WPS_OVERLAP_TIMEOUT,	// expiration_input
	  ...
	  pMac->lim.limTimers.gLimWPSOverlapTimerObj.sessionId =
	    pMlmStartReq->sessionId;
	  pMac->lim.limTimers.gLimWPSOverlapTimerObj.isTimerCreated =
	    eANI_BOOLEAN_TRUE;
	  limLog (pMac, LOGE, FL ("Create WPS overlap Timer, session=%d"),
		  pMlmStartReq->sessionId);

	  if (tx_timer_activate
	      (&pMac->lim.limTimers.gLimWPSOverlapTimerObj.
	       gLimWPSOverlapTimer) != TX_SUCCESS)
	    {
...
static tANI_U8
__limMlmScanAllowed (tpAniSirGlobal pMac)
{
  int i;
...
limProcessMlmScanReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if (pMac->lim.gLimSystemInScanLearnMode)
    {
      PELOGE (limLog (pMac, LOGE,
		      FL
		      ("Sending START_SCAN from LIM while one req is pending"));
	)vos_mem_free (pMsgBuf);
      /*Send back a failure */
  ...
  if (__limMlmScanAllowed (pMac) &&
	  ...
	  eSIR_SUCCESS)
	{
	  ...
	  limLog (pMac, LOGP,
		  FL
		  ("could not retrieve passive max channel value use def"));
	  /* use a default value */
	  val = WNI_CFG_PASSIVE_MAXIMUM_CHANNEL_TIME_STADEF;
	}
	...
	{
	  tANI_U8 channelNum =
	    pMac->lim.gpLimMlmScanReq->channelList.channelNumber[i];
	...
	/**
      ...
      limLog (pMac, LOGW,
	      FL
	      ("received unexpected MLM_SCAN_REQ in state %d OR zero number of channels: %d"),
	      pMac->lim.gLimMlmState,
	      ((tLimMlmScanReq *) pMsgBuf)->channelList.numChannels);
...
#ifdef FEATURE_OEM_DATA_SUPPORT
static void
limProcessMlmOemDataReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if (((pMac->lim.gLimMlmState == eLIM_MLM_IDLE_STATE) ||
	...
	/**
      ...
      PELOGW (limLog
	      (pMac, LOGW,
	       FL
	       ("OEM_DATA: unexpected LIM_MLM_OEM_DATA_REQ in invalid state %d"),
	       pMac->lim.gLimMlmState);
	)limPrintMlmState (pMac, LOGW, pMac->lim.gLimMlmState);
	...
	{
	  limPostSmeMessage (pMac, LIM_MLM_OEM_DATA_CNF,
			     (tANI_U32 *) pMlmOemDataRsp);
	  vos_mem_free (pMlmOemDataRsp);
	}
      else
	{
	  limLog (pMac, LOGP,
		  FL ("Could not allocate memory for pMlmOemDataRsp"));
	  return;
	}
    }
 ...
 */
static void
limProcessMlmPostJoinSuspendLink (tpAniSirGlobal pMac, eHalStatus status,
				  tANI_U32 * ctx)
{
  ...
  linkState =
    ((psessionEntry->limSystemRole ==
      eLIM_BT_AMP_STA_ROLE) ? eSIR_LINK_BTAMP_PREASSOC_STATE :
  ...
  limLog (pMac, LOG1, FL ("[limProcessMlmJoinReq]: suspend link success(%d) "
			  ...
			  "and maxtxPower: %d"), status,
	  psessionEntry->peSessionId, chanNum, secChanOffset,
	  psessionEntry->maxTxPower);
  limSetChannel (pMac, chanNum, secChanOffset, psessionEntry->maxTxPower,
		 psessionEntry->peSessionId);
...
limProcessMlmJoinReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if (((psessionEntry->limSystemRole != eLIM_AP_ROLE)
      ...
      &&
      (SIR_MAC_GET_ESS
       (((tpLimMlmJoinReq) pMsgBuf)->bssDescription.capabilityInfo) !=
       SIR_MAC_GET_IBSS (((tpLimMlmJoinReq) pMsgBuf)->bssDescription.
			 capabilityInfo)))
    {
#if 0
      ...
      if (isLimSessionOffChannel (pMac, sessionId))
	{
	  ...
	  if (limIsLinkSuspended (pMac))
	    {
	      limLog (pMac, LOGE,
		      FL ("Link is already suspended for some other"
			  " reason. Return failure on sessionId:%d"),
		      sessionId);
	      goto error;
	    }
	...
	/**
      ...
      limLog (pMac, LOGE,
	      FL
	      ("SessionId:%d Unexpected Join request for role %d state %d "),
...
limProcessMlmAuthReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if (((((psessionEntry->limSystemRole == eLIM_STA_ROLE)
	...
	&& ((psessionEntry->limMlmState == eLIM_MLM_JOINED_STATE)
      ...
      && (limIsGroupAddr (pMac->lim.gpLimMlmAuthReq->peerMacAddr) == false)
      &&
      (limIsAuthAlgoSupported
       (pMac, pMac->lim.gpLimMlmAuthReq->authType, psessionEntry) == true))
    {
	/**
      ...
      if ((((psessionEntry->limSystemRole == eLIM_STA_ROLE)
	   ...
	   &&
	   (vos_mem_compare
	    (pMac->lim.gpLimMlmAuthReq->peerMacAddr, currentBssId,
	     sizeof (tSirMacAddr))))
	  ||
	  (((preAuthNode =
	     limSearchPreAuthList (pMac,
				   pMac->lim.gpLimMlmAuthReq->peerMacAddr)) !=
	    NULL)
	   && (preAuthNode->authType == pMac->lim.gpLimMlmAuthReq->authType)))
	{
	  limLog (pMac, LOG2,
	  ...
	  mlmAuthCnf.resultCode = (tSirResultCodes) eSIR_MAC_SUCCESS_STATUS;
      ...
      else
	{
	  ...
	  if (pMac->lim.gLimNumPreAuthContexts == numPreAuthContexts)
		...
		mlmAuthCnf.resultCode = eSIR_SME_MAX_NUM_OF_PRE_AUTH_REACHED;
      ...
      if (preAuthNode)
	limDeletePreAuthNode (pMac, pMac->lim.gpLimMlmAuthReq->peerMacAddr);
...
limProcessMlmAssocReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pMlmAssocReq->sessionId)) == NULL)
    {
      limLog (pMac, LOGP, FL ("SessionId:%d Session Does not exist"),
  ...
  limLog (pMac, LOG1, FL ("Process Assoc Req on sessionID %d Systemrole %d"
			  "mlmstate %d from: " MAC_ADDRESS_STR),
	  pMlmAssocReq->sessionId, psessionEntry->limSystemRole,
	  psessionEntry->limMlmState,
  ...
  if ((psessionEntry->limSystemRole != eLIM_AP_ROLE
      ...
      &&
      (vos_mem_compare
       (pMlmAssocReq->peerMacAddr, currentBssId, sizeof (tSirMacAddr))))
    {

      /// map the session entry pointer to the AssocFailureTimer 
      pMac->lim.limTimers.gLimAssocFailureTimer.sessionId =
	pMlmAssocReq->sessionId;
	...
	{
	  if (limSetLinkState
	      (pMac, eSIR_LINK_BTAMP_POSTASSOC_STATE, currentBssId,
	  ...
	  TX_SUCCESS)
	{
	  ...
	  limLog (pMac, LOGP,
		  FL
		  ("SessionId:%d could not start Association failure timer"),
		  psessionEntry->peSessionId);
	...
	/**
      ...
      PELOGW (limLog (pMac, LOGW,
		      FL
		      ("received unexpected MLM_ASSOC_CNF in state %d for role=%d, MAC addr= "
...
limProcessMlmReassocReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pMlmReassocReq->sessionId)) == NULL)
    {
      limLog (pMac, LOGE,
  ...
  limLog (pMac, LOG1, FL ("Process ReAssoc Req on sessionID %d Systemrole %d"
			  "mlmstate %d from: " MAC_ADDRESS_STR),
	  pMlmReassocReq->sessionId, psessionEntry->limSystemRole,
	  psessionEntry->limMlmState,
  ...
  if (((psessionEntry->limSystemRole != eLIM_AP_ROLE)
	...
	/** Switch channel to the new Operating channel for Reassoc*/
      ...
      limSetChannel (pMac, chanNum, secChannelOffset,
	...
	/**
      ...
      limLog (pMac, LOGW,
	      FL
	      ("received unexpected MLM_REASSOC_CNF in state %d for role=%d, "
	       "MAC addr= " MAC_ADDRESS_STR), psessionEntry->limMlmState,
	      psessionEntry->limSystemRole,
	      MAC_ADDR_ARRAY (pMlmReassocReq->peerMacAddr));
      limPrintMlmState (pMac, LOGW, psessionEntry->limMlmState);
...
static void
limProcessMlmDisassocReqNtf (tpAniSirGlobal pMac, eHalStatus suspendStatus,
			     tANI_U32 * pMsgBuf)
{
  tANI_U16 aid;
  tSirMacAddr currentBssId;
  ...
  if (eHAL_STATUS_SUCCESS != suspendStatus)
    {
      PELOGE (limLog
	      (pMac, LOGE, FL ("Suspend Status is not success %X"),
	       suspendStatus);
	)
#if 0
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pMlmDisassocReq->sessionId)) == NULL)
    {
  ...
  limLog (pMac, LOG1, FL ("Process DisAssoc Req on sessionID %d Systemrole %d"
			  "mlmstate %d from: " MAC_ADDRESS_STR),
	  pMlmDisassocReq->sessionId, psessionEntry->limSystemRole,
	  psessionEntry->limMlmState,
  ...
  switch (psessionEntry->limSystemRole)
			    ...
			    currentBssId, sizeof (tSirMacAddr)))
	{
	  PELOGW (limLog (pMac, LOGW,
			  FL
			  ("received MLM_DISASSOC_REQ with invalid BSS id from: "
			   MAC_ADDRESS_STR),
			  MAC_ADDR_ARRAY (pMlmDisassocReq->peerMacAddr));
	    )
     ...
     */
  pStaDs =
    dphLookupHashEntry (pMac, pMlmDisassocReq->peerMacAddr, &aid,
  ...
  if ((pStaDs == NULL)
	  ...
	  ((pStaDs->mlmStaContext.mlmState != eLIM_MLM_LINK_ESTABLISHED_STATE)
	   ...
	   && (pStaDs->mlmStaContext.mlmState != eLIM_MLM_ASSOCIATED_STATE))))
    {
	/**
      ...
      limLog (pMac, LOGE,
	      FL
	      ("received MLM_DISASSOC_REQ for STA that either has no context "
	       "or in some transit state, Addr= " MAC_ADDRESS_STR),
	      MAC_ADDR_ARRAY (pMlmDisassocReq->peerMacAddr));
      if (pStaDs != NULL)
      ...
      if (NULL == pSirSmeDisassocRsp)
	{
	  ...
	  limLog (pMac, LOGP,
		  FL
		  ("call to AllocateMemory failed for eWNI_SME_DISASSOC_RSP"));
	  return;
	}
  ...
  limPostSmeMessage (pMac,
		     LIM_MLM_DISASSOC_CNF, (tANI_U32 *) & mlmDisassocCnf);
}

tANI_BOOLEAN
limCheckDisassocDeauthAckPending (tpAniSirGlobal pMac, tANI_U8 * staMac)
{
  tLimMlmDisassocReq *pMlmDisassocReq;
...
void
limCleanUpDisassocDeauthReq (tpAniSirGlobal pMac,
			     tANI_U8 * staMac, tANI_BOOLEAN cleanRxPath)
{
  tLimMlmDisassocReq *pMlmDisassocReq;
  tLimMlmDeauthReq *pMlmDeauthReq;
  pMlmDisassocReq = pMac->lim.limDisassocDeauthCnfReq.pMlmDisassocReq;
...
void
limProcessDisassocAckTimeout (tpAniSirGlobal pMac)
{
  limLog (pMac, LOG1, FL (""));
  limSendDisassocCnf (pMac);
}
...
limProcessMlmDisassocReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  limProcessMlmDisassocReqNtf (pMac, eHAL_STATUS_SUCCESS,
			       (tANI_U32 *) pMsgBuf);

} /*** limProcessMlmDisassocReq() ***/

static void
limProcessMlmDeauthReqNtf (tpAniSirGlobal pMac, eHalStatus suspendStatus,
			   tANI_U32 * pMsgBuf)
{
  tANI_U16 aid;
  tSirMacAddr currentBssId;
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pMlmDeauthReq->sessionId)) == NULL)
    {
  ...
  limLog (pMac, LOG1, FL ("Process Deauth Req on sessionID %d Systemrole %d"
			  "mlmstate %d from: " MAC_ADDRESS_STR),
	  pMlmDeauthReq->sessionId, psessionEntry->limSystemRole,
	  psessionEntry->limMlmState,
  ...
  switch (psessionEntry->limSystemRole)
	...
	default:
	  ...
	  PELOGW (limLog (pMac, LOGW,
			  FL
			  ("received MLM_DEAUTH_REQ with in state %d for peer "
			   MAC_ADDRESS_STR), psessionEntry->limMlmState,
			  MAC_ADDR_ARRAY (pMlmDeauthReq->peerMacAddr));
	    )limPrintMlmState (pMac, LOGW, psessionEntry->limMlmState);
    ...
    case eLIM_STA_IN_IBSS_ROLE:
      limLog (pMac, LOGE, FL ("received MLM_DEAUTH_REQ IBSS Mode "));
      mlmDeauthCnf.resultCode = eSIR_SME_INVALID_PARAMETERS;
      goto end;
    default:			// eLIM_AP_ROLE
      break;
     ...
     */
  pStaDs =
    dphLookupHashEntry (pMac, pMlmDeauthReq->peerMacAddr, &aid,
			&psessionEntry->dph.dphHashTable);

  if (pStaDs == NULL)
    {
      /// Check if there exists pre-auth context for this STA
      pAuthNode = limSearchPreAuthList (pMac, pMlmDeauthReq->peerMacAddr);
      ...
      if (pAuthNode == NULL)
	{
	  ...
	  PELOGW (limLog (pMac, LOGW,
			  FL
			  ("received MLM_DEAUTH_REQ in mlme state %d for STA that "
	...
	/**
      ...
      PELOGW (limLog (pMac, LOGW,
		      FL
		      ("received MLM_DEAUTH_REQ for STA that either has no context or in some transit state, Addr="
		       MAC_ADDRESS_STR),
		      MAC_ADDR_ARRAY (pMlmDeauthReq->peerMacAddr));
	)
	/*
  ...
  vos_mem_copy ((tANI_U8 *) & mlmDeauthCnf.peerMacAddr,
		(tANI_U8 *) pMlmDeauthReq->peerMacAddr, sizeof (tSirMacAddr));
  ...
  limPostSmeMessage (pMac, LIM_MLM_DEAUTH_CNF, (tANI_U32 *) & mlmDeauthCnf);

}


void
limProcessDeauthAckTimeout (tpAniSirGlobal pMac)
{
  limLog (pMac, LOG1, FL (""));
  limSendDeauthCnf (pMac);
}
...
void
lim_process_ap_ecsa_timeout (tpAniSirGlobal mac_ctx)
{
  tSirMsgQ msg = { 0 };
  ...
  if (session->gLimChannelSwitch.switchCount > 0)
    {
      ...
      lim_send_chan_switch_action_frame (mac_ctx,
					 session->gLimChannelSwitch.
					 primaryChannel, session);
      mac_ctx->lim.limTimers.g_lim_ap_ecsa_timer.sessionId =
	session->peSessionId;
	...
	{
	  limLog (mac_ctx, LOGE,
		  FL ("Couldn't activate g_lim_ap_ecsa_timer"));
...
void
lim_send_sme_ap_channel_switch_resp (tpAniSirGlobal mac_ctx,
				     tpPESession session,
				     tpSwitchChannelParams chan_params)
{
  tSirMsgQ msg = { 0 };
  ...
  if (chan_params->channelNumber == session->currentOperChannel)
  ...
  else
    {
      limLog (mac_ctx, LOGE,
	      FL
	      ("channel switch resp chan %d and session channel doesnt match %d"),
...
limProcessMlmSetKeysReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pMlmSetKeysReq->sessionId)) == NULL)
    {
      PELOGE (limLog
  ...
  limLog (pMac, LOGW,
	  ...
	  pMlmSetKeysReq->aid,
	  pMlmSetKeysReq->edType, pMlmSetKeysReq->numKeys);
  limPrintMacAddr (pMac, pMlmSetKeysReq->peerMacAddr, LOGW);

#if 0
  if (eSIR_SUCCESS !=
      wlan_cfgGetStr (pMac, WNI_CFG_BSSID, currentBssId, &cfg))
    {
      limLog (pMac, LOGP, FL ("Could not retrieve BSSID"));
      return;
    }
  ...
  if (limIsAddrBC (pMlmSetKeysReq->peerMacAddr))
     ...
     */
      ...
      pStaDs =
	dphLookupHashEntry (pMac, pMlmSetKeysReq->peerMacAddr, &aid,
			    &psessionEntry->dph.dphHashTable);
	...
	/**
	  ...
	  limLog (pMac, LOG1,
		  FL
		  ("Received MLM_SETKEYS_REQ for STA that either has no context or in some transit state, Addr = "));
	  limPrintMacAddr (pMac, pMlmSetKeysReq->peerMacAddr, LOGW);
  ...
  if ((pMlmSetKeysReq->numKeys == 0)
      && (pMlmSetKeysReq->edType != eSIR_ED_NONE))
    {
      //
  ...
  limLog (pMac, LOG1,
	  FL
	  ("Trying to set keys for STA Index [%d], using defaultKeyId [%d]"),
	  staIdx, defaultKeyId);
  ...
  if (limIsAddrBC (pMlmSetKeysReq->peerMacAddr))
  ...
  else
    {
      ...
      limSendSetStaKeyReq (pMac, pMlmSetKeysReq, staIdx,
			   (tANI_U8) defaultKeyId, psessionEntry);
      return;
    }
...
limProcessMlmRemoveKeyReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pMlmRemoveKeyReq->sessionId)) == NULL)
    {
      PELOGE (limLog (pMac, LOGE,
  ...
  if (eSIR_SUCCESS != wlan_cfgGetStr (pMac,
				      WNI_CFG_BSSID, currentBssId, &cfg))
    limLog (pMac, LOGP, FL ("Could not retrieve BSSID"));
#endif //TO-SUPPORT BT-AMP
  sirCopyMacAddr (currentBssId, psessionEntry->bssId);
  ...
  /**
    ...
    */
  pStaDs =
    dphLookupHashEntry (pMac, pMlmRemoveKeyReq->peerMacAddr, &aid,
			&psessionEntry->dph.dphHashTable);
  if ((pStaDs == NULL)
      || (pStaDs
	  && (pStaDs->mlmStaContext.mlmState !=
	      eLIM_MLM_LINK_ESTABLISHED_STATE)))
    {
     /**
      ...
      limLog (pMac, LOGW,
	      FL
	      ("Received MLM_REMOVEKEYS_REQ for STA that either has no context or in some transit state, Addr = "));
      limPrintMacAddr (pMac, pMlmRemoveKeyReq->peerMacAddr, LOGW);
  ...
  limPostSmeRemoveKeyCnf (pMac,
			  psessionEntry, pMlmRemoveKeyReq, &mlmRemoveKeyCnf);
...
limProcessMinChannelTimeout (tpAniSirGlobal pMac)
{
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac,
				 pMac->lim.limTimers.gLimMinChannelTimer.
				 sessionId)) == NULL)
    {
      limLog (pMac, LOGP, FL ("Session Does not exist for given sessionID"));
      return;
  ...
  if (pMac->lim.gLimMlmState == eLIM_MLM_WT_PROBE_RESP_STATE &&
      ...
      else
	{
	  ...
	  if (pMac->lim.gpLimMlmScanReq->channelList.channelNumber)
	    {
	      channelNum =
		pMac->lim.gpLimMlmScanReq->channelList.channelNumber[pMac->
								     lim.
								     gpLimMlmScanReq->
								     channelList.
								     numChannels
      ...
      limLog (pMac, LOGW,
	      FL
	      ("Sending End Scan req from MIN_CH_TIMEOUT in state %d ch-%d"),
    ...
    /**
      ...
      limLog (pMac, LOGW,
	      FL
	      ("received unexpected MIN channel timeout in mlme state %d and hal scan State %d"),
...
limProcessMaxChannelTimeout (tpAniSirGlobal pMac)
{
  ...
  if ((pMac->lim.gLimMlmState == eLIM_MLM_WT_PROBE_RESP_STATE ||
      ...
      else
	{
      ...
      limLog (pMac, LOGW,
	      FL
	      ("Sending End Scan req from MAX_CH_TIMEOUT in state %d on ch-%d"),
	      pMac->lim.gLimMlmState, channelNum);
      limSendHalEndScanReq (pMac, channelNum, eLIM_HAL_END_SCAN_WAIT_STATE);
    }
  else
	...
	/**
      ...
      limLog (pMac, LOGW,
	      FL
	      ("received unexpected MAX channel timeout in mlme state %d and hal scan state %d"),
	      pMac->lim.gLimMlmState, pMac->lim.gLimHalScanState);
      limPrintMlmState (pMac, LOGW, pMac->lim.gLimMlmState);
    }
} /*** limProcessMaxChannelTimeout() ***/
  ...
  if ((pMac->lim.gLimMlmState == eLIM_MLM_WT_PROBE_RESP_STATE) &&
      ...
      do
	{
	  ...
	  status = limSendProbeReqMgmtFrame (pMac, &pLimMlmScanReq->ssId[i],
					     pLimMlmScanReq->bssId,
					     channelNum, gSelfMacAddr,
					     pLimMlmScanReq->dot11mode,
					     pLimMlmScanReq->uIEFieldLen,
					     (tANI_U8 *) (pLimMlmScanReq) +
					     pLimMlmScanReq->uIEFieldOffset);
	  ...
	  if (status != eSIR_SUCCESS)
	    {
	      PELOGE (limLog (pMac, LOGE, FL ("send ProbeReq failed for SSID "
					      "%s on channel: %d"),
			      pLimMlmScanReq->ssId[i].ssId, channelNum);
		)return;
	    }
	  i++;
	}
...
limProcessJoinFailureTimeout (tpAniSirGlobal pMac)
{
  ...
  if (psessionEntry->limMlmState == eLIM_MLM_WT_JOIN_BEACON_STATE)
	...
	/**
         ...
         */
      PELOGE (limLog
	      (pMac, LOGE, FL (" In state eLIM_MLM_WT_JOIN_BEACON_STATE."));
	)PELOGE (limLog
		 (pMac, LOGE,
		  FL
		  (" Join Failure Timeout occurred for session %d with BSS "),
		  psessionEntry->peSessionId);
	...
	/**
         ...
         */
      limLog (pMac, LOGW,
	      FL ("received unexpected JOIN failure timeout in state %d"),
	      psessionEntry->limMlmState);
      limPrintMlmState (pMac, LOGW, psessionEntry->limMlmState);
    }
} /*** limProcessJoinFailureTimeout() ***/
...
static void
limProcessPeriodicJoinProbeReqTimer (tpAniSirGlobal pMac)
{
  tpPESession psessionEntry;
  tSirMacSSid ssId;
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac,
				 pMac->lim.limTimers.
				 gLimPeriodicJoinProbeReqTimer.sessionId)) ==
      NULL)
    {
      limLog (pMac, LOGE,
	      FL ("session does not exist for given SessionId : %d"),
  ...
  if ((VOS_TRUE ==
      ...
      && (psessionEntry->limMlmState == eLIM_MLM_WT_JOIN_BEACON_STATE))
    {
      ...
      limSendProbeReqMgmtFrame (pMac, &ssId,
				psessionEntry->pLimMlmJoinReq->bssDescription.
				bssId,
				psessionEntry->
				currentOperChannel /*chanNum */ ,
				psessionEntry->selfMacAddr,
				psessionEntry->dot11mode,
				psessionEntry->pLimJoinReq->addIEScan.length,
				psessionEntry->pLimJoinReq->addIEScan.
				addIEdata);
      ...
      if (tx_timer_activate
	  (&pMac->lim.limTimers.gLimPeriodicJoinProbeReqTimer) != TX_SUCCESS)
	{
	  limLog (pMac, LOGP,
...
static void
limProcessAuthRetryTimer (tpAniSirGlobal pMac)
{
  tpPESession psessionEntry;
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac,
				 pMac->lim.limTimers.
				 gLimPeriodicAuthRetryTimer.sessionId)) ==
      NULL)
    {
      limLog (pMac, LOGE,
	      FL ("session does not exist for given SessionId : %d"),
  ...
  switch (psessionEntry->limMlmState)
	  ...
	  && psessionEntry->defaultAuthFailureTimeout)
	{
	  ccmCfgSetInt (pMac, WNI_CFG_AUTHENTICATE_FAILURE_TIMEOUT,
			psessionEntry->defaultAuthFailureTimeout, NULL,
			eANI_BOOLEAN_FALSE);
	}
      limRestoreFromAuthState (pMac, eSIR_SME_AUTH_TIMEOUT_RESULT_CODE,
    ...
    default:
      ...
      PELOGE (limLog
	      (pMac, LOGE,
	       FL ("received unexpected AUTH failure timeout in state %d"),
	       psessionEntry->limMlmState);
	)limPrintMlmState (pMac, LOGE, psessionEntry->limMlmState);
...
limProcessAuthRspTimeout (tpAniSirGlobal pMac, tANI_U32 authIndex)
{
  ...
  pAuthNode =
    limGetPreAuthNodeFromIndex (pMac, &pMac->lim.gLimPreAuthTimerTable,
				authIndex);
  ...
  if ((psessionEntry =
       peFindSessionByBssid (pMac, pAuthNode->peerMacAddr,
			     &sessionId)) == NULL)
    {
...
limProcessAssocFailureTimeout (tpAniSirGlobal pMac, tANI_U32 MsgType)
{
  ...
  limSendDeauthMgmtFrame (pMac, eSIR_MAC_UNSPEC_FAILURE_REASON,
			  psessionEntry->bssId, psessionEntry, FALSE);
  ...
  if (((psessionEntry->limSystemRole == eLIM_AP_ROLE) ||
	...
	/**
      ...
      limLog (pMac, LOGW,
	      FL
	      ("received unexpected REASSOC failure timeout in state %d for role %d"),
	      psessionEntry->limMlmState, psessionEntry->limSystemRole);
      limPrintMlmState (pMac, LOGW, psessionEntry->limMlmState);
    }
  else
	...
	{
	  PELOGE (limLog
		  (pMac, LOGE, FL ("(Re)Assoc Failure Timeout occurred."));
	    )psessionEntry->limMlmState = eLIM_MLM_IDLE_STATE;
	  MTRACE (macTrace
		  (pMac, TRACE_CODE_MLM_STATE, psessionEntry->peSessionId,
		   psessionEntry->limMlmState));
	  ...
	  mlmAssocCnf.resultCode = eSIR_SME_ASSOC_TIMEOUT_RESULT_CODE;
	  mlmAssocCnf.protStatusCode = eSIR_MAC_UNSPEC_FAILURE_STATUS;
 ...
 */
void
limProcessMlmAddBAReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  tSirRetStatus status = eSIR_SUCCESS;
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pMlmAddBAReq->sessionId)) == NULL)
    {
      PELOGE (limLog (pMac, LOGE,
		      FL ("session does not exist for given sessionId"));
	)vos_mem_free (pMsgBuf);
  ...
  if (eSIR_SUCCESS != status)
      ...
      else
	{
	  ...
	  limPostSmeMessage (pMac,
			     LIM_MLM_ADDBA_CNF, (tANI_U32 *) pMlmAddBACnf);
	}
 ...
 */
void
limProcessMlmAddBARsp (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pMlmAddBARsp->sessionId)) == NULL)
    {
      PELOGE (limLog (pMac, LOGE,
  ...
  if (eSIR_SUCCESS != limSendAddBARsp (pMac, pMlmAddBARsp, psessionEntry))
    {
      limLog (pMac, LOGE, FL ("Failed to send ADDBA Rsp to peer "));
 ...
 */
void
limProcessMlmDelBAReq (tpAniSirGlobal pMac, tANI_U32 * pMsgBuf)
{
  tSirRetStatus status = eSIR_SUCCESS;
  ...
  if ((psessionEntry =
       peFindSessionBySessionId (pMac, pMlmDelBAReq->sessionId)) == NULL)
    {
      PELOGE (limLog
  ...
  if (eSIR_SUCCESS !=
  ...
  else
    {
      ...
      // to cleanup the HAL and SoftMAC entries
      pSta =
	dphLookupHashEntry (pMac, pMlmDelBAReq->peerMacAddr, &aid,
			    &psessionEntry->dph.dphHashTable);
      if (NULL != pSta)
	{
  ...
  if (NULL == pMlmDelBACnf)
  ...
  else
    {
      ...
      vos_mem_copy ((void *) pMlmDelBACnf,
		    (void *) pMlmDelBAReq, sizeof (tLimMlmDelBAReq));
      ...
      limPostSmeMessage (pMac, LIM_MLM_DELBA_CNF, (tANI_U32 *) pMlmDelBACnf);
    }
...
tSirRetStatus
limSMPowerSaveStateInd (tpAniSirGlobal pMac,
			tSirMacHTMIMOPowerSaveState state)
{
  ...
  if (pMac->lim.gLimSystemRole != eSYSTEM_STA_ROLE)
    {
      PELOGE (limLog
	      (pMac, LOGE,
	       FL ("Got PMC indication when System not in the STA Role"));
	)return eSIR_FAILURE;
    }
  ...
  if ((pMac->lim.gHTMIMOPSState == state) || (state == eSIR_HT_MIMO_PS_NA))
    {
      PELOGE (limLog
	      (pMac, LOGE,
	       FL
	       ("Got Indication when already in the same mode or State passed is NA:%d "),
	       state);
	)return eSIR_FAILURE;
    }
  ...
  PELOG2 (limLog
	  (pMac, LOG2,
	   FL (" The HT Capability for Mimo Pwr is updated to State: %u  "),
	   state);
    )if (pMac->lim.gLimSmeState != eLIM_SME_LINK_EST_STATE)
    {
      PELOG2 (limLog
	      (pMac, LOG2,
	       FL
	       (" The STA is not in the Connected/Link Est Sme_State: %d  "),
  ...
  if (!pSta->mlmStaContext.htCapability)
    {
      limLog (pMac, LOGE,
	      FL ("limSendSMPowerState: Peer is not HT Capable "));
      return eSIR_FAILURE;
    }
  ...
  if (isEnteringMimoPS (pMac->lim.gHTMIMOPSState, state))
	...
	/** Obtain the AP's Mac Address */
      vos_mem_copy ((tANI_U8 *) macAddr, psessionEntry->bssId,
		    sizeof (tSirMacAddr));
      ...
      if (retStatus != eSIR_SUCCESS)
	{
	  PELOGE (limLog
		  (pMac, LOGE,
		   "Update SM POWER: Sending Action Frame has failed");
	    )return retStatus;
	}
    }
...
#ifdef WLAN_FEATURE_11AC
ePhyChanBondState
limGet11ACPhyCBState (tpAniSirGlobal pMac, tANI_U8 channel,
		      tANI_U8 htSecondaryChannelOffset,
		      tANI_U8 peerCenterChan, tpPESession psessionEntry)
{
...
void
limSetChannel (tpAniSirGlobal pMac, tANI_U8 channel, tANI_U8 secChannelOffset,
	       tPowerdBm maxTxPower, tANI_U8 peSessionId)
{
  ...
  if (wlan_cfgGetInt
    ...
    {
      limLog (pMac, LOGP,
	      FL ("could not read WNI_CFG_LOCAL_POWER_CONSTRAINT from CFG"));
      return;
    }
